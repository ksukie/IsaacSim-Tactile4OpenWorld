...
import tu_tactile_field as tactile_field_v9
...
EPS = 1.0e-12
FROZEN_HASHES_BEFORE = tactile_field_v9.capture_frozen_hashes()


parser = argparse.ArgumentParser(
    description=(
        "V5 new 9 TU tactile field rendering. The passed v8 Piper close/hold/release trajectory "
        "moves the link8-mounted UIPC membrane against a fixed rigid grasp object. After an explicit "
        "no-contact warm-up and rest capture, frozen 7g per-vertex terms are conservatively splatted "
        "to fixed Fx/Fy/Fz TU grids and fixed-range videos."
    )
)
...
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_9_tu_tactile_field_rendering")
parser.add_argument("--workspace_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_9_tu_tactile_workspace")
...
parser.add_argument("--uipc_warmup_steps", type=int, default=30)
...
# Freeze the effective schedule of the passed 225-frame v8 artifact: five cycles,
# each containing 5 pre-contact and 10 close/hold/release/recovery frames.
parser.add_argument("--pre_contact_frames", type=int, default=5)
parser.add_argument("--close_frames", type=int, default=10)
parser.add_argument("--hold_frames", type=int, default=10)
parser.add_argument("--release_frames", type=int, default=10)
parser.add_argument("--recovery_frames", type=int, default=10)
...
parser.add_argument("--field_height", type=int, default=81)
parser.add_argument("--field_width", type=int, default=65)
...
_OWT_UIPC_SOURCE = _OWT_MAINLINE_ROOT / "packages" / "uipc"
...
def main() -> None:
    ...
    metadata = {
        "script_version": "v5_new_8_grasp_tactile_integration_v1_frozen_source_for_v9",
        "architecture": (
        ...
        "visual_camera_surface_mapping_stage": "v9 force-conserving TU field after frozen 7g",
    ...
    v9_verdict = tactile_field_v9.build_tactile_field_outputs(
        output_dir=output_dir,
        contract_dir=contract_dir,
        source_v8_metadata=metadata,
        source_v8_verdict=verdict,
        ...
        expected_frame_count=int(args_cli.expected_frame_count),
        expected_cycle_count=int(args_cli.grasp_cycles),
        no_contact_tolerance_tu=float(args_cli.accept_max_warmup_force_tu),
        video_fps=float(args_cli.video_fps),
    )
    print(json.dumps(v9_verdict, indent=2), flush=True)
    if bool(args_cli.fail_on_verdict_fail) and not bool(
        v9_verdict["v5_new_9_tu_tactile_field_rendering_passed"]
    ):
        raise RuntimeError(f"v5_new_9 TU tactile field rendering failed: {v9_verdict}")
    ...
    try:
        ...
        try:
            ...
            (output_dir / "error.json").write_text(
                json.dumps(
                    {
                        "script_version": "v5_new_9_tu_tactile_field_rendering_v1",
