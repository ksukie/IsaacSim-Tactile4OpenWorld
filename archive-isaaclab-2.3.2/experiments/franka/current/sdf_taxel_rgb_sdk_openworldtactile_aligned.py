...
"""
Run Franka IK-Abs nut lift and preview OpenWorldTactile-aligned SDF-taxel OpenWorldTactile RGB.
...
* OpenWorldTactile-style height/pressure comes from SDF penetration depth;
* shear force comes from relative tangential velocity with a Coulomb cap;
* penetration depth drives OpenWorldTactile RGB hue with a fixed OpenWorldTactile scale;
* local shear is passed as a OpenWorldTactile-style displacement map for SDK fx/fy.

It does not fall back to the older COP or friction scripts if an SDF collision mesh is missing.

.. code-block:: bash

    ./isaaclab.sh -p experiments/franka/current/sdf_taxel_rgb_sdk_openworldtactile_aligned.py
...
parser.add_argument(
    "--output_dir",
    type=str,
    default="outputs/franka_lift_object_sdf_taxel_rgb_openworldtactile_aligned",
...
parser.add_argument(
    ...
    type=float,
    default=6.0e-4,
    help="OpenWorldTactile-style penetration/depth full scale for OpenWorldTactile pressure rendering.",
)
parser.add_argument(
    "--openworldtactile_displacement_scale",
    type=float,
    default=12000.0,
    help="OpenWorldTactile-style scale applied by the renderer to shear displacement maps.",
)
parser.add_argument(
    "--openworldtactile_saturation",
    type=int,
    default=255,
    help="HSV saturation for generated OpenWorldTactile RGB; 255 keeps the no-contact baseline visibly red.",
)
...
parser.add_argument(
    ...
    action="store_true",
    default=True,
    help="Show a live Isaac UI preview of the left/right OpenWorldTactile-aligned OpenWorldTactile RGB and SDK fxyz maps.",
)
parser.add_argument(
    "--hide_rgb_maps",
    action="store_false",
...
def openworldtactile_fxyz_arrow_scale() -> float:
    ...
    rows, cols = pad_image_shape()
    return 8.0 * min(rows, cols) / 240.0
...
def height_map_rgb_scale(height_map: np.ndarray) -> float:
    """Return the SDF penetration scale used by the OpenWorldTactile-aligned renderer."""
...
def force_map_to_gray_image(height_map: np.ndarray) -> np.ndarray:
    """Map one height/penetration map to an 8-bit grayscale image using the OpenWorldTactile pressure scale."""
...
def make_openworldtactile_renderer() -> OpenWorldTactileRGBRenderer:
    """Create one OpenWorldTactile-aligned textured OpenWorldTactile renderer."""
    rows, cols = pad_image_shape()
    return OpenWorldTactileRGBRenderer(
        ...
        base_hue=0.0,
        max_hue=120.0,
        saturation=args_cli.openworldtactile_saturation,
        base_value=220,
        ...
        displacement_scale=args_cli.openworldtactile_displacement_scale,
    )


def render_openworldtactile_rgb(pad_maps: ContactPadMaps, renderer: OpenWorldTactileRGBRenderer) -> np.ndarray:
    """Render one OpenWorldTactile-style virtual pad from SDF penetration and shear maps."""
    return renderer.render(
        pad_maps.height_map,
        displacement=pad_maps.displacement,
    )
...
def main():
    ...
    if args_cli.openworldtactile_saturation < 0 or args_cli.openworldtactile_saturation > 255:
        raise ValueError("--openworldtactile_saturation must be in [0, 255].")
    ...
    print(
        "[INFO]: OpenWorldTactile-aligned OpenWorldTactile textured RGB renderer "
        f"rgb_size={pad_image_shape()} "
        f"max_pressure={args_cli.openworldtactile_max_pressure} "
        f"saturation={args_cli.openworldtactile_saturation} "
        ...
        f"displacement_scale={args_cli.openworldtactile_displacement_scale}."
    )
    print(
