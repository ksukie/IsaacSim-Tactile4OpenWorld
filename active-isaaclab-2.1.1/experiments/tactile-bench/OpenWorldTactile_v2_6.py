...
parser = argparse.ArgumentParser(
    description=(
        "V2.6 OpenWorldTactile UIPC mechanics-first bench. It keeps fxyz from UIPC "
        "membrane surface deformation, separates display from raw mechanics, "
        "and writes force/area/compression summaries for validation."
    )
)
parser.add_argument(
    "--shape",
    ...
    help="Indenter geometry used to press the membrane. V2.6 defaults to the small cuboid contact tool.",
)
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_surface_force_v2_6")
parser.add_argument("--indent_depth_mm", type=float, default=0.25)
...
parser.add_argument(
    ...
    default=True,
    action="store_true",
    help="Run the bench without writing fxyz, metadata, preview frames, or videos. This is the V2.6 default.",
)
parser.add_argument(
    "--save_output",
    dest="no_save",
...
parser.add_argument(
    ...
    default=True,
    action="store_true",
    help="Repeat the press trajectory until the app closes or Ctrl+C is pressed. This is the V2.6 default.",
)
parser.add_argument(
    "--single_run",
    dest="loop_forever",
...
parser.add_argument("--front_face_eps_mm", type=float, default=0.03)
...
def _mechanics_sequence_summary(
    ...
    *,
    ...
    summary: dict[str, object] = {
        "version": "V2.6",
        "force_source": "surface_deformation_constitutive",
...
def main() -> None:
    ...
    if args_cli.display_tactile:
        if omni_ui is None:
            ...
            title = "OpenWorldTactile UIPC V2.6 Live fxyz"
            if display_tactile_camera:
                display_width += display_camera_panel_width
                title = "OpenWorldTactile UIPC V2.6 Live fxyz + visual camera"
    ...
    print(
        "[INFO] OpenWorldTactileBench V2.6 started: "
        ...
        f"force_source=surface_deformation_constitutive, render_viewport={args_cli.render_viewport}, "
    ...
    if not should_save:
        print(
            "[INFO] OpenWorldTactileBench V2.6 complete: "
    ...
    print(
        ...
        f"mechanics_summary={mechanics_summary_path}, max_conservation_error={max_conservation_error:.6f}",
        flush=True,
    )
