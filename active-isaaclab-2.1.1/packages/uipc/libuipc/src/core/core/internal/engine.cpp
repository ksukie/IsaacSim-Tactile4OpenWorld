...
class Engine::Impl
{
  ...
  public:
    ...
    bool do_write_global_vertex_pos_pair_to_sim(span<const Vector3> previous_positions, span<const Vector3> current_positions, IndexT global_vertex_offset, SizeT vertex_count)
    {
        LogPatternGuard guard{backend_name()};
        return m_engine->write_global_vertex_pos_pair_to_sim(
            previous_positions, current_positions, global_vertex_offset, vertex_count);
    }

    bool do_write_kinematic_abd_pose_pair_to_sim(IndexT body_id, const Matrix4x4& previous_transform, const Matrix4x4& current_transform, Float dt)
    {
        LogPatternGuard guard{backend_name()};
        return m_engine->write_kinematic_abd_pose_pair_to_sim(
            body_id, previous_transform, current_transform, dt);
    }

    vector<Vector12> do_read_kinematic_abd_state_from_sim(IndexT body_id)
    {
        LogPatternGuard guard{backend_name()};
...
bool Engine::write_global_vertex_pos_pair_to_sim(span<const Vector3> previous_positions, span<const Vector3> current_positions, IndexT global_vertex_offset, SizeT vertex_count)
{
    return m_impl->do_write_global_vertex_pos_pair_to_sim(
        previous_positions, current_positions, global_vertex_offset, vertex_count);
}

bool Engine::write_kinematic_abd_pose_pair_to_sim(IndexT body_id, const Matrix4x4& previous_transform, const Matrix4x4& current_transform, Float dt)
{
    return m_impl->do_write_kinematic_abd_pose_pair_to_sim(
        body_id, previous_transform, current_transform, dt);
}

vector<Vector12> Engine::read_kinematic_abd_state_from_sim(IndexT body_id)
{
    return m_impl->do_read_kinematic_abd_state_from_sim(body_id);
}
