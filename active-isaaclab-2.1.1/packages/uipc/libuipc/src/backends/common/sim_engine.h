...
class SimEngine : public core::IEngine
{
  ...
  private:
    ...
    virtual bool do_write_global_vertex_pos_pair_to_sim(span<const Vector3> previous_positions, span<const Vector3> current_positions, IndexT global_vertex_offset, SizeT vertex_count) final override;
    virtual bool do_write_kinematic_abd_pose_pair_to_sim(IndexT body_id, const Matrix4x4& previous_transform, const Matrix4x4& current_transform, Float dt) final override;
    virtual vector<Vector12> do_read_kinematic_abd_state_from_sim(IndexT body_id) final override;
