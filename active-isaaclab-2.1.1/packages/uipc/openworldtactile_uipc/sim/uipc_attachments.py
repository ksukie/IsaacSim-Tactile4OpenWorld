...
class UipcIsaacAttachments:
    ...
    def __init__(
        ...
        if "attachment_points_pos" in locals():
            self.aim_positions = np.asarray(attachment_points_pos, dtype=np.float32).reshape(-1, 3)
        else:
            self.aim_positions = np.zeros((self.num_attachment_points_per_obj, 3), dtype=np.float32)
    ...
    def _create_animation(self):
        ...
        def animate_tet(info: Animation.UpdateInfo):  # animation function
            ...
            self.animation_last_frame = int(info.frame())
            ...
            expected_positions = len(self.attachment_points_idx)
            if expected_positions == 0:
                return
            if self.aim_positions.shape != (expected_positions, 3):
                try:
                    self._compute_aim_positions()
                except Exception as err:
                    if not getattr(self, "_aim_position_warning_printed", False):
                        print(f"Failed to compute UIPC attachment aim positions: {err}")
                        self._aim_position_warning_printed = True
                    return
            if self.aim_positions.shape != (expected_positions, 3):
                return
    ...
    def _compute_aim_positions(self, dt=0):
        ...
        if isinstance(self.isaaclab_rigid_object, Articulation):
        ...
        elif isinstance(self.isaaclab_rigid_object, RigidObject):
            # IsaacLab 2.x exposes rigid-object state through the data buffer.
            pose = self.isaaclab_rigid_object.data.root_state_w[:, 0:7].clone().view(-1, 1, 7)
        else:
