...
# 中文说明：OpenWorldTactile/GelSight 触觉渲染工具，当前额外包含 OpenWorldTactile RGB 适配器，可把 depth/shear 转成 SDK 需要的触觉 RGB。
...
class OpenWorldTactileTactileRGBRender:
    """Adapter that renders OpenWorldTactile depth-difference maps with the OpenWorldTactile SDK RGB renderer."""

    def __init__(self, cfg: GelSightRenderCfg, device: str | torch.device):
        """Initialize the OpenWorldTactile-style tactile RGB renderer.

        Args:
            cfg: Tactile render configuration. The image size comes from the existing
                GelSight render config so the tactile camera buffer shape stays unchanged.
            device: Torch device where the output tensor should be written.
        """
        self.cfg = cfg
        self.device = device

        openworldtactile_renderer_cls = self._load_openworldtactile_renderer()
        self.renderer = openworldtactile_renderer_cls(
            height=self.cfg.image_height,
            width=self.cfg.image_width,
            max_pressure=getattr(self.cfg, "openworldtactile_max_pressure", 1.0),
            base_hue=getattr(self.cfg, "openworldtactile_base_hue", 0.0),
            max_hue=getattr(self.cfg, "openworldtactile_max_hue", 120.0),
            saturation=getattr(self.cfg, "openworldtactile_saturation", 255),
            base_value=getattr(self.cfg, "openworldtactile_base_value", 210),
            texture_strength=getattr(self.cfg, "openworldtactile_texture_strength", 36),
            texture_blur=getattr(self.cfg, "openworldtactile_texture_blur", 3),
            pressure_blur=getattr(self.cfg, "openworldtactile_pressure_blur", 5),
            displacement_scale=getattr(self.cfg, "openworldtactile_displacement_scale", 1.0),
        )

        logger.info("OpenWorldTactile tactile RGB renderer initialization done!")

    def render(self, height_map: torch.Tensor, displacement: torch.Tensor | None = None) -> torch.Tensor:
        """Render a OpenWorldTactile depth-difference tensor into OpenWorldTactile-style RGB images.

        Args:
            height_map: Input depth/penetration map tensor. Shape is (N, H, W).
            displacement: Optional x/y displacement map tensor. Shape is (N, H, W, 2).

        Returns:
            Rendered RGB tensor. Shape is (N, H, W, 3), dtype is uint8.
        """
        pressure_maps = height_map.detach().cpu().numpy()
        if pressure_maps.ndim == 2:
            pressure_maps = pressure_maps[None, ...]

        displacement_maps = None
        if displacement is not None:
            displacement_maps = displacement.detach().cpu().numpy()
            if displacement_maps.ndim == 3:
                displacement_maps = displacement_maps[None, ...]

        rendered_images = []
        for index, pressure_map in enumerate(pressure_maps):
            displacement_map = None if displacement_maps is None else displacement_maps[index]
            rendered_images.append(self.renderer.render(pressure_map, displacement=displacement_map))
        rendered_images = np.stack(rendered_images, axis=0)
        return torch.as_tensor(rendered_images, dtype=torch.uint8, device=self.device)

    @staticmethod
    def _load_openworldtactile_renderer():
        try:
            from api import OpenWorldTactileRGBRenderer
        ...
        for parent in Path(__file__).resolve().parents:
            ...
            if (sdk_root / "api" / "openworldtactile_rgb_renderer.py").exists():
        ...
        raise ModuleNotFoundError(
            "Could not import OpenWorldTactileRGBRenderer. Add hardware-sdk/openworldtactile to PYTHONPATH "
            "or keep it under the archive root."
        )
