...
parser = argparse.ArgumentParser(
    description="AgileX Piper pick-and-place demo with UIPC deformation-driven GelSight RGB."
)
...
parser.add_argument(
    "--dataset_dir",
    type=str,
    default="dataset/0624data_uipc_deform_rgb",
...
def _set_mesh_display_appearance_for_matching_prims(
    prim_path_expr: str,
    color: tuple[float, float, float],
    opacity: float,
):
    ...
    if mesh_count:
        print(f"[INFO] set black opaque display appearance on {mesh_count} mesh prim(s) under {prim_path_expr}.")
...
class CylinderContainerSceneCfg(DirectRLEnvCfg):
    ...
    uipc_gelpad_height_m = 2.0 / 1000.0
    uipc_gelpad_youngs_modulus = 0.06
    uipc_gelpad_poisson_rate = 0.45
    uipc_gelpad_attachment_strength_ratio = 300.0
    uipc_gelpad_attachment_points_radius = 0.0035
    uipc_gelpad_contact_friction_ratio = 0.2
    uipc_tactile_normal_stiffness = 1000.0
    uipc_tactile_shear_stiffness = 500.0
    uipc_tactile_contact_threshold_mm = 0.1
    gsmini_left.gelpad_dimensions.height = uipc_gelpad_height_m
    gsmini_left.optical_sim_cfg = gsmini_left.optical_sim_cfg.replace(
        with_shadow=False,
        device="cuda",
        tactile_img_res=(32, 32),
        gelpad_height=uipc_gelpad_height_m,
    )
    ...
    uipc_sim = UipcSimCfg(
        ...
        ground_height=-1.0,
        contact=UipcSimCfg.Contact(d_hat=5.0e-4, default_friction_ratio=uipc_gelpad_contact_friction_ratio),
    )
    ...
    uipc_gelpad_cfg = UipcObjectCfg(
        ...
        constitution_cfg=UipcObjectCfg.StableNeoHookeanCfg(
            youngs_modulus=uipc_gelpad_youngs_modulus,
            poisson_rate=uipc_gelpad_poisson_rate,
        ),
    )
    uipc_gelpad_attachment_cfg = UipcIsaacAttachmentsCfg(
        constraint_strength_ratio=uipc_gelpad_attachment_strength_ratio,
        body_name=None,
        compute_attachment_data=True,
        attachment_points_radius=uipc_gelpad_attachment_points_radius,
    )
...
class CylinderContainerSceneEnv(UipcRLEnv):
    ...
    def __init__(self, cfg: CylinderContainerSceneCfg, render_mode: str | None = None, **kwargs):
        ...
        tactile_width, tactile_height = self.cfg.gsmini_left.optical_sim_cfg.tactile_img_res
        self._uipc_gelpad_surface_ref_s = None
        self._uipc_gelpad_front_surface_mask = None
        self._uipc_height_map_mm = torch.zeros((self.num_envs, tactile_height, tactile_width), device=self.device)
        self._uipc_deformation_map_mm = torch.zeros_like(self._uipc_height_map_mm)
    ...
    def _build_pick_place_plan(self) -> list[PickPlacePhase]:
        ...
        gx = cx + self.cfg.grasp_x_offset
        above_pick = (gx, cy, self.cfg.approach_z)
        grasp_pose = (gx, cy, grasp_z)
        ...
        return [
            ...
            PickPlacePhase("CONFIRM_GRASP", grasp_pose, closed_fingers, 45, "grasp"),
            PickPlacePhase("LIFT_OBJECT", (gx, cy, self.cfg.lift_z), closed_fingers, 90),
    ...
    def _uipc_gelpad_surface_points_sensor_frame(self) -> torch.Tensor | None:
        if not self.cfg.enable_tactile or not hasattr(self, "_uipc_gelpad"):
            return None

        surf_points_w = self._uipc_gelpad.data.surf_nodal_pos_w
        if surf_points_w is None or surf_points_w.numel() == 0:
            return None
        if surf_points_w.ndim == 3:
            surf_points_w = surf_points_w[0]

        surf_points_w = surf_points_w.to(device=self.device, dtype=self._runtime_gelsight_pos_w.dtype)
        sensor_pos_w = self._runtime_gelsight_pos_w[0]
        sensor_quat_w = self._runtime_gelsight_quat_w[0]
        rel_points_w = surf_points_w - sensor_pos_w.unsqueeze(0)
        sensor_quat = sensor_quat_w.unsqueeze(0).expand(rel_points_w.shape[0], 4)
        return math_utils.quat_apply_inverse(sensor_quat, rel_points_w)

    def _uipc_gelpad_front_surface_samples(self) -> tuple[torch.Tensor, torch.Tensor] | None:
        points_s = self._uipc_gelpad_surface_points_sensor_frame()
        if points_s is None or points_s.shape[0] == 0:
            return None

        if (
            self._uipc_gelpad_surface_ref_s is None
            or self._uipc_gelpad_surface_ref_s.shape != points_s.shape
        ):
            self._uipc_gelpad_surface_ref_s = points_s.detach().clone()
            ref_x = self._uipc_gelpad_surface_ref_s[:, 0]
            front_shell = max(self.cfg.uipc_gel_height * 0.25, 1.0e-4)
            front_threshold = ref_x.max() - front_shell
            front_mask = ref_x >= front_threshold
            if int(front_mask.sum().item()) < 4:
                top_count = min(max(4, points_s.shape[0] // 4), points_s.shape[0])
                top_ids = torch.topk(ref_x, k=top_count).indices
                front_mask = torch.zeros_like(ref_x, dtype=torch.bool)
                front_mask[top_ids] = True
            self._uipc_gelpad_front_surface_mask = front_mask

        front_mask = self._uipc_gelpad_front_surface_mask
        if front_mask is None or int(front_mask.sum().item()) == 0:
            return None
        return self._uipc_gelpad_surface_ref_s[front_mask], points_s[front_mask]

    def _resample_uipc_deformation_height_map(self, gsmini) -> tuple[torch.Tensor, torch.Tensor] | None:
        samples = self._uipc_gelpad_front_surface_samples()
        if samples is None:
            return None

        ref_s, current_s = samples
        tactile_width, tactile_height = gsmini.cfg.optical_sim_cfg.tactile_img_res
        gel_width = gsmini.cfg.gelpad_dimensions.width
        gel_length = gsmini.cfg.gelpad_dimensions.length

        indentation_negative_x = torch.clamp(ref_s[:, 0] - current_s[:, 0], min=0.0)
        indentation_positive_x = torch.clamp(current_s[:, 0] - ref_s[:, 0], min=0.0)
        if indentation_positive_x.amax() > indentation_negative_x.amax():
            indentation_m = indentation_positive_x
        else:
            indentation_m = indentation_negative_x
        indentation_mm = indentation_m * 1000.0

        u = torch.round((gel_width * 0.5 - ref_s[:, 1]) / gel_width * (tactile_width - 1)).long()
        v = torch.round((gel_length * 0.5 - ref_s[:, 2]) / gel_length * (tactile_height - 1)).long()
        valid = (u >= 0) & (u < tactile_width) & (v >= 0) & (v < tactile_height)

        deformation_flat = torch.zeros(
            tactile_height * tactile_width,
            device=self.device,
            dtype=indentation_mm.dtype,
        )
        if valid.any():
            flat_ids = v[valid] * tactile_width + u[valid]
            values = indentation_mm[valid]
            try:
                deformation_flat.scatter_reduce_(0, flat_ids, values, reduce="amax", include_self=True)
            except (AttributeError, RuntimeError, TypeError):
                for flat_id, value in zip(flat_ids.detach().cpu().tolist(), values.detach().cpu().tolist()):
                    if value > float(deformation_flat[flat_id].item()):
                        deformation_flat[flat_id] = value

        deformation_mm = deformation_flat.view(tactile_height, tactile_width)
        if float(deformation_mm.amax().item()) > 0.0:
            deformation_mm = F.max_pool2d(
                deformation_mm.view(1, 1, tactile_height, tactile_width),
                kernel_size=3,
                stride=1,
                padding=1,
            )[0, 0]
            deformation_mm = F.avg_pool2d(
                deformation_mm.view(1, 1, tactile_height, tactile_width),
                kernel_size=3,
                stride=1,
                padding=1,
        ...
        height_map_mm = -deformation_mm
        return height_map_mm, deformation_mm

    def _update_uipc_tactile_deformation_maps(self):
        if not self.cfg.enable_tactile or not hasattr(self, "gsmini_left"):
            return

        result = self._resample_uipc_deformation_height_map(self.gsmini_left)
        if result is None:
            return

        height_map_mm, deformation_mm = result
        self._uipc_height_map_mm[0] = height_map_mm.detach()
        self._uipc_deformation_map_mm[0] = deformation_mm.detach()

        sensor_height_map = self.gsmini_left._data.output.get("height_map")
        if sensor_height_map is None:
            return

        height_map_to_write = height_map_mm
        if tuple(sensor_height_map.shape[1:3]) != tuple(height_map_mm.shape):
            height_map_to_write = F.interpolate(
                height_map_mm.view(1, 1, *height_map_mm.shape),
                size=tuple(sensor_height_map.shape[1:3]),
                mode="bilinear",
                align_corners=False,
            )[0, 0]
        sensor_height_map[:] = height_map_to_write.unsqueeze(0).to(dtype=sensor_height_map.dtype)

        indentation_depth = deformation_mm.amax().reshape(1)
        if getattr(self.gsmini_left, "_indentation_depth", None) is not None:
            self.gsmini_left._indentation_depth[:] = indentation_depth.to(
                device=self.gsmini_left._indentation_depth.device,
                dtype=self.gsmini_left._indentation_depth.dtype,
            )

    def _resample_uipc_deformation_force_field(self, gsmini) -> torch.Tensor | None:
        samples = self._uipc_gelpad_front_surface_samples()
        if samples is None:
            return None

        ref_s, current_s = samples
        tactile_width, tactile_height = gsmini.cfg.sensor_camera_cfg.resolution
        gel_width = gsmini.cfg.gelpad_dimensions.width
        gel_length = gsmini.cfg.gelpad_dimensions.length

        displacement_s = current_s - ref_s
        normal_delta = displacement_s[:, 0]
        positive_indentation = torch.clamp(normal_delta, min=0.0)
        negative_indentation = torch.clamp(-normal_delta, min=0.0)
        if positive_indentation.amax() > negative_indentation.amax():
            indentation_m = positive_indentation
            normal_sign = 1.0
        else:
            indentation_m = negative_indentation
            normal_sign = -1.0
        contact_threshold_m = self.cfg.uipc_tactile_contact_threshold_mm / 1000.0
        indentation_m = torch.clamp(indentation_m - contact_threshold_m, min=0.0)

        normal_force = self.cfg.uipc_tactile_normal_stiffness * indentation_m
        force_s = torch.zeros_like(displacement_s)
        force_s[:, 0] = normal_sign * normal_force
        force_s[:, 1] = self.cfg.uipc_tactile_shear_stiffness * displacement_s[:, 1]
        force_s[:, 2] = self.cfg.uipc_tactile_shear_stiffness * displacement_s[:, 2]

        force_values = torch.stack(
            (
                -force_s[:, 1],
                -force_s[:, 2],
                normal_force,
        ...
        force_flat = torch.zeros(
            ...
            3,
            device=self.device,
            dtype=force_values.dtype,
        )
        if not valid.any():
            return force_flat.view(tactile_height, tactile_width, 3)

        flat_ids = v[valid] * tactile_width + u[valid]
        values = force_values[valid]
        normal_values = normal_force[valid]

        weighted_lateral = values[:, :2] * normal_values.unsqueeze(-1)
        lateral_sum = torch.zeros(
            tactile_height * tactile_width,
            2,
            device=self.device,
            dtype=force_values.dtype,
        )
        lateral_weight = torch.zeros(
            tactile_height * tactile_width,
            1,
            device=self.device,
            dtype=force_values.dtype,
        )
        normal_flat = torch.zeros(
            tactile_height * tactile_width,
            device=self.device,
            dtype=force_values.dtype,
        )

        lateral_sum.scatter_add_(0, flat_ids[:, None].expand(-1, 2), weighted_lateral)
        lateral_weight.scatter_add_(0, flat_ids[:, None], normal_values.unsqueeze(-1))
        force_flat[:, :2] = lateral_sum / lateral_weight.clamp_min(1.0e-9)

        try:
            normal_flat.scatter_reduce_(0, flat_ids, normal_values, reduce="amax", include_self=True)
        except (AttributeError, RuntimeError, TypeError):
            for flat_id, value in zip(flat_ids.detach().cpu().tolist(), normal_values.detach().cpu().tolist()):
                if value > float(normal_flat[flat_id].item()):
                    normal_flat[flat_id] = value
        force_flat[:, 2] = normal_flat

        force_field = force_flat.view(tactile_height, tactile_width, 3)
        if float(force_field[..., 2].amax().item()) > 0.0:
            lateral = F.avg_pool2d(
                force_field[..., :2].permute(2, 0, 1).unsqueeze(0),
                kernel_size=3,
                stride=1,
                padding=1,
            )[0].permute(1, 2, 0)
            normal = F.max_pool2d(
                force_field[..., 2].view(1, 1, tactile_height, tactile_width),
                kernel_size=3,
                stride=1,
                padding=1,
            )
            normal = F.avg_pool2d(normal, kernel_size=3, stride=1, padding=1)[0, 0]
            force_field = torch.cat((lateral, normal.unsqueeze(-1)), dim=-1)

        return force_field

    def _compute_uipc_deformation_tactile_force_field(self):
        if not self.cfg.enable_tactile or not hasattr(self, "gsmini_left"):
            return
        ...
        for gsmini in [self.gsmini_left]:
            ...
            force_field = self._resample_uipc_deformation_force_field(gsmini)
            if force_field is None:
                continue

            sensor_force_field = gsmini._data.output["tactile_force_field"]
            force_field_to_write = force_field
            if tuple(sensor_force_field.shape[1:3]) != tuple(force_field.shape[:2]):
                force_field_to_write = F.interpolate(
                    force_field.permute(2, 0, 1).unsqueeze(0),
                    size=tuple(sensor_force_field.shape[1:3]),
                    mode="bilinear",
                    align_corners=False,
                )[0].permute(1, 2, 0)
            sensor_force_field[:] = force_field_to_write.unsqueeze(0).to(dtype=sensor_force_field.dtype)
    ...
    def _setup_scene(self):
        ...
        if self.cfg.enable_tactile:
            ...
            _set_mesh_display_appearance_for_matching_prims(
                self.cfg.uipc_gelpad_cfg.prim_path,
                color=(0.0, 0.0, 0.0),
                opacity=1.0,
            )
            _set_visibility_for_matching_prims(self.cfg.uipc_gelpad_cfg.prim_path, True)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        if hasattr(self, "_uipc_gelpad_surface_ref_s"):
            self._uipc_gelpad_surface_ref_s = None
            self._uipc_gelpad_front_surface_mask = None
            self._uipc_height_map_mm[env_ids] = 0.0
            self._uipc_deformation_map_mm[env_ids] = 0.0
...
class HDF5EpisodeRecorder:
    ...
    def _clear_buffers(self):
        ...
        self.tactile1_height_map_float32: list[np.ndarray] = []
        self.tactile1_deformation_float32: list[np.ndarray] = []
    ...
    def append_from_env(self, env: CylinderContainerSceneEnv):
        ...
        height_map = env._uipc_height_map_mm[0].detach().cpu().numpy().astype(np.float32, copy=True)
        deformation_map = env._uipc_deformation_map_mm[0].detach().cpu().numpy().astype(np.float32, copy=True)
        self.tactile1_fxyz_float32.append(force_field)
        self.tactile1_height_map_float32.append(height_map)
        self.tactile1_deformation_float32.append(deformation_map)
    ...
    def close_episode(self):
        ...
        tactile1_height_map_float32 = np.stack(self.tactile1_height_map_float32, axis=0).astype(np.float32)
        tactile1_deformation_float32 = np.stack(self.tactile1_deformation_float32, axis=0).astype(np.float32)
        ...
        compress_len = np.stack((top_len, left_wrist_len), axis=0).astype(np.float32)
        ...
        with h5py.File(episode_path, "w") as h5:
            ...
            tactile_fxyz_dataset.attrs["channels"] = "fx_image, fy_image, fz_normal_magnitude"
            tactile_fxyz_dataset.attrs["source"] = "uipc_gelpad_surface_deformation_force_field"
            tactile_fxyz_dataset.attrs["method"] = (
                "resampled from UIPC gelpad front-surface displacement; "
                "normal force from indentation, shear force from tangential displacement"
            )
            tactile_height_map_dataset = observations.create_dataset(
                "tactile1_height_map_float32", data=tactile1_height_map_float32, dtype="float32"
            )
            tactile_height_map_dataset.attrs["source"] = "uipc_gelpad_surface_deformation_height_map"
            tactile_height_map_dataset.attrs["units"] = "mm"
            tactile_height_map_dataset.attrs["sign"] = "negative values are indentation"
            tactile_deformation_dataset = observations.create_dataset(
                "tactile1_deformation_float32", data=tactile1_deformation_float32, dtype="float32"
            )
            tactile_deformation_dataset.attrs["source"] = "uipc_gelpad_surface_deformation"
            tactile_deformation_dataset.attrs["units"] = "mm"
            tactile_deformation_dataset.attrs["sign"] = "positive values are indentation magnitude"
...
def run_simulator(
    ...
    while simulation_app.is_running():
        ...
        if env.cfg.enable_tactile:
            ...
            try:
                env._update_uipc_tactile_deformation_maps()
            except Exception as err:
                if step < 5:
                    carb.log_warn(f"_update_uipc_tactile_deformation_maps failed: {err}")
            try:
                env._compute_uipc_deformation_tactile_force_field()
            except Exception as err:
                if step < 5:
                    carb.log_warn(f"_compute_uipc_deformation_tactile_force_field failed: {err}")
            try:
