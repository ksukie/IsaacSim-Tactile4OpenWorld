...
try:
    ...
    runtime_env = os.environ.get("CONDA_DEFAULT_ENV", "")
    raise ModuleNotFoundError(
        "isaaclab is not importable. Run this script from the read-only conda environment "
        f"'{EXPECTED_CONDA_ENV}', for example: "
        f"conda run -n {EXPECTED_CONDA_ENV} python {Path(__file__).as_posix()} "
        "--output_dir /tmp/openworldtactile_uipc_v5_new_4_4_native_gradient_source_geometry_audit "
        f"(current CONDA_DEFAULT_ENV={runtime_env!r})."
    ) from exc
...
SOURCE_REGION_LABELS = {
    0: "unknown",
    1: "membrane_front_face_uv_inside",
    2: "membrane_front_face_uv_outside",
    3: "membrane_internal_or_back",
    4: "tool",
    5: "membrane_outside_front_plane",
}


parser = argparse.ArgumentParser(
    description=(
        "V5 new stage 4_4: native UIPC contact-gradient source geometry audit. "
        "The 3h contact_geometry penetration proxy remains the compatibility tactile output; this script "
        "audits where nonzero native contact_gradient vertices come from before any force scale, threshold, "
        "or native patch replacement work."
    )
)
parser.add_argument(
    "--output_dir",
    type=str,
    default="/tmp/openworldtactile_uipc_v5_new_4_4_native_gradient_source_geometry_audit",
)
parser.add_argument("--workspace_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_4_4_workspace")
...
parser.add_argument(
    "--native_front_face_near_tolerance_mm",
    type=float,
    default=0.25,
    help="Distance tolerance for classifying a native gradient source as lying on the sim-mesh front face.",
)
...
def _validate_args() -> None:
    ...
    if float(args_cli.native_front_face_near_tolerance_mm) < 0.0:
        parser.error("--native_front_face_near_tolerance_mm must be >= 0.")
...
def _phase_signed_stats(values: np.ndarray, phases: list[str], phase_names: tuple[str, ...]) -> dict[str, object]:
    array = np.asarray(values, dtype=np.float64).reshape(-1)
    mask = np.asarray([str(phase) in phase_names for phase in phases], dtype=bool)
    if array.shape[0] != mask.shape[0]:
        return {
...
def _phase_grid_overlap_stats(
    *,
    ...
    native = np.asarray(native_mask, dtype=bool)
    if proxy.shape != native.shape or proxy.ndim < 3:
        return {"frames": 0, "union_nonzero_frames": 0, "mean_iou": 0.0, "max_iou": 0.0}
    phase_mask = np.asarray([str(phase) in phase_names for phase in phases], dtype=bool)
    if phase_mask.shape[0] != proxy.shape[0]:
        return {"frames": 0, "union_nonzero_frames": 0, "mean_iou": 0.0, "max_iou": 0.0}
    ...
    for frame_idx in indices:
        p = proxy[frame_idx]
        n = native[frame_idx]
        union = int(np.count_nonzero(p | n))
        if union == 0:
            ious.append(0.0)
            continue
        intersection = int(np.count_nonzero(p & n))
        union_nonzero += 1
        ious.append(float(intersection) / float(union))
    return {
        "frames": int(indices.size),
        "union_nonzero_frames": int(union_nonzero),
        "mean_iou": float(np.mean(ious)) if ious else 0.0,
        "max_iou": float(np.max(ious)) if ious else 0.0,
    }


def _phase_groups_for_native_calibration() -> dict[str, tuple[str, ...]]:
    return {
        "pre_close": tuple(NON_CONTACT_BASELINE_PHASES),
...
def _linear_fit_proxy_from_candidate(proxy: np.ndarray, candidate: np.ndarray) -> dict[str, object]:
    y = np.asarray(proxy, dtype=np.float64).reshape(-1)
    x = np.asarray(candidate, dtype=np.float64).reshape(-1)
    length = int(min(y.shape[0], x.shape[0]))
    y = y[:length]
    x = x[:length]
    mask = np.isfinite(y) & np.isfinite(x)
    if int(np.count_nonzero(mask)) < 2:
        return {
            "valid_frames": int(np.count_nonzero(mask)),
            "pearson": None,
            "slope": None,
            "intercept": None,
            "r2": None,
            ...
            "rmse_through_origin": None,
        }
    y = y[mask]
    x = x[mask]
    pearson = _finite_pearson(y, x)
    if float(np.std(x)) <= EPS:
        return {
            "valid_frames": int(y.shape[0]),
            "pearson": pearson,
            "slope": None,
            "intercept": None,
            "r2": None,
    ...
    slope_origin = float(np.sum(x * y) / denom) if denom > EPS else None
    pred_origin = slope_origin * x if slope_origin is not None else np.zeros_like(y)
    return {
        "valid_frames": int(y.shape[0]),
        "pearson": pearson,
        "slope": float(slope),
        ...
        "rmse_through_origin": float(np.sqrt(np.mean((y - pred_origin) ** 2))) if slope_origin is not None else None,
    }


def _candidate_series_stats(
    *,
    ...
    series = np.asarray(candidate, dtype=np.float64).reshape(-1)
    available = np.asarray(native_available, dtype=bool).reshape(-1)
    contact = np.asarray(contact_candidate_count, dtype=np.int64).reshape(-1)
    length = int(min(proxy.shape[0], series.shape[0], available.shape[0], contact.shape[0], len(phases)))
    proxy = proxy[:length]
    series = series[:length]
    available = available[:length]
    contact = contact[:length]
    phase_mask = np.asarray([str(phase) in phase_names for phase in phases[:length]], dtype=bool)
    proxy = proxy[phase_mask]
    series = series[phase_mask]
    available = available[phase_mask]
    contact = contact[phase_mask]
    proxy_nonzero = np.abs(proxy) > EPS
    series_nonzero = np.abs(series) > EPS
    return {
        "frames": int(proxy.shape[0]),
        "available_frames": int(np.count_nonzero(available)),
        "contact_candidate_nonzero_frames": int(np.count_nonzero(contact > 0)),
        "proxy_nonzero_frames": int(np.count_nonzero(proxy_nonzero)),
        "candidate_nonzero_frames": int(np.count_nonzero(series_nonzero)),
        "overlap_proxy_candidate_nonzero_frames": int(np.count_nonzero(proxy_nonzero & series_nonzero)),
        "candidate_min": float(np.min(series)) if series.size else 0.0,
        "candidate_max": float(np.max(series)) if series.size else 0.0,
        "candidate_mean": float(np.mean(series)) if series.size else 0.0,
        "candidate_mean_abs": float(np.mean(np.abs(series))) if series.size else 0.0,
        "pearson_proxy_candidate": _finite_pearson(proxy, series),
        "pearson_proxy_abs_candidate": _finite_pearson(proxy, np.abs(series)),
        "proxy_from_candidate_fit": _linear_fit_proxy_from_candidate(proxy, series),
    }


def _nonzero_divide(numerator: np.ndarray, denominator: np.ndarray) -> np.ndarray:
    n = np.asarray(numerator, dtype=np.float64).reshape(-1)
    d = np.asarray(denominator, dtype=np.float64).reshape(-1)
    length = int(min(n.shape[0], d.shape[0]))
    n = n[:length]
    d = d[:length]
    return np.divide(n, d, out=np.zeros_like(n), where=np.abs(d) > EPS)


def _build_native_axis_sign_candidates(
    *,
    proxy_sum_fz: np.ndarray,
    native_local_xyz: np.ndarray,
    native_available: np.ndarray,
    gradient_count: np.ndarray,
    matched_front_count: np.ndarray,
    contact_candidate_count: np.ndarray,
    phases: list[str],
    normal_sign: int,
    front_vertex_count: int,
) -> dict[str, object]:
    proxy = np.asarray(proxy_sum_fz, dtype=np.float64).reshape(-1)
    local = np.asarray(native_local_xyz, dtype=np.float64)
    available = np.asarray(native_available, dtype=bool).reshape(-1)
    grad = np.asarray(gradient_count, dtype=np.float64).reshape(-1)
    matched = np.asarray(matched_front_count, dtype=np.float64).reshape(-1)
    contact = np.asarray(contact_candidate_count, dtype=np.int64).reshape(-1)
    if local.ndim != 3 or local.shape[-1] < 3:
        return {"status": "missing_native_local_xyz", "candidates": {}, "best_candidates": {}}
    length = int(min(proxy.shape[0], local.shape[0], available.shape[0], grad.shape[0], matched.shape[0], contact.shape[0], len(phases)))
    proxy = proxy[:length]
    local = local[:length, :, :3]
    available = available[:length]
    grad = grad[:length]
    matched = matched[:length]
    contact = contact[:length]
    phase_list = [str(phase) for phase in phases[:length]]
    phase_groups = _phase_groups_for_native_calibration()
    axis_names = ("pad_local_x", "pad_local_y", "pad_local_z")
    candidates: dict[str, object] = {}
    for axis_idx, axis_name in enumerate(axis_names):
        component = local[..., axis_idx]
        for sign in (-1.0, 1.0):
            signed = sign * component
            signed_sum = np.sum(signed, axis=1)
            compressive_sum = np.sum(np.clip(signed, 0.0, None), axis=1)
            abs_sum = np.sum(np.abs(signed), axis=1)
            key = f"{axis_name}_sign_{int(sign):+d}"
            weighted_series = {
                "signed_sum": signed_sum,
                "compressive_sum": compressive_sum,
                "abs_component_sum": abs_sum,
                "compressive_per_matched_front": _nonzero_divide(compressive_sum, matched),
                "compressive_per_gradient_count": _nonzero_divide(compressive_sum, grad),
                "compressive_per_front_vertex": compressive_sum / float(max(int(front_vertex_count), 1)),
            }
            candidates[key] = {
                "axis": axis_name,
                "axis_index": int(axis_idx),
                "sign": int(sign),
                "definition": f"{int(sign):+d} * {axis_name}",
                "is_current_4_1_fz_definition": bool(axis_idx == 0 and int(sign) == int(-normal_sign)),
                "summary": {
                    name: {
                        "max": float(np.max(values)) if np.asarray(values).size else 0.0,
                        "max_abs": float(np.max(np.abs(values))) if np.asarray(values).size else 0.0,
                        "nonzero_frames": int(np.count_nonzero(np.abs(values) > EPS)),
                    }
                    for name, values in weighted_series.items()
                },
                "phase_correlation": {
                    group_name: {
                        series_name: _candidate_series_stats(
                            proxy_sum_fz=proxy,
                            candidate=series_values,
                            phases=phase_list,
                            phase_names=group_phases,
                            native_available=available,
                            contact_candidate_count=contact,
                        )
                        for series_name, series_values in weighted_series.items()
                    }
                    for group_name, group_phases in phase_groups.items()
                },
            }

    def score(candidate: dict[str, object], series_name: str, phase_name: str, *, absolute: bool = False) -> float:
        value = (
            candidate.get("phase_correlation", {})
            .get(phase_name, {})
            .get(series_name, {})
            .get("pearson_proxy_candidate")
        )
        if value is None:
            return float("-inf")
        return abs(float(value)) if absolute else float(value)

    ranked_compressive = sorted(
        candidates.items(),
        key=lambda item: score(item[1], "compressive_sum", "all_contact"),
        reverse=True,
    )
    ranked_abs_component = sorted(
        candidates.items(),
        key=lambda item: score(item[1], "abs_component_sum", "all_contact"),
        reverse=True,
    )
    ranked_signed_abs = sorted(
        candidates.items(),
        key=lambda item: score(item[1], "signed_sum", "all_contact", absolute=True),
        reverse=True,
    )
    return {
        "status": "ok",
        "question": "Which pad-local axis/sign best matches the proxy Fz timing before any native replacement?",
        "current_4_1_fz_candidate": f"pad_local_x_sign_{int(-normal_sign):+d}",
        "normal_sign": int(normal_sign),
        "front_vertex_count": int(front_vertex_count),
        "phase_groups": {name: list(values) for name, values in phase_groups.items()},
        "best_candidates": {
            "best_compressive_sum_by_all_contact_pearson": ranked_compressive[0][0] if ranked_compressive else "",
            "best_abs_component_sum_by_all_contact_pearson": ranked_abs_component[0][0] if ranked_abs_component else "",
            "best_signed_sum_by_all_contact_abs_pearson": ranked_signed_abs[0][0] if ranked_signed_abs else "",
        },
        "ranked_compressive_sum_all_contact": [
            {
                "candidate": key,
                "pearson": (
                    value.get("phase_correlation", {})
                    .get("all_contact", {})
                    .get("compressive_sum", {})
                    .get("pearson_proxy_candidate")
                ),
                "overlap_frames": (
                    ...
                    .get("all_contact", {})
                    .get("compressive_sum", {})
                    .get("overlap_proxy_candidate_nonzero_frames")
                ),
            }
            for key, value in ranked_compressive
        ],
        "candidates": candidates,
    }


def _build_native_proxy_phase_correlation(
    *,
    proxy_sum_fz: np.ndarray,
    native_sum_fx: np.ndarray,
    native_sum_fy: np.ndarray,
    native_sum_fz: np.ndarray,
    native_fz: np.ndarray,
    native_sum_fz_compressive: np.ndarray,
    native_sum_abs: np.ndarray,
    native_sum_uipc_force: np.ndarray,
    native_available: np.ndarray,
    matched_front_count: np.ndarray,
    gradient_count: np.ndarray,
    contact_candidate_count: np.ndarray,
    ...
    fz = np.asarray(native_fz, dtype=np.float64)
    fz_abs_sum = np.sum(np.abs(fz), axis=1) if fz.ndim == 2 else np.zeros((0,), dtype=np.float64)
    neg_fz = -np.asarray(native_sum_fz, dtype=np.float64).reshape(-1)
    neg_fz_compressive = np.clip(neg_fz, 0.0, None)
    matched = np.asarray(matched_front_count, dtype=np.float64).reshape(-1)
    grad = np.asarray(gradient_count, dtype=np.float64).reshape(-1)
    series_map = {
        "sum_native_fx": np.asarray(native_sum_fx, dtype=np.float64).reshape(-1),
        "sum_native_fy": np.asarray(native_sum_fy, dtype=np.float64).reshape(-1),
        "sum_native_fz_signed": np.asarray(native_sum_fz, dtype=np.float64).reshape(-1),
        "sum_native_fz_signed_flipped": neg_fz,
        "sum_native_fz_compressive": np.asarray(native_sum_fz_compressive, dtype=np.float64).reshape(-1),
        "sum_native_fz_flipped_compressive": neg_fz_compressive,
        "sum_native_fz_abs_component": fz_abs_sum,
        "sum_native_abs": np.asarray(native_sum_abs, dtype=np.float64).reshape(-1),
        "sum_native_uipc_force_4_1_compressive": np.asarray(native_sum_uipc_force, dtype=np.float64).reshape(-1),
    }
    series_map.update(
        {
            "mean_native_fz_compressive_per_matched_front": _nonzero_divide(
                series_map["sum_native_fz_compressive"], matched
            ),
            "mean_native_fz_compressive_per_gradient": _nonzero_divide(
                series_map["sum_native_fz_compressive"], grad
            ),
            "mean_native_abs_per_matched_front": _nonzero_divide(series_map["sum_native_abs"], matched),
            "mean_native_abs_per_gradient": _nonzero_divide(series_map["sum_native_abs"], grad),
        }
    )
    phase_groups = _phase_groups_for_native_calibration()
    return {
        "status": "ok",
        "definition": "Proxy/native phase-wise timing and scale correlation; diagnostic only.",
        "series": list(series_map.keys()),
        "phase_groups": {name: list(values) for name, values in phase_groups.items()},
        "phase_correlation": {
            group_name: {
                series_name: _candidate_series_stats(
                    proxy_sum_fz=proxy_sum_fz,
                    candidate=series_values,
                    phases=phases,
                    phase_names=group_phases,
...
def _weighted_grid_center(values: np.ndarray) -> tuple[np.ndarray | None, int]:
    v = np.asarray(values, dtype=np.float64)
    if v.ndim != 2:
        return None, 0
    weights = np.clip(v, 0.0, None)
    mask = weights > EPS
    count = int(np.count_nonzero(mask))
    total = float(np.sum(weights))
    if total <= EPS:
        return None, count
    rows, cols = np.indices(v.shape, dtype=np.float64)
    return np.asarray([float(np.sum(rows * weights) / total), float(np.sum(cols * weights) / total)]), count


def _grid_alignment_for_phase(
    *,
    ...
    if proxy.shape != native.shape or proxy.ndim != 3:
        return {
            "frames": 0,
            "both_nonzero_frames": 0,
            "union_nonzero_frames": 0,
            "mean_iou": 0.0,
            "max_iou": 0.0,
            "mean_centroid_distance_cells": None,
            "max_centroid_distance_cells": None,
            "mean_centroid_distance_normalized": None,
        }
    phase_mask = np.asarray([str(phase) in phase_names for phase in phases[: proxy.shape[0]]], dtype=bool)
    if phase_mask.shape[0] != proxy.shape[0]:
        return {
            "frames": 0,
            "both_nonzero_frames": 0,
    ...
    union_nonzero = 0
    both_nonzero = 0
    diag = float(np.sqrt(proxy.shape[1] ** 2 + proxy.shape[2] ** 2))
    for frame_idx in indices:
        p = np.clip(proxy[frame_idx], 0.0, None)
        n = np.clip(native[frame_idx], 0.0, None)
        p_mask = p > EPS
        n_mask = n > EPS
        union = int(np.count_nonzero(p_mask | n_mask))
        if union > 0:
            union_nonzero += 1
            ious.append(float(np.count_nonzero(p_mask & n_mask)) / float(union))
        else:
            ious.append(0.0)
        p_center, p_count = _weighted_grid_center(p)
        n_center, n_count = _weighted_grid_center(n)
        if p_center is not None and n_center is not None:
            both_nonzero += 1
            distances.append(float(np.linalg.norm(p_center - n_center)))
        elif p_count > 0 and n_count > 0:
            both_nonzero += 1
    return {
        "frames": int(indices.size),
        "both_nonzero_frames": int(both_nonzero),
        "union_nonzero_frames": int(union_nonzero),
        "mean_iou": float(np.mean(ious)) if ious else 0.0,
        "max_iou": float(np.max(ious)) if ious else 0.0,
        "mean_centroid_distance_cells": float(np.mean(distances)) if distances else None,
        "max_centroid_distance_cells": float(np.max(distances)) if distances else None,
        "mean_centroid_distance_normalized": float(np.mean(distances) / diag) if distances and diag > EPS else None,
    }


def _build_native_proxy_grid_alignment(
    *,
    proxy_fz_grid: np.ndarray,
    native_local_xyz_grid: np.ndarray,
    normal_sign: int,
    phases: list[str],
) -> dict[str, object]:
    proxy = np.asarray(proxy_fz_grid, dtype=np.float64)
    local = np.asarray(native_local_xyz_grid, dtype=np.float64)
    if local.ndim != 4 or local.shape[-1] < 3:
        return {"status": "missing_native_local_xyz_grid", "candidates": {}}
    length = int(min(proxy.shape[0], local.shape[0], len(phases)))
    proxy = proxy[:length]
    local = local[:length, :, :, :3]
    phase_list = [str(phase) for phase in phases[:length]]
    phase_groups = _phase_groups_for_native_calibration()
    current_fz = -float(normal_sign) * local[..., 0]
    grid_candidates = {
        "native_abs_norm": np.linalg.norm(local, axis=-1),
        "current_fz_abs": np.abs(current_fz),
        "current_fz_compressive": np.clip(current_fz, 0.0, None),
        "current_fz_flipped_compressive": np.clip(-current_fz, 0.0, None),
        "pad_local_x_positive": np.clip(local[..., 0], 0.0, None),
        "pad_local_x_negative": np.clip(-local[..., 0], 0.0, None),
        "pad_local_y_positive": np.clip(local[..., 1], 0.0, None),
        "pad_local_y_negative": np.clip(-local[..., 1], 0.0, None),
        "pad_local_z_positive": np.clip(local[..., 2], 0.0, None),
        "pad_local_z_negative": np.clip(-local[..., 2], 0.0, None),
    }
    candidates = {
        name: {
            group_name: _grid_alignment_for_phase(
                proxy_values=proxy,
        ...
        for name, values in grid_candidates.items()
    }

    def iou_score(item: tuple[str, dict[str, object]]) -> float:
        value = item[1].get("all_contact", {}).get("mean_iou")
        return float(value) if value is not None else float("-inf")

    ranked = sorted(candidates.items(), key=iou_score, reverse=True)
    return {
        "status": "ok",
        "definition": "Grid alignment between proxy local_fz_grid and native projection candidates.",
        "proxy_grid": "local_fz_grid.npy, baseline-corrected contact_geometry proxy",
        "phase_groups": {name: list(values) for name, values in phase_groups.items()},
        "best_by_all_contact_mean_iou": ranked[0][0] if ranked else "",
        "ranked_by_all_contact_mean_iou": [
            {
                "candidate": key,
                "mean_iou": value.get("all_contact", {}).get("mean_iou"),
                "mean_centroid_distance_cells": value.get("all_contact", {}).get("mean_centroid_distance_cells"),
                "both_nonzero_frames": value.get("all_contact", {}).get("both_nonzero_frames"),
            }
...
def _build_native_sign_correlation_diagnostics(
    *,
    native_axis_sign_candidates: dict[str, object],
    native_proxy_phase_correlation: dict[str, object],
    native_proxy_grid_alignment: dict[str, object],
    normal_sign: int,
) -> dict[str, object]:
    current_key = f"pad_local_x_sign_{int(-normal_sign):+d}"
    candidates = native_axis_sign_candidates.get("candidates", {})
    current = candidates.get(current_key, {}) if isinstance(candidates, dict) else {}
    phase_corr = native_proxy_phase_correlation.get("phase_correlation", {})
    all_contact = phase_corr.get("all_contact", {}) if isinstance(phase_corr, dict) else {}
    return {
        "validation_name": "4_2 = native projection sign/correlation diagnostics completed",
        "native_uipc_contact_force_used": False,
        "native_uipc_contact_force_probed": True,
        "native_uipc_contact_force_replaces_proxy": False,
        "scope": (
            "Diagnostic-only analysis of native contact_gradient projection sign, scale, cancellation, "
            "and grid alignment. This does not promote native force to the main tactile output."
        ),
        "current_4_1_fz_candidate": current_key,
        "current_4_1_fz_candidate_summary": {
            "compressive_all_contact": (
                current.get("phase_correlation", {})
                .get("all_contact", {})
                .get("compressive_sum", {})
        ...
        "best_axis_sign_candidates": native_axis_sign_candidates.get("best_candidates", {}),
        "best_grid_alignment_candidate": native_proxy_grid_alignment.get("best_by_all_contact_mean_iou", ""),
        "phase_correlation_focus": {
            "sum_native_fz_signed": all_contact.get("sum_native_fz_signed", {}),
            "sum_native_fz_signed_flipped": all_contact.get("sum_native_fz_signed_flipped", {}),
            "sum_native_fz_compressive": all_contact.get("sum_native_fz_compressive", {}),
            "sum_native_fz_flipped_compressive": all_contact.get("sum_native_fz_flipped_compressive", {}),
            "sum_native_fz_abs_component": all_contact.get("sum_native_fz_abs_component", {}),
            "sum_native_abs": all_contact.get("sum_native_abs", {}),
        },
        "interpretation_rules": [
            "Prefer phase overlap and grid alignment before treating Pearson correlation as force calibration.",
            "Signed native sums can cancel across nodes; compare compressive and abs candidates as diagnostics.",
            "High timing overlap alone is not native force validation.",
            "Keep local_fxyz.npy as proxy output unless sign, scale, area weighting, and spatial mapping are calibrated.",
        ],
    }
...
def _uipc_object_vertex_positions_w(uipc_sim: UipcSim, uipc_obj: UipcObject) -> np.ndarray:
    geom = uipc_sim.scene.geometries()
    geo_slot, _geo_slot_rest = geom.find(uipc_obj.obj_id)
    return np.asarray(geo_slot.geometry().positions().view()).copy().reshape(-1, 3).astype(np.float32)


def _value_stats(array: np.ndarray) -> dict[str, object]:
...
def _count_labels(values: np.ndarray, labels: dict[int, str]) -> dict[str, int]:
    array = np.asarray(values, dtype=np.int64).reshape(-1)
    return {label: int(np.count_nonzero(array == int(value))) for value, label in labels.items()}


def _empty_native_gradient_source_audit() -> dict[str, object]:
    return {
        "count": 0,
        "global_indices": np.zeros((0,), dtype=np.int64),
        "geometry_id": np.zeros((0,), dtype=np.int16),
        "source_region_id": np.zeros((0,), dtype=np.int16),
        "local_vertex_indices": np.full((0,), -1, dtype=np.int64),
        "source_points_l": np.zeros((0, 3), dtype=np.float32),
        "source_points_w": np.zeros((0, 3), dtype=np.float32),
        "gradient_w": np.zeros((0, 3), dtype=np.float32),
        "force_l": np.zeros((0, 3), dtype=np.float32),
        "gradient_norm": np.zeros((0,), dtype=np.float32),
        "nearest_front_vertex_distance_mm": np.zeros((0,), dtype=np.float32),
        "nearest_front_face_distance_mm": np.zeros((0,), dtype=np.float32),
        "local_x_to_front_face_mm": np.zeros((0,), dtype=np.float32),
        "uv_on_front_face": np.zeros((0, 2), dtype=np.float32),
        "inside_front_uv_mask": np.zeros((0,), dtype=bool),
        "summary": {
            "count": 0,
            "geometry_counts": _count_labels(np.zeros((0,), dtype=np.int16), SOURCE_GEOMETRY_LABELS),
            "source_region_counts": _count_labels(np.zeros((0,), dtype=np.int16), SOURCE_REGION_LABELS),
        },
    }


def _native_gradient_source_geometry_frame(
    *,
    ...
    current_front_l: np.ndarray,
    normal_sign: int,
    membrane_global_vertex_offset: int,
    membrane_vertex_positions_w: np.ndarray,
    tool_global_vertex_offset: int,
    tool_vertex_positions_w: np.ndarray,
    front_face_near_tolerance_mm: float,
) -> dict[str, object]:
    indices = np.asarray(global_indices, dtype=np.int64).reshape(-1)
    grad = np.asarray(gradient_w, dtype=np.float64)
    if grad.ndim != 2 or grad.shape[1] < 3 or indices.size == 0:
        return _empty_native_gradient_source_audit()
    grad = grad[:, :3]
    length = int(min(indices.shape[0], grad.shape[0]))
    indices = indices[:length]
    grad = grad[:length]
    grad_norm = np.linalg.norm(grad, axis=1)
    nonzero = np.isfinite(grad_norm) & (grad_norm > EPS)
    if int(np.count_nonzero(nonzero)) == 0:
        return _empty_native_gradient_source_audit()

    indices = indices[nonzero]
    grad = grad[nonzero]
    grad_norm = grad_norm[nonzero]
    count = int(indices.shape[0])

    source_points_w = np.full((count, 3), np.nan, dtype=np.float64)
    source_points_l = np.full((count, 3), np.nan, dtype=np.float64)
    geometry_id = np.zeros((count,), dtype=np.int16)
    source_region_id = np.zeros((count,), dtype=np.int16)
    local_vertex_indices = np.full((count,), -1, dtype=np.int64)

    membrane_vertices = np.asarray(membrane_vertex_positions_w, dtype=np.float64).reshape(-1, 3)
    tool_vertices = np.asarray(tool_vertex_positions_w, dtype=np.float64).reshape(-1, 3)

    membrane_start = int(membrane_global_vertex_offset)
    membrane_end = membrane_start + int(membrane_vertices.shape[0])
    if membrane_start >= 0:
        membrane_mask = (indices >= membrane_start) & (indices < membrane_end)
        if np.any(membrane_mask):
            local_idx = indices[membrane_mask] - membrane_start
            local_vertex_indices[membrane_mask] = local_idx
            source_points_w[membrane_mask] = membrane_vertices[local_idx]
            geometry_id[membrane_mask] = 1

    tool_start = int(tool_global_vertex_offset)
    tool_end = tool_start + int(tool_vertices.shape[0])
    if tool_start >= 0:
        tool_mask = (indices >= tool_start) & (indices < tool_end)
        if np.any(tool_mask):
            local_idx = indices[tool_mask] - tool_start
            local_vertex_indices[tool_mask] = local_idx
            source_points_w[tool_mask] = tool_vertices[local_idx]
            geometry_id[tool_mask] = 2
            source_region_id[tool_mask] = 4

    known = np.isfinite(source_points_w).all(axis=1)
    if np.any(known):
        source_points_l[known] = _local_from_world(source_points_w[known], pad_pos_w, pad_quat_wxyz).astype(np.float64)

    nearest_front_vertex_distance_mm = np.full((count,), np.nan, dtype=np.float64)
    nearest_front_face_distance_mm = np.full((count,), np.nan, dtype=np.float64)
    local_x_to_front_face_mm = np.full((count,), np.nan, dtype=np.float64)
    uv_on_front_face = np.full((count, 2), np.nan, dtype=np.float64)
    inside_front_uv_mask = np.zeros((count,), dtype=bool)
    front = np.asarray(current_front_l, dtype=np.float64).reshape(-1, 3)
    valid_local = known & np.isfinite(source_points_l).all(axis=1) & (front.shape[0] > 0)
    if np.any(valid_local):
        query = source_points_l[valid_local]
        nearest_3d_idx, nearest_3d_dist = _nearest_indices_3d(front, query)
        nearest_yz_idx, _nearest_yz_dist = _nearest_indices(front, query)
        nearest_front_vertex_distance_mm[valid_local] = np.asarray(nearest_3d_dist, dtype=np.float64) * 1000.0
        front_x = front[nearest_yz_idx, 0]
        signed_x_mm = float(normal_sign) * (query[:, 0] - front_x) * 1000.0
        local_x_to_front_face_mm[valid_local] = signed_x_mm
        nearest_front_face_distance_mm[valid_local] = np.abs(signed_x_mm)

        yz = query[:, 1:3]
        yz_min = np.min(front[:, 1:3], axis=0)
        yz_max = np.max(front[:, 1:3], axis=0)
        yz_span = np.maximum(yz_max - yz_min, EPS)
        uv = (yz - yz_min) / yz_span
        uv_on_front_face[valid_local] = uv
        inside = np.all((uv >= 0.0) & (uv <= 1.0), axis=1)
        inside_front_uv_mask[valid_local] = inside

        membrane_valid_indices = np.flatnonzero(valid_local & (geometry_id == 1))
        if membrane_valid_indices.size:
            near = nearest_front_face_distance_mm[membrane_valid_indices] <= float(front_face_near_tolerance_mm)
            inside_membrane = inside_front_uv_mask[membrane_valid_indices]
            signed = local_x_to_front_face_mm[membrane_valid_indices]
            source_region_id[membrane_valid_indices[near & inside_membrane]] = 1
            source_region_id[membrane_valid_indices[near & ~inside_membrane]] = 2
            source_region_id[membrane_valid_indices[(~near) & (signed > float(front_face_near_tolerance_mm))]] = 5
            source_region_id[membrane_valid_indices[(~near) & (signed <= float(front_face_near_tolerance_mm))]] = 3

    force_l = _local_vectors_from_world(-grad, pad_quat_wxyz).astype(np.float32)
    return {
        "count": count,
        "global_indices": indices.astype(np.int64),
        "geometry_id": geometry_id.astype(np.int16),
        "source_region_id": source_region_id.astype(np.int16),
        "local_vertex_indices": local_vertex_indices.astype(np.int64),
        "source_points_l": source_points_l.astype(np.float32),
        "source_points_w": source_points_w.astype(np.float32),
        "gradient_w": grad.astype(np.float32),
        "force_l": force_l.astype(np.float32),
        "gradient_norm": grad_norm.astype(np.float32),
        "nearest_front_vertex_distance_mm": nearest_front_vertex_distance_mm.astype(np.float32),
        "nearest_front_face_distance_mm": nearest_front_face_distance_mm.astype(np.float32),
        "local_x_to_front_face_mm": local_x_to_front_face_mm.astype(np.float32),
        "uv_on_front_face": uv_on_front_face.astype(np.float32),
        "inside_front_uv_mask": inside_front_uv_mask.astype(bool),
        "summary": {
            "count": count,
            "geometry_counts": _count_labels(geometry_id, SOURCE_GEOMETRY_LABELS),
            "source_region_counts": _count_labels(source_region_id, SOURCE_REGION_LABELS),
            "nearest_front_vertex_distance_mm": _value_stats(nearest_front_vertex_distance_mm),
            "nearest_front_face_distance_mm": _value_stats(nearest_front_face_distance_mm),
            "local_x_to_front_face_mm": _value_stats(local_x_to_front_face_mm),
            "inside_front_uv_ratio": float(np.count_nonzero(inside_front_uv_mask)) / float(count) if count else 0.0,
        },
    }
...
def _native_uipc_contact_force_frame(
    *,
    ...
    pad_quat_wxyz: tuple[float, float, float, float],
    normal_sign: int,
    pad_pos_w: np.ndarray | None = None,
    current_front_l: np.ndarray | None = None,
    membrane_global_vertex_offset: int = -1,
    membrane_vertex_positions_w: np.ndarray | None = None,
    tool_global_vertex_offset: int = -1,
    tool_vertex_positions_w: np.ndarray | None = None,
    front_face_near_tolerance_mm: float = 0.25,
) -> dict[str, object]:
    ...
    result: dict[str, object] = {
        ...
        "attribute_shapes": {},
        "attribute_types": {},
        "source_geometry_audit": _empty_native_gradient_source_audit(),
        "error": "",
    }
    if feature is None:
        return result
    try:
        ...
        if (
            pad_pos_w is not None
            and current_front_l is not None
            and membrane_vertex_positions_w is not None
            and tool_vertex_positions_w is not None
        ):
            result["source_geometry_audit"] = _native_gradient_source_geometry_frame(
                global_indices=global_indices,
                gradient_w=grad,
                pad_pos_w=np.asarray(pad_pos_w, dtype=np.float64),
                pad_quat_wxyz=pad_quat_wxyz,
                current_front_l=np.asarray(current_front_l, dtype=np.float64),
                normal_sign=int(normal_sign),
                membrane_global_vertex_offset=int(membrane_global_vertex_offset),
                membrane_vertex_positions_w=np.asarray(membrane_vertex_positions_w, dtype=np.float64),
                tool_global_vertex_offset=int(tool_global_vertex_offset),
                tool_vertex_positions_w=np.asarray(tool_vertex_positions_w, dtype=np.float64),
                front_face_near_tolerance_mm=float(front_face_near_tolerance_mm),
            )
...
def _phase_source_summary(
    *,
    phase_name: str,
    phase_frames: list[str],
    source_frame_index: np.ndarray,
    source_geometry_id: np.ndarray,
    source_region_id: np.ndarray,
    nearest_front_vertex_distance_mm: np.ndarray,
    nearest_front_face_distance_mm: np.ndarray,
    local_x_to_front_face_mm: np.ndarray,
    inside_front_uv_mask: np.ndarray,
) -> dict[str, object]:
    frame_set = {idx for idx, phase in enumerate(phase_frames) if str(phase) == str(phase_name)}
    if not frame_set:
        return {"frames": 0, "native_nonzero_source_count": 0}
    frame_indices = np.asarray(source_frame_index, dtype=np.int64).reshape(-1)
    mask = np.asarray([int(frame) in frame_set for frame in frame_indices], dtype=bool)
    count = int(np.count_nonzero(mask))
    if count == 0:
        return {"frames": int(len(frame_set)), "native_nonzero_source_count": 0}
    return {
        "frames": int(len(frame_set)),
        "native_nonzero_source_count": count,
        "geometry_counts": _count_labels(source_geometry_id[mask], SOURCE_GEOMETRY_LABELS),
        "source_region_counts": _count_labels(source_region_id[mask], SOURCE_REGION_LABELS),
        "nearest_front_vertex_distance_mm": _value_stats(nearest_front_vertex_distance_mm[mask]),
        "nearest_front_face_distance_mm": _value_stats(nearest_front_face_distance_mm[mask]),
        "local_x_to_front_face_mm": _value_stats(local_x_to_front_face_mm[mask]),
        "inside_front_uv_ratio": float(np.count_nonzero(inside_front_uv_mask[mask])) / float(count),
    }


def _build_native_geometry_id_report(
    *,
    phase_frames: list[str],
    source_frame_index: np.ndarray,
    source_geometry_id: np.ndarray,
    ...
    inside_front_uv_mask: np.ndarray,
    membrane_global_vertex_offset: int,
    membrane_vertex_count: int,
    tool_global_vertex_offset: int,
    tool_vertex_count: int,
    front_face_near_tolerance_mm: float,
) -> dict[str, object]:
    count = int(np.asarray(source_geometry_id).reshape(-1).shape[0])
    unique_phases = list(dict.fromkeys(str(phase) for phase in phase_frames))
    return {
        "validation_name": "4_4 = native gradient source geometry audit",
        "question": "native nonzero contact_gradient 到底来自哪里？",
        "native_uipc_contact_force_replaces_proxy": False,
        "native_force_calibration_performed": False,
        "source_geometry_labels": {str(key): value for key, value in SOURCE_GEOMETRY_LABELS.items()},
        "source_region_labels": {str(key): value for key, value in SOURCE_REGION_LABELS.items()},
        "front_face_near_tolerance_mm": float(front_face_near_tolerance_mm),
        "membrane_global_vertex_offset": int(membrane_global_vertex_offset),
        "membrane_vertex_count": int(membrane_vertex_count),
        "tool_global_vertex_offset": int(tool_global_vertex_offset),
        "tool_vertex_count": int(tool_vertex_count),
        "native_nonzero_source_count": count,
        "geometry_counts": _count_labels(source_geometry_id, SOURCE_GEOMETRY_LABELS),
        "source_region_counts": _count_labels(source_region_id, SOURCE_REGION_LABELS),
        "nearest_front_vertex_distance_mm": _value_stats(nearest_front_vertex_distance_mm),
        "nearest_front_face_distance_mm": _value_stats(nearest_front_face_distance_mm),
        "local_x_to_front_face_mm": _value_stats(local_x_to_front_face_mm),
        "inside_front_uv_ratio": float(np.count_nonzero(inside_front_uv_mask)) / float(max(count, 1)),
        "phase_summary": {
            phase: _phase_source_summary(
                phase_name=phase,
                phase_frames=phase_frames,
                source_frame_index=source_frame_index,
                source_geometry_id=source_geometry_id,
                source_region_id=source_region_id,
                nearest_front_vertex_distance_mm=nearest_front_vertex_distance_mm,
                nearest_front_face_distance_mm=nearest_front_face_distance_mm,
                local_x_to_front_face_mm=local_x_to_front_face_mm,
                inside_front_uv_mask=inside_front_uv_mask,
            )
...
def _build_native_gradient_source_geometry_audit(
    *,
    geometry_report: dict[str, object],
    output_dir: Path,
) -> dict[str, object]:
    source_count = int(geometry_report.get("native_nonzero_source_count", 0))
    region_counts = geometry_report.get("source_region_counts", {})
    geometry_counts = geometry_report.get("geometry_counts", {})
    front_inside = int(region_counts.get("membrane_front_face_uv_inside", 0)) if isinstance(region_counts, dict) else 0
    front_outside = int(region_counts.get("membrane_front_face_uv_outside", 0)) if isinstance(region_counts, dict) else 0
    internal_or_back = int(region_counts.get("membrane_internal_or_back", 0)) if isinstance(region_counts, dict) else 0
    outside_front_plane = int(region_counts.get("membrane_outside_front_plane", 0)) if isinstance(region_counts, dict) else 0
    tool_count = int(geometry_counts.get("tool", 0)) if isinstance(geometry_counts, dict) else 0
    unknown_count = int(geometry_counts.get("unknown", 0)) if isinstance(geometry_counts, dict) else 0
    denom = float(max(source_count, 1))
    front_inside_ratio = float(front_inside) / denom
    not_current_front_face_ratio = float(
        source_count - front_inside
    ) / denom

    if source_count == 0:
        status = "no_native_nonzero_gradient_sources"
        interpretation = "No nonzero native contact_gradient source was observed in this run."
    elif front_inside_ratio >= 0.80:
        status = "native_sources_close_to_front_face_uv_inside"
        interpretation = (
            "Most nonzero native contact_gradient sources are close to the current sim-mesh front face and inside "
            "front UV bounds. If 4_3 remains spatially bad, the next likely issue is rasterization/index/UV mapping."
        )
    elif tool_count / denom >= 0.50:
        status = "native_sources_dominated_by_tool_vertices"
        interpretation = (
            "Most nonzero native contact_gradient sources are on the tool geometry, not the membrane front-face "
            "tactile layer. These cannot be directly rasterized as a membrane tactile patch."
        )
    elif (internal_or_back + outside_front_plane + front_outside + unknown_count) / denom >= 0.50:
        status = "native_sources_not_current_front_face_contact_layer"
        interpretation = (
            "Nonzero native contact_gradient sources are not dominated by front-face UV-inside membrane vertices. "
            "Current native gradient to front-face grid mapping is not valid without an additional geometry filter."
        )
    else:
        status = "mixed_native_gradient_sources"
        interpretation = (
            "Native contact_gradient sources are mixed across front-face and non-front-face regions. A geometry "
            "filter is required before any native tactile patch calibration."
        )

    return {
        "validation_name": "4_4 = native gradient source geometry audit",
        "native_gradient_source_geometry_audit_completed": True,
        "native_contact_gradient_readable": True,
        "native_contact_gradient_has_contact_stage_response": True,
        "raw_native_gradient_mixed_source": True,
        "raw_native_gradient_can_be_used_directly_as_tactile_patch": False,
        "native_can_replace_proxy": False,
        "next_step": "native_front_face_source_filter",
        "conclusion": (
            "4_4 completed the source geometry audit: native contact_gradient is readable and has contact-stage "
            "response, but raw native gradient is mixed across membrane front-face, membrane internal/back, and tool "
            "sources. Raw native gradient is not a clean tactile membrane output and must not be used directly as a "
            "tactile patch."
        ),
        "native_uipc_contact_force_replaces_proxy": False,
        "native_force_calibration_performed": False,
        ...
        "acceptance_logic": {
            "if_native_nonzero_points_far_from_front_face": (
                "contact_gradient source is not the current front-face tactile layer; do not directly use it as a tactile map"
            ),
            "if_native_nonzero_points_near_front_face_but_uv_grid_wrong": (
                "source geometry is plausible; inspect rasterization, index ordering, and YZ/UV mapping"
            ),
            "if_native_nonzero_points_on_boundary_anchor_back_or_internal": (
                "native gradient is not surface pressure patch; add geometry filtering before any patch calibration"
            ),
        },
        "ratios": {
            "front_face_uv_inside_ratio": front_inside_ratio,
            "not_current_front_face_ratio": not_current_front_face_ratio,
            "tool_ratio": float(tool_count) / denom,
            "unknown_ratio": float(unknown_count) / denom,
            "front_face_uv_outside_ratio": float(front_outside) / denom,
            "membrane_internal_or_back_ratio": float(internal_or_back) / denom,
            "membrane_outside_front_plane_ratio": float(outside_front_plane) / denom,
        },
        "geometry_report": geometry_report,
        "outputs": {
            "native_nonzero_local_points": str(output_dir / "native_nonzero_local_points.npy"),
            "native_nonzero_world_points": str(output_dir / "native_nonzero_world_points.npy"),
            "native_nearest_front_vertex_distance_mm": str(
                output_dir / "native_nearest_front_vertex_distance_mm.npy"
            ),
            "native_nearest_front_face_distance_mm": str(output_dir / "native_nearest_front_face_distance_mm.npy"),
            "native_local_x_to_front_face_mm": str(output_dir / "native_local_x_to_front_face_mm.npy"),
            "native_uv_on_sim_mesh_front_face": str(output_dir / "native_uv_on_sim_mesh_front_face.npy"),
            "native_inside_front_uv_mask": str(output_dir / "native_inside_front_uv_mask.npy"),
            "native_geometry_id_report": str(output_dir / "native_geometry_id_report.json"),
            "native_gradient_source_geometry_audit": str(output_dir / "native_gradient_source_geometry_audit.json"),
        },
    }
...
def _build_proxy_native_projection_comparison(
    *,
    proxy_sum_fz: np.ndarray,
    native_sum_fx: np.ndarray,
    native_sum_fy: np.ndarray,
    ...
    proxy = np.asarray(proxy_sum_fz, dtype=np.float64).reshape(-1)
    fx = np.asarray(native_sum_fx, dtype=np.float64).reshape(-1)
    fy = np.asarray(native_sum_fy, dtype=np.float64).reshape(-1)
    fz = np.asarray(native_sum_fz, dtype=np.float64).reshape(-1)
    fz_pos = np.asarray(native_sum_fz_compressive, dtype=np.float64).reshape(-1)
    native_abs = np.asarray(native_sum_abs, dtype=np.float64).reshape(-1)
    contact_counts = np.asarray(contact_candidate_count, dtype=np.int64).reshape(-1)
    available = np.asarray(native_available, dtype=bool).reshape(-1)
    length = int(
    ...
    contact_nonzero = contact_counts > 0
    proxy_nonzero = np.abs(proxy) > EPS
    fz_nonzero = np.abs(fz) > EPS
    fz_pos_nonzero = fz_pos > EPS
    abs_nonzero = native_abs > EPS
    phase_groups = {
        "pre_close": tuple(NON_CONTACT_BASELINE_PHASES),
        "close_confirm": ("CLOSE_GRIPPER", "CONFIRM_GRASP"),
        "lift_check_hold": ("LIFT_OBJECT", "CHECK_GRASP", "HOLD_VIEW"),
    ...
    return {
        "definition": {
            "proxy": "sum_fz.npy from contact_geometry penetration proxy, baseline corrected",
            "native_fx": "signed tangent component mapped from pad-local +Y force",
            "native_fy": "signed tangent component mapped from pad-local +Z force",
            "native_fz": "signed tactile normal component, -uipc_surface_normal_sign * pad_local_force_x",
            "native_fz_compressive": "clip(native_fz, 0, inf), useful for proxy timing comparison",
            "native_abs": "sum of Euclidean norms of pad-local native force vectors",
        },
        "frames": int(length),
        "native_available_frames": int(np.count_nonzero(available)),
        "proxy_nonzero_frames": int(np.count_nonzero(proxy_nonzero)),
        "native_fz_nonzero_frames": int(np.count_nonzero(fz_nonzero)),
        "native_fz_compressive_nonzero_frames": int(np.count_nonzero(fz_pos_nonzero)),
        "native_abs_nonzero_frames": int(np.count_nonzero(abs_nonzero)),
        "contact_candidate_nonzero_frames": int(np.count_nonzero(contact_nonzero)),
        "overlap_proxy_native_fz_nonzero_frames": int(np.count_nonzero(proxy_nonzero & fz_nonzero)),
        "overlap_proxy_native_fz_compressive_nonzero_frames": int(np.count_nonzero(proxy_nonzero & fz_pos_nonzero)),
        "overlap_contact_native_abs_nonzero_frames": int(np.count_nonzero(contact_nonzero & abs_nonzero)),
        "proxy_sum_max": float(np.max(proxy)) if proxy.size else 0.0,
        "native_fx_max_abs": float(np.max(np.abs(fx))) if fx.size else 0.0,
        "native_fy_max_abs": float(np.max(np.abs(fy))) if fy.size else 0.0,
        "native_fz_max_abs": float(np.max(np.abs(fz))) if fz.size else 0.0,
        "native_fz_compressive_max": float(np.max(fz_pos)) if fz_pos.size else 0.0,
        "native_abs_sum_max": float(np.max(native_abs)) if native_abs.size else 0.0,
        "pearson_proxy_native_fz": _finite_pearson(proxy, fz),
        "pearson_proxy_native_fz_compressive": _finite_pearson(proxy, fz_pos),
        "pearson_proxy_native_abs": _finite_pearson(proxy, native_abs),
        "phase_stats": {
            name: {
                "proxy": _phase_signed_stats(proxy, phase_list, phase_names),
                "native_fx": _phase_signed_stats(fx, phase_list, phase_names),
                "native_fy": _phase_signed_stats(fy, phase_list, phase_names),
                "native_fz": _phase_signed_stats(fz, phase_list, phase_names),
                "native_fz_compressive": _phase_signed_stats(fz_pos, phase_list, phase_names),
                "native_abs": _phase_signed_stats(native_abs, phase_list, phase_names),
            }
            for name, phase_names in phase_groups.items()
        },
    }


def _build_native_projection_diagnostics(
    *,
    native_local_xyz: np.ndarray,
    native_local_xyz_grid: np.ndarray,
    native_fx: np.ndarray,
    native_fy: np.ndarray,
    native_fz: np.ndarray,
    native_fz_compressive: np.ndarray,
    native_sum_fx: np.ndarray,
    native_sum_fy: np.ndarray,
    native_sum_fz: np.ndarray,
    native_sum_fz_compressive: np.ndarray,
    ...
    local_xyz = np.asarray(native_local_xyz, dtype=np.float64)
    local_xyz_grid = np.asarray(native_local_xyz_grid, dtype=np.float64)
    fx = np.asarray(native_fx, dtype=np.float64)
    fy = np.asarray(native_fy, dtype=np.float64)
    fz = np.asarray(native_fz, dtype=np.float64)
    fz_pos = np.asarray(native_fz_compressive, dtype=np.float64)
    sum_abs = np.asarray(native_sum_abs, dtype=np.float64).reshape(-1)
    grad = np.asarray(gradient_count, dtype=np.float64).reshape(-1)
    matched = np.asarray(matched_front_count, dtype=np.float64).reshape(-1)
    contact = np.asarray(contact_candidate_count, dtype=np.int64).reshape(-1)
    available = np.asarray(native_available, dtype=bool).reshape(-1)
    proxy_mask = np.asarray(proxy_fz_grid, dtype=np.float64) > EPS
    native_grid_norm = np.linalg.norm(local_xyz_grid, axis=-1) if local_xyz_grid.size else np.zeros_like(proxy_mask)
    native_mask = native_grid_norm > EPS
    native_fz_mask = np.abs(
        -float(normal_sign) * local_xyz_grid[..., 0]
        if local_xyz_grid.size
        else np.zeros_like(proxy_mask, dtype=np.float64)
    ) > EPS
    phase_groups = {
        "pre_close": tuple(NON_CONTACT_BASELINE_PHASES),
        "close_confirm": ("CLOSE_GRIPPER", "CONFIRM_GRASP"),
    ...
    per_frame_abs_component_sum = {
        "fx": np.sum(np.abs(fx), axis=1) if fx.ndim == 2 else np.zeros((0,), dtype=np.float64),
        "fy": np.sum(np.abs(fy), axis=1) if fy.ndim == 2 else np.zeros((0,), dtype=np.float64),
        "fz": np.sum(np.abs(fz), axis=1) if fz.ndim == 2 else np.zeros((0,), dtype=np.float64),
        "fz_compressive": np.sum(np.abs(fz_pos), axis=1) if fz_pos.ndim == 2 else np.zeros((0,), dtype=np.float64),
    }

    def cancellation_ratio(signed_sum: np.ndarray, abs_sum: np.ndarray) -> float:
        signed = np.asarray(signed_sum, dtype=np.float64).reshape(-1)
        absolute = np.asarray(abs_sum, dtype=np.float64).reshape(-1)
        mask = absolute > EPS
        if not np.any(mask):
            return 0.0
        return float(np.mean(np.abs(signed[mask]) / absolute[mask]))

    coverage = np.divide(matched, np.maximum(grad, 1.0), out=np.zeros_like(matched), where=grad > 0.0)
    front_coverage = matched / float(max(int(front_vertex_count), 1))
    return {
        "projection_frame": "pad-local tactile frame",
        "source_vector": "native_force_world = -ContactSystemFeature.contact_gradient",
        "pad_local_vector": "native_uipc_force_local_xyz.npy stores [pad_local_x, pad_local_y, pad_local_z]",
        "tactile_axis_mapping": {
            "Fx": "pad_local_y signed tangent force",
            "Fy": "pad_local_z signed tangent force",
            "Fz": f"-normal_sign * pad_local_x signed normal force; normal_sign={int(normal_sign)}",
            "Fz_compressive": "clip(Fz, 0, inf), not used as main tactile output",
        },
        "native_uipc_contact_force_used": False,
        "native_uipc_contact_force_replaces_proxy": False,
        "front_vertex_count": int(front_vertex_count),
        "available_frames": int(np.count_nonzero(available)),
        "contact_candidate_nonzero_frames": int(np.count_nonzero(contact > 0)),
        "gradient_count_max": int(np.max(grad)) if grad.size else 0,
        "matched_front_count_max": int(np.max(matched)) if matched.size else 0,
        "matched_over_gradient_mean_when_gradient_nonzero": float(np.mean(coverage[grad > 0.0]))
        if np.any(grad > 0.0)
        else 0.0,
        "matched_over_front_mean_when_available": float(np.mean(front_coverage[available]))
        if np.any(available)
        else 0.0,
        "component_max_abs": {
            "pad_local_x": float(np.max(np.abs(local_xyz[..., 0]))) if local_xyz.size else 0.0,
            "pad_local_y": float(np.max(np.abs(local_xyz[..., 1]))) if local_xyz.size else 0.0,
            "pad_local_z": float(np.max(np.abs(local_xyz[..., 2]))) if local_xyz.size else 0.0,
            "Fx": float(np.max(np.abs(fx))) if fx.size else 0.0,
            "Fy": float(np.max(np.abs(fy))) if fy.size else 0.0,
            "Fz": float(np.max(np.abs(fz))) if fz.size else 0.0,
            "Fz_compressive": float(np.max(np.abs(fz_pos))) if fz_pos.size else 0.0,
        },
        "sum_max_abs": {
            "sum_native_fx": float(np.max(np.abs(native_sum_fx))) if np.asarray(native_sum_fx).size else 0.0,
            "sum_native_fy": float(np.max(np.abs(native_sum_fy))) if np.asarray(native_sum_fy).size else 0.0,
            "sum_native_fz": float(np.max(np.abs(native_sum_fz))) if np.asarray(native_sum_fz).size else 0.0,
            "sum_native_fz_compressive": float(np.max(np.abs(native_sum_fz_compressive)))
            if np.asarray(native_sum_fz_compressive).size
            else 0.0,
            "sum_native_abs": float(np.max(np.abs(sum_abs))) if sum_abs.size else 0.0,
        },
        "signed_cancellation_ratio_mean": {
            "Fx": cancellation_ratio(native_sum_fx, per_frame_abs_component_sum["fx"]),
            "Fy": cancellation_ratio(native_sum_fy, per_frame_abs_component_sum["fy"]),
            "Fz": cancellation_ratio(native_sum_fz, per_frame_abs_component_sum["fz"]),
            "Fz_compressive": cancellation_ratio(native_sum_fz_compressive, per_frame_abs_component_sum["fz_compressive"]),
        },
        "grid_overlap_with_proxy_contact": {
            name: {
                "native_abs": _phase_grid_overlap_stats(
                    proxy_mask=proxy_mask,
                    native_mask=native_mask,
                    phases=phases,
                    phase_names=phase_names,
                ),
                "native_fz": _phase_grid_overlap_stats(
                    proxy_mask=proxy_mask,
                    native_mask=native_fz_mask,
                    phases=phases,
                    phase_names=phase_names,
                ),
            }
...
def main() -> None:
    ...
    print(
        json.dumps(
            {
                "script_version": "v5_new_4_4_native_gradient_source_geometry_audit",
                "expected_conda_env": EXPECTED_CONDA_ENV,
                "runtime_conda_env": os.environ.get("CONDA_DEFAULT_ENV", ""),
                "native_uipc_contact_force_probe_enabled": bool(args_cli.native_uipc_force_probe),
                "native_gradient_source_geometry_audit_enabled": True,
                "native_force_projection_calibration_enabled": False,
                "native_projection_sign_correlation_calibration_enabled": False,
                "native_uipc_contact_force_used": False,
                "native_uipc_contact_force_replaces_proxy": False,
                "native_force_projection_axis_mapping": {
                    "native_uipc_force_local_xyz": "[pad_local_x, pad_local_y, pad_local_z]",
                    "Fx": "pad_local_y signed tangent component",
                    "Fy": "pad_local_z signed tangent component",
                    "Fz": "-uipc_surface_normal_sign * pad_local_x signed normal component",
                    "Fz_compressive": "clip(Fz, 0, inf), diagnostic only",
                },
    ...
    try:
        tool_global_vertex_offset = _uipc_global_vertex_offset(tool)
    except Exception as exc:
        tool_global_vertex_offset = -1
        native_uipc_force_probe_info["tool_global_vertex_offset_status"] = "missing"
        native_uipc_force_probe_info["tool_global_vertex_offset_error"] = f"{type(exc).__name__}: {exc}"
    _set_collision_enabled_for_tree(stage, ANCHOR_PATH, False)
    _set_collision_enabled_for_tree(stage, TOOL_ROOT, False)
    ...
    native_uipc_force_probe_info.update(
        {
            "membrane_global_vertex_offset": int(membrane_global_vertex_offset),
            "membrane_vertex_count": int(gel_init_vertices_l.shape[0]),
            "tool_global_vertex_offset": int(tool_global_vertex_offset),
            "tool_vertex_count": int(tool_init_vertices_l.shape[0]),
    ...
    sum_native_fx_frames: list[float] = []
    sum_native_fy_frames: list[float] = []
    sum_native_fz_frames: list[float] = []
    sum_native_fz_compressive_frames: list[float] = []
    sum_native_abs_frames: list[float] = []
    ...
    native_source_frame_index_chunks: list[np.ndarray] = []
    native_source_global_index_chunks: list[np.ndarray] = []
    native_source_geometry_id_chunks: list[np.ndarray] = []
    native_source_region_id_chunks: list[np.ndarray] = []
    native_source_local_vertex_index_chunks: list[np.ndarray] = []
    native_source_local_point_chunks: list[np.ndarray] = []
    native_source_world_point_chunks: list[np.ndarray] = []
    native_source_gradient_w_chunks: list[np.ndarray] = []
    native_source_force_l_chunks: list[np.ndarray] = []
    native_source_gradient_norm_chunks: list[np.ndarray] = []
    native_nearest_front_vertex_distance_mm_chunks: list[np.ndarray] = []
    native_nearest_front_face_distance_mm_chunks: list[np.ndarray] = []
    native_local_x_to_front_face_mm_chunks: list[np.ndarray] = []
    native_uv_on_sim_mesh_front_face_chunks: list[np.ndarray] = []
    native_inside_front_uv_mask_chunks: list[np.ndarray] = []
    native_gradient_source_frame_summaries: list[dict[str, object]] = []
    ...
    try:
        ...
        for phase in phases:
            ...
            for phase_frame in range(frame_count):
                ...
                current_membrane_vertices_w = _uipc_object_vertex_positions_w(uipc_sim, membrane)
                current_tool_vertices_w = _uipc_object_vertex_positions_w(uipc_sim, tool)
                native_probe_frame = _native_uipc_contact_force_frame(
                    ...
                    pad_pos_w=current_pad_pos,
                    current_front_l=current_front_l,
                    membrane_global_vertex_offset=membrane_global_vertex_offset,
                    membrane_vertex_positions_w=current_membrane_vertices_w,
                    tool_global_vertex_offset=tool_global_vertex_offset,
                    tool_vertex_positions_w=current_tool_vertices_w,
                    front_face_near_tolerance_mm=float(args_cli.native_front_face_near_tolerance_mm),
                )
                ...
                native_fx = native_force_l[:, 1].astype(np.float32)
                native_fy = native_force_l[:, 2].astype(np.float32)
                native_fz = (-float(uipc_normal_sign) * native_force_l[:, 0]).astype(np.float32)
                native_fz_compressive = np.clip(native_fz, 0.0, None).astype(np.float32)
                native_fx_grid = native_force_grid[..., 1].astype(np.float32)
                native_fy_grid = native_force_grid[..., 2].astype(np.float32)
                native_fz_grid = (-float(uipc_normal_sign) * native_force_grid[..., 0]).astype(np.float32)
                native_fz_compressive_grid = np.clip(native_fz_grid, 0.0, None).astype(np.float32)
                ...
                sum_native_fx = float(np.sum(native_fx))
                sum_native_fy = float(np.sum(native_fy))
                sum_native_fz = float(np.sum(native_fz))
                sum_native_fz_compressive = float(np.sum(native_fz_compressive))
                sum_native_abs = float(np.sum(np.linalg.norm(native_force_l, axis=1))) if native_force_l.size else 0.0
                ...
                sum_native_fx_frames.append(sum_native_fx)
                sum_native_fy_frames.append(sum_native_fy)
                sum_native_fz_frames.append(sum_native_fz)
                sum_native_fz_compressive_frames.append(sum_native_fz_compressive)
                sum_native_abs_frames.append(sum_native_abs)
                ...
                source_audit = native_probe_frame.get("source_geometry_audit", _empty_native_gradient_source_audit())
                source_count = int(source_audit.get("count", 0)) if isinstance(source_audit, dict) else 0
                native_gradient_source_frame_summaries.append(
                    {
                        "frame": int(total_frames),
                        "phase": str(phase_name),
                        "summary": source_audit.get("summary", {}) if isinstance(source_audit, dict) else {},
                    }
                )
                if source_count > 0 and isinstance(source_audit, dict):
                    native_source_frame_index_chunks.append(
                        np.full((source_count,), int(total_frames), dtype=np.int32)
                    )
                    native_source_global_index_chunks.append(
                        np.asarray(source_audit["global_indices"], dtype=np.int64)
                    )
                    native_source_geometry_id_chunks.append(np.asarray(source_audit["geometry_id"], dtype=np.int16))
                    native_source_region_id_chunks.append(
                        np.asarray(source_audit["source_region_id"], dtype=np.int16)
                    )
                    native_source_local_vertex_index_chunks.append(
                        np.asarray(source_audit["local_vertex_indices"], dtype=np.int64)
                    )
                    native_source_local_point_chunks.append(
                        np.asarray(source_audit["source_points_l"], dtype=np.float32)
                    )
                    native_source_world_point_chunks.append(
                        np.asarray(source_audit["source_points_w"], dtype=np.float32)
                    )
                    native_source_gradient_w_chunks.append(
                        np.asarray(source_audit["gradient_w"], dtype=np.float32)
                    )
                    native_source_force_l_chunks.append(np.asarray(source_audit["force_l"], dtype=np.float32))
                    native_source_gradient_norm_chunks.append(
                        np.asarray(source_audit["gradient_norm"], dtype=np.float32)
                    )
                    native_nearest_front_vertex_distance_mm_chunks.append(
                        np.asarray(source_audit["nearest_front_vertex_distance_mm"], dtype=np.float32)
                    )
                    native_nearest_front_face_distance_mm_chunks.append(
                        np.asarray(source_audit["nearest_front_face_distance_mm"], dtype=np.float32)
                    )
                    native_local_x_to_front_face_mm_chunks.append(
                        np.asarray(source_audit["local_x_to_front_face_mm"], dtype=np.float32)
                    )
                    native_uv_on_sim_mesh_front_face_chunks.append(
                        np.asarray(source_audit["uv_on_front_face"], dtype=np.float32)
                    )
                    native_inside_front_uv_mask_chunks.append(
                        np.asarray(source_audit["inside_front_uv_mask"], dtype=bool)
                    )
                ...
                if total_frames % max(1, int(args_cli.native_uipc_force_probe_log_every)) == 0:
                    print(
                        ...
                        f"sum_proj_fx={sum_native_fx:.6e} "
                        f"sum_proj_fy={sum_native_fy:.6e} "
                        f"sum_proj_fz={sum_native_fz:.6e} "
                        f"sum_proj_fz_pos={sum_native_fz_compressive:.6e} "
        ...
        compare_proxy_vs_native_projection: dict[str, object] = {}
        native_projection_diagnostics: dict[str, object] = {}
        native_axis_sign_candidates: dict[str, object] = {}
        native_proxy_phase_correlation: dict[str, object] = {}
        native_proxy_grid_alignment: dict[str, object] = {}
        native_sign_correlation_diagnostics: dict[str, object] = {}
        native_geometry_id_report: dict[str, object] = {}
        native_gradient_source_geometry_audit: dict[str, object] = {}
        if uipc_frame_count > 0:
            ...
            sum_native_fx_array = np.asarray(sum_native_fx_frames, dtype=np.float32)
            sum_native_fy_array = np.asarray(sum_native_fy_frames, dtype=np.float32)
            sum_native_fz_array = np.asarray(sum_native_fz_frames, dtype=np.float32)
            sum_native_fz_compressive_array = np.asarray(sum_native_fz_compressive_frames, dtype=np.float32)
            sum_native_abs_array = np.asarray(sum_native_abs_frames, dtype=np.float32)
            ...
            native_source_frame_index_array = (
                np.concatenate(native_source_frame_index_chunks, axis=0).astype(np.int32)
                if native_source_frame_index_chunks
                else np.zeros((0,), dtype=np.int32)
            )
            native_source_global_index_array = (
                np.concatenate(native_source_global_index_chunks, axis=0).astype(np.int64)
                if native_source_global_index_chunks
                else np.zeros((0,), dtype=np.int64)
            )
            native_source_geometry_id_array = (
                np.concatenate(native_source_geometry_id_chunks, axis=0).astype(np.int16)
                if native_source_geometry_id_chunks
                else np.zeros((0,), dtype=np.int16)
            )
            native_source_region_id_array = (
                np.concatenate(native_source_region_id_chunks, axis=0).astype(np.int16)
                if native_source_region_id_chunks
                else np.zeros((0,), dtype=np.int16)
            )
            native_source_local_vertex_index_array = (
                np.concatenate(native_source_local_vertex_index_chunks, axis=0).astype(np.int64)
                if native_source_local_vertex_index_chunks
                else np.full((0,), -1, dtype=np.int64)
            )
            native_nonzero_local_points_array = (
                np.concatenate(native_source_local_point_chunks, axis=0).astype(np.float32)
                if native_source_local_point_chunks
                else np.zeros((0, 3), dtype=np.float32)
            )
            native_nonzero_world_points_array = (
                np.concatenate(native_source_world_point_chunks, axis=0).astype(np.float32)
                if native_source_world_point_chunks
                else np.zeros((0, 3), dtype=np.float32)
            )
            native_source_gradient_w_array = (
                np.concatenate(native_source_gradient_w_chunks, axis=0).astype(np.float32)
                if native_source_gradient_w_chunks
                else np.zeros((0, 3), dtype=np.float32)
            )
            native_source_force_l_array = (
                np.concatenate(native_source_force_l_chunks, axis=0).astype(np.float32)
                if native_source_force_l_chunks
                else np.zeros((0, 3), dtype=np.float32)
            )
            native_source_gradient_norm_array = (
                np.concatenate(native_source_gradient_norm_chunks, axis=0).astype(np.float32)
                if native_source_gradient_norm_chunks
                else np.zeros((0,), dtype=np.float32)
            )
            native_nearest_front_vertex_distance_mm_array = (
                np.concatenate(native_nearest_front_vertex_distance_mm_chunks, axis=0).astype(np.float32)
                if native_nearest_front_vertex_distance_mm_chunks
                else np.zeros((0,), dtype=np.float32)
            )
            native_nearest_front_face_distance_mm_array = (
                np.concatenate(native_nearest_front_face_distance_mm_chunks, axis=0).astype(np.float32)
                if native_nearest_front_face_distance_mm_chunks
                else np.zeros((0,), dtype=np.float32)
            )
            native_local_x_to_front_face_mm_array = (
                np.concatenate(native_local_x_to_front_face_mm_chunks, axis=0).astype(np.float32)
                if native_local_x_to_front_face_mm_chunks
                else np.zeros((0,), dtype=np.float32)
            )
            native_uv_on_sim_mesh_front_face_array = (
                np.concatenate(native_uv_on_sim_mesh_front_face_chunks, axis=0).astype(np.float32)
                if native_uv_on_sim_mesh_front_face_chunks
                else np.zeros((0, 2), dtype=np.float32)
            )
            native_inside_front_uv_mask_array = (
                np.concatenate(native_inside_front_uv_mask_chunks, axis=0).astype(bool)
                if native_inside_front_uv_mask_chunks
                else np.zeros((0,), dtype=bool)
            )
            native_uipc_force_local_xyz_array = native_uipc_contact_force_raw_array
            native_uipc_force_local_xyz_grid_array = native_uipc_contact_force_grid_array
            native_uipc_force_fx_array = native_uipc_force_local_xyz_array[..., 1].astype(np.float32)
            native_uipc_force_fy_array = native_uipc_force_local_xyz_array[..., 2].astype(np.float32)
            native_uipc_force_fz_array = (-float(uipc_normal_sign) * native_uipc_force_local_xyz_array[..., 0]).astype(
                np.float32
            )
            native_uipc_force_fz_compressive_array = np.clip(native_uipc_force_fz_array, 0.0, None).astype(np.float32)
            native_uipc_force_fx_grid_array = native_uipc_force_local_xyz_grid_array[..., 1].astype(np.float32)
            native_uipc_force_fy_grid_array = native_uipc_force_local_xyz_grid_array[..., 2].astype(np.float32)
            native_uipc_force_fz_grid_array = (
                -float(uipc_normal_sign) * native_uipc_force_local_xyz_grid_array[..., 0]
            ).astype(np.float32)
            native_uipc_force_fz_compressive_grid_array = np.clip(native_uipc_force_fz_grid_array, 0.0, None).astype(
                np.float32
            )
            ...
            compare_proxy_vs_native_projection = _build_proxy_native_projection_comparison(
                proxy_sum_fz=sum_fz_array,
                native_sum_fx=sum_native_fx_array,
                native_sum_fy=sum_native_fy_array,
                native_sum_fz=sum_native_fz_array,
                native_sum_fz_compressive=sum_native_fz_compressive_array,
                native_sum_abs=sum_native_abs_array,
                contact_candidate_count=contact_candidate_count_array,
                native_available=native_uipc_force_available_array,
                phases=phase_frames,
            )
            native_projection_diagnostics = _build_native_projection_diagnostics(
                native_local_xyz=native_uipc_force_local_xyz_array,
                native_local_xyz_grid=native_uipc_force_local_xyz_grid_array,
                native_fx=native_uipc_force_fx_array,
                native_fy=native_uipc_force_fy_array,
                native_fz=native_uipc_force_fz_array,
                native_fz_compressive=native_uipc_force_fz_compressive_array,
                native_sum_fx=sum_native_fx_array,
                native_sum_fy=sum_native_fy_array,
                native_sum_fz=sum_native_fz_array,
                native_sum_fz_compressive=sum_native_fz_compressive_array,
                native_sum_abs=sum_native_abs_array,
                proxy_fz_grid=local_fxyz_grid_array[..., 2],
                native_available=native_uipc_force_available_array,
                gradient_count=native_uipc_gradient_count_array,
                matched_front_count=native_uipc_matched_front_count_array,
                contact_candidate_count=contact_candidate_count_array,
                phases=phase_frames,
                normal_sign=uipc_normal_sign,
                front_vertex_count=int(front_indices.shape[0]),
            )
            native_axis_sign_candidates = _build_native_axis_sign_candidates(
                proxy_sum_fz=sum_fz_array,
                native_local_xyz=native_uipc_force_local_xyz_array,
                native_available=native_uipc_force_available_array,
                gradient_count=native_uipc_gradient_count_array,
            ...
            native_proxy_phase_correlation = _build_native_proxy_phase_correlation(
                proxy_sum_fz=sum_fz_array,
                native_sum_fx=sum_native_fx_array,
                native_sum_fy=sum_native_fy_array,
                native_sum_fz=sum_native_fz_array,
                ...
                native_sum_fz_compressive=sum_native_fz_compressive_array,
                native_sum_abs=sum_native_abs_array,
                native_sum_uipc_force=sum_native_uipc_force_array,
                native_available=native_uipc_force_available_array,
                matched_front_count=native_uipc_matched_front_count_array,
                gradient_count=native_uipc_gradient_count_array,
                contact_candidate_count=contact_candidate_count_array,
            ...
            native_proxy_grid_alignment = _build_native_proxy_grid_alignment(
                proxy_fz_grid=local_fxyz_grid_array[..., 2],
                native_local_xyz_grid=native_uipc_force_local_xyz_grid_array,
                normal_sign=uipc_normal_sign,
                phases=phase_frames,
            )
            native_sign_correlation_diagnostics = _build_native_sign_correlation_diagnostics(
                native_axis_sign_candidates=native_axis_sign_candidates,
                native_proxy_phase_correlation=native_proxy_phase_correlation,
                native_proxy_grid_alignment=native_proxy_grid_alignment,
                normal_sign=uipc_normal_sign,
            )
            native_geometry_id_report = _build_native_geometry_id_report(
                phase_frames=phase_frames,
                source_frame_index=native_source_frame_index_array,
                source_geometry_id=native_source_geometry_id_array,
                source_region_id=native_source_region_id_array,
                nearest_front_vertex_distance_mm=native_nearest_front_vertex_distance_mm_array,
                nearest_front_face_distance_mm=native_nearest_front_face_distance_mm_array,
                local_x_to_front_face_mm=native_local_x_to_front_face_mm_array,
                inside_front_uv_mask=native_inside_front_uv_mask_array,
                membrane_global_vertex_offset=int(membrane_global_vertex_offset),
                membrane_vertex_count=int(gel_init_vertices_l.shape[0]),
                tool_global_vertex_offset=int(tool_global_vertex_offset),
                tool_vertex_count=int(tool_init_vertices_l.shape[0]),
            ...
            native_gradient_source_geometry_audit = _build_native_gradient_source_geometry_audit(
                geometry_report=native_geometry_id_report,
            ...
            np.save(output_dir / "native_uipc_force_local_xyz.npy", native_uipc_force_local_xyz_array)
            np.save(output_dir / "native_uipc_force_local_xyz_grid.npy", native_uipc_force_local_xyz_grid_array)
            np.save(output_dir / "native_uipc_force_fx.npy", native_uipc_force_fx_array)
            np.save(output_dir / "native_uipc_force_fy.npy", native_uipc_force_fy_array)
            np.save(output_dir / "native_uipc_force_fz.npy", native_uipc_force_fz_array)
            np.save(output_dir / "native_uipc_force_fz_compressive.npy", native_uipc_force_fz_compressive_array)
            np.save(output_dir / "native_uipc_force_fx_grid.npy", native_uipc_force_fx_grid_array)
            np.save(output_dir / "native_uipc_force_fy_grid.npy", native_uipc_force_fy_grid_array)
            np.save(output_dir / "native_uipc_force_fz_grid.npy", native_uipc_force_fz_grid_array)
            np.save(
                output_dir / "native_uipc_force_fz_compressive_grid.npy",
                native_uipc_force_fz_compressive_grid_array,
            )
            np.save(output_dir / "native_nonzero_local_points.npy", native_nonzero_local_points_array)
            np.save(output_dir / "native_nonzero_world_points.npy", native_nonzero_world_points_array)
            np.save(
                output_dir / "native_nearest_front_vertex_distance_mm.npy",
                native_nearest_front_vertex_distance_mm_array,
            )
            np.save(
                output_dir / "native_nearest_front_face_distance_mm.npy",
                native_nearest_front_face_distance_mm_array,
            )
            np.save(output_dir / "native_local_x_to_front_face_mm.npy", native_local_x_to_front_face_mm_array)
            np.save(output_dir / "native_uv_on_sim_mesh_front_face.npy", native_uv_on_sim_mesh_front_face_array)
            np.save(output_dir / "native_inside_front_uv_mask.npy", native_inside_front_uv_mask_array)
            np.save(output_dir / "native_source_frame_index.npy", native_source_frame_index_array)
            np.save(output_dir / "native_source_global_index.npy", native_source_global_index_array)
            np.save(output_dir / "native_source_geometry_id.npy", native_source_geometry_id_array)
            np.save(output_dir / "native_source_region_id.npy", native_source_region_id_array)
            np.save(output_dir / "native_source_local_vertex_index.npy", native_source_local_vertex_index_array)
            np.save(output_dir / "native_source_gradient_w.npy", native_source_gradient_w_array)
            np.save(output_dir / "native_source_force_l.npy", native_source_force_l_array)
            np.save(output_dir / "native_source_gradient_norm.npy", native_source_gradient_norm_array)
            np.save(output_dir / "sum_native_fx.npy", sum_native_fx_array)
            np.save(output_dir / "sum_native_fy.npy", sum_native_fy_array)
            np.save(output_dir / "sum_native_fz.npy", sum_native_fz_array)
            np.save(output_dir / "sum_native_fz_compressive.npy", sum_native_fz_compressive_array)
            np.save(output_dir / "sum_native_abs.npy", sum_native_abs_array)
            ...
            (output_dir / "native_projection_diagnostics.json").write_text(
                json.dumps(native_projection_diagnostics, indent=2),
                encoding="utf-8",
            )
            (output_dir / "compare_proxy_vs_native_projection.json").write_text(
                json.dumps(compare_proxy_vs_native_projection, indent=2),
                encoding="utf-8",
            )
            (output_dir / "native_sign_correlation_diagnostics.json").write_text(
                json.dumps(native_sign_correlation_diagnostics, indent=2),
                encoding="utf-8",
            )
            (output_dir / "native_axis_sign_candidates.json").write_text(
                json.dumps(native_axis_sign_candidates, indent=2),
                encoding="utf-8",
            )
            (output_dir / "native_proxy_phase_correlation.json").write_text(
                json.dumps(native_proxy_phase_correlation, indent=2),
                encoding="utf-8",
            )
            (output_dir / "native_proxy_grid_alignment.json").write_text(
                json.dumps(native_proxy_grid_alignment, indent=2),
                encoding="utf-8",
            )
            (output_dir / "native_geometry_id_report.json").write_text(
                json.dumps(native_geometry_id_report, indent=2),
                encoding="utf-8",
            )
            (output_dir / "native_gradient_source_geometry_audit.json").write_text(
                json.dumps(native_gradient_source_geometry_audit, indent=2),
                encoding="utf-8",
            )
            (output_dir / "native_gradient_source_frame_summaries.json").write_text(
                json.dumps(native_gradient_source_frame_summaries, indent=2),
                encoding="utf-8",
            )
        ...
        native_uipc_probe_summary.update(
            {
                ...
                "max_sum_native_fx_abs": float(np.max(np.abs(sum_native_fx_frames)))
                if sum_native_fx_frames
                else 0.0,
                "max_sum_native_fy_abs": float(np.max(np.abs(sum_native_fy_frames)))
                if sum_native_fy_frames
                else 0.0,
                "max_sum_native_fz_abs": float(np.max(np.abs(sum_native_fz_frames)))
                if sum_native_fz_frames
                else 0.0,
                "max_sum_native_fz_compressive": float(np.max(sum_native_fz_compressive_frames))
                if sum_native_fz_compressive_frames
                else 0.0,
                "max_sum_native_abs": float(np.max(sum_native_abs_frames))
                if sum_native_abs_frames
                else 0.0,
                "outputs": {
                    ...
                    "native_uipc_force_local_xyz": str(output_dir / "native_uipc_force_local_xyz.npy"),
                    "native_uipc_force_local_xyz_grid": str(output_dir / "native_uipc_force_local_xyz_grid.npy"),
                    "native_uipc_force_fx": str(output_dir / "native_uipc_force_fx.npy"),
                    "native_uipc_force_fy": str(output_dir / "native_uipc_force_fy.npy"),
                    "native_uipc_force_fz": str(output_dir / "native_uipc_force_fz.npy"),
                    "native_uipc_force_fz_compressive": str(output_dir / "native_uipc_force_fz_compressive.npy"),
                    "native_uipc_force_fx_grid": str(output_dir / "native_uipc_force_fx_grid.npy"),
                    "native_uipc_force_fy_grid": str(output_dir / "native_uipc_force_fy_grid.npy"),
                    "native_uipc_force_fz_grid": str(output_dir / "native_uipc_force_fz_grid.npy"),
                    "native_uipc_force_fz_compressive_grid": str(
                        output_dir / "native_uipc_force_fz_compressive_grid.npy"
                    ),
                    "sum_native_fx": str(output_dir / "sum_native_fx.npy"),
                    "sum_native_fy": str(output_dir / "sum_native_fy.npy"),
                    "sum_native_fz": str(output_dir / "sum_native_fz.npy"),
                    "sum_native_fz_compressive": str(output_dir / "sum_native_fz_compressive.npy"),
                    "sum_native_abs": str(output_dir / "sum_native_abs.npy"),
                    "compare_proxy_vs_native": str(output_dir / "compare_proxy_vs_native.json"),
                    "native_projection_diagnostics": str(output_dir / "native_projection_diagnostics.json"),
                    "compare_proxy_vs_native_projection": str(
                        output_dir / "compare_proxy_vs_native_projection.json"
                    ),
                    "native_sign_correlation_diagnostics": str(
                        output_dir / "native_sign_correlation_diagnostics.json"
                    ),
                    "native_axis_sign_candidates": str(output_dir / "native_axis_sign_candidates.json"),
                    "native_proxy_phase_correlation": str(output_dir / "native_proxy_phase_correlation.json"),
                    "native_proxy_grid_alignment": str(output_dir / "native_proxy_grid_alignment.json"),
                    "native_nonzero_local_points": str(output_dir / "native_nonzero_local_points.npy"),
                    "native_nonzero_world_points": str(output_dir / "native_nonzero_world_points.npy"),
                    "native_nearest_front_vertex_distance_mm": str(
                        output_dir / "native_nearest_front_vertex_distance_mm.npy"
                    ),
                    "native_nearest_front_face_distance_mm": str(
                        output_dir / "native_nearest_front_face_distance_mm.npy"
                    ),
                    "native_local_x_to_front_face_mm": str(output_dir / "native_local_x_to_front_face_mm.npy"),
                    "native_uv_on_sim_mesh_front_face": str(output_dir / "native_uv_on_sim_mesh_front_face.npy"),
                    "native_inside_front_uv_mask": str(output_dir / "native_inside_front_uv_mask.npy"),
                    "native_geometry_id_report": str(output_dir / "native_geometry_id_report.json"),
                    "native_gradient_source_geometry_audit": str(
                        output_dir / "native_gradient_source_geometry_audit.json"
                    ),
                },
            }
        )
        ...
        metadata = {
            ...
            "expected_conda_env": EXPECTED_CONDA_ENV,
            "runtime_conda_env": os.environ.get("CONDA_DEFAULT_ENV", ""),
            "purpose": "audit_native_contact_gradient_source_geometry_before_any_force_or_patch_calibration",
            "pad_layer_direction_contract": PAD_LAYER_DIRECTION_CONTRACT,
            "uipc_contact_face_source": PAD_CONTACT_FACE_SOURCE,
            "visual_layer_swap_enabled": bool(args_cli.visual_layer_swap),
            ...
            "native_force_projection_calibration": native_projection_diagnostics,
            "compare_proxy_vs_native_projection": compare_proxy_vs_native_projection,
            "native_projection_sign_correlation_calibration": native_sign_correlation_diagnostics,
            "native_axis_sign_candidates": native_axis_sign_candidates,
            "native_proxy_phase_correlation": native_proxy_phase_correlation,
            "native_proxy_grid_alignment": native_proxy_grid_alignment,
            "native_geometry_id_report": native_geometry_id_report,
            "native_gradient_source_geometry_audit": native_gradient_source_geometry_audit,
            "native_gradient_source_geometry_audit_completed": bool(
                native_gradient_source_geometry_audit.get("native_gradient_source_geometry_audit_completed", False)
            )
            if isinstance(native_gradient_source_geometry_audit, dict)
            else False,
            "native_can_replace_proxy": False,
            "next_step": "interpret_native_gradient_source_geometry_before_mapper_or_filter_fix",
            "native_force_projection_axis_mapping": {
                "native_uipc_force_local_xyz": "[pad_local_x, pad_local_y, pad_local_z]",
                "Fx": "pad_local_y signed tangent component",
                "Fy": "pad_local_z signed tangent component",
            ...
            "native_projection_fx_fy_outputs_created": uipc_frame_count > 0,
            "native_projection_replaces_local_fxyz": False,
            "native_projection_sign_correlation_outputs_created": uipc_frame_count > 0,
            "fx_fy_disabled": True,
            "fx_fy_disabled_scope": "compatibility local_fxyz remains proxy Fz only; native Fx/Fy are side-channel outputs",
            ...
            json.dumps(
                {
                    ...
                    "max_sum_native_fx_abs": float(native_uipc_probe_summary.get("max_sum_native_fx_abs", 0.0)),
                    "max_sum_native_fy_abs": float(native_uipc_probe_summary.get("max_sum_native_fy_abs", 0.0)),
                    "max_sum_native_fz_abs": float(native_uipc_probe_summary.get("max_sum_native_fz_abs", 0.0)),
                    "max_sum_native_abs": float(native_uipc_probe_summary.get("max_sum_native_abs", 0.0)),
                    "compare_proxy_vs_native": str(output_dir / "compare_proxy_vs_native.json"),
                    "native_projection_diagnostics": str(output_dir / "native_projection_diagnostics.json"),
                    "compare_proxy_vs_native_projection": str(output_dir / "compare_proxy_vs_native_projection.json"),
                    "native_sign_correlation_diagnostics": str(
                        output_dir / "native_sign_correlation_diagnostics.json"
                    ),
                    "native_axis_sign_candidates": str(output_dir / "native_axis_sign_candidates.json"),
