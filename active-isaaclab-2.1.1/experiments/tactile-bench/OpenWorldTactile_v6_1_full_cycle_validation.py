...
"""V6.1 full-cycle free-object tactile validation on frozen 7f -> 7g -> v9.
...
def _validate_v6_args() -> None:
    ...
    if int(_v6_args.init_unrecorded_max_frames) != 180:
        raise ValueError("v6.1 freezes INIT_UNRECORDED to a maximum of 180 frames")
    if int(args_cli.warmup_stability_frames) != 30:
        raise ValueError("v6.1 freezes the readiness window at 30 consecutive frames")
    if int(args_cli.uipc_substeps_per_record) != UIPC_SUBSTEPS:
        raise ValueError("v6.1 requires exactly three UIPC substeps per PhysX frame")
    if int(args_cli.membrane_cells_x) != 1 or int(args_cli.membrane_cells_y) != 22 or int(args_cli.membrane_cells_z) != 26:
        raise ValueError("v6.1 freezes the structured membrane at 1x22x26 cells")
    if str(args_cli.membrane_mesh_mode) != "structured":
        raise ValueError("v6.1 requires the frozen structured membrane")
    if not math.isclose(float(args_cli.youngs_modulus_mpa), 0.05) or not math.isclose(float(args_cli.poisson_rate), 0.49) or not math.isclose(float(args_cli.mass_density), 1050.0):
        raise ValueError("v6.1 freezes membrane material at E=0.05 MPa, nu=0.49, density=1050")
    if int(args_cli.field_height) != 81 or int(args_cli.field_width) != 65 or not math.isclose(float(args_cli.gaussian_sigma_cells), 1.25):
        raise ValueError("v6.1 freezes the v9 field at 81x65 and sigma=1.25 cells")
...
def _phase_plan(waypoints: dict[str, np.ndarray]) -> list[dict[str, object]]:
    ...
    targets = {
        ...
        "RETURN_OBJECT": (waypoints["place"], closed),
        "CONFIRM_SUPPORT": (waypoints["place"], closed),
        "RELEASE_GRIPPER": (waypoints["place"], opened),
        "RETREAT_AND_RECOVER": (waypoints["above_pick"], opened),
    }
    plan = [
        {"name": name, "target": targets[name][0], "opening": targets[name][1], "frames": frames}
        for name, frames in PHASE_SPECS
    ]
    if sum(int(phase["frames"]) for phase in plan) != OFFICIAL_FRAME_COUNT:
        raise RuntimeError("The frozen v6.1 formal phase plan must total exactly 495 frames")
    return plan
...
def _save_record_arrays(output_dir: Path, records: dict[str, list[object]]) -> None:
    array_names = {
        ...
        "uipc_mirror_pose_w": "uipc_mirror_pose_w.npy",
        "uipc_mirror_substep_pose_w": "uipc_mirror_substep_pose_w.npy",
        "uipc_attachment_substep_pose_w": "uipc_attachment_substep_pose_w.npy",
        "uipc_mirror_substep_displacement_mm": "uipc_mirror_substep_displacement_mm.npy",
        "uipc_mirror_substep_rotation_deg": "uipc_mirror_substep_rotation_deg.npy",
        "uipc_mirror_generalized_velocity_norm": "uipc_mirror_generalized_velocity_norm.npy",
        "uipc_mirror_q_prev_q_delta_norm": "uipc_mirror_q_prev_q_delta_norm.npy",
    ...
    np.save(output_dir / "physx_object_pose_w.npy", np.asarray(records["object_pose_w"]))
    np.save(output_dir / "physx_object_velocity_w.npy", np.asarray(records["object_velocity_w"]))
    np.save(output_dir / "gripper_opening.npy", np.asarray(records["gripper_opening_mm"]))
    np.save(output_dir / "penetration_depth_max_mm.npy", np.asarray(records["max_penetration_mm"]))
    np.save(output_dir / "contact_count.npy", np.asarray(records["contact_vertex_count"]))
...
def main() -> None:
    ...
    if total_formal_frames != OFFICIAL_FRAME_COUNT:
        raise RuntimeError(f"Expected 495 formal frames, got {total_formal_frames}")
        ...
        for phase in phases:
            ...
            for phase_frame in range(frame_count):
                ...
                for substep_index in range(substep_count):
                    ...
                    if any(error > 0.05 for error in errors):
                        ...
                        raise RuntimeError(
                            "V6.1 substep synchronization exceeded 0.05 mm or 0.05 deg"
                        )
                    ...
                    if not (
                        ...
                        raise RuntimeError("Backend ABD q_prev/q state does not match pose-pair targets")
            ...
            if phase_name == "CONFIRM_GRASP":
                ...
                if not grasp_ready:
                    ...
                    raise RuntimeError("CONFIRM_GRASP readiness failed; refusing to lift")
            if phase_name == "CONFIRM_SUPPORT":
                support_clearance_tail = np.asarray(
                    records["support_clearance_mm"][-10:], dtype=np.float64
                )
                vertical_velocity_tail = np.asarray(
                    records["object_velocity_w"][-10:], dtype=np.float64
                )[:, 2]
                support_ready = bool(
                    support_clearance_tail.size == 10
                    and np.max(np.abs(support_clearance_tail))
                    <= float(_v6_args.support_contact_tolerance_mm)
                    and np.max(np.abs(vertical_velocity_tail)) < 0.02
                    and np.ptp(support_clearance_tail) < 0.5
                )
                if not support_ready:
                    _json_write(
                        output_dir / "failure_snapshot.json",
                        {
                            "official_pass": False,
                            "phase": phase_name,
                            "reason": "CONFIRM_SUPPORT failed; refusing to release",
                        },
                    )
                    raise RuntimeError("CONFIRM_SUPPORT failed; refusing to open gripper")
...
if __name__ == "__main__":
    try:
        ...
        try:
            ...
            for video_name in (
                "free_object_grasp_scene.mp4",
                "tactile_fxyz_composite_sequence.mp4",
            ):
                (error_output_dir / video_name).unlink(missing_ok=True)
            ...
            if not (error_output_dir / "gate_results.json").exists():
                _json_write(
                    error_output_dir / "gate_results.json",
                    {
                        "official_pass": False,
                        "v6_1_gate_1_12_passed": False,
                        "v6_1_gate_13_five_episode_passed": False,
                        "error": traceback.format_exc(),
                    },
                )
