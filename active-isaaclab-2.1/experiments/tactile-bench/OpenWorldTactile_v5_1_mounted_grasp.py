...
parser = argparse.ArgumentParser(
    description=(
        "V5.1 direct UIPC mounted grasp. Native Piper carries UIPC_Pad on link7, a "
        "rigid nut proxy is mirrored into UIPC as a kinematic contact tool, and the "
        "UIPC membrane deformation is written back to the mounted pad visual mesh."
    )
)
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_uipc_v5_1_direct_uipc_mounted_grasp")
parser.add_argument("--workspace_dir", type=str, default="/tmp/openworldtactile_uipc_v5_1_workspace")
...
parser.add_argument(
    "--nut_place_mode",
    type=str,
    default="pad_front_auto",
    choices=("pad_front_auto", "link_midpoint", "manual_world"),
)
parser.add_argument("--nut_setup_gap_mm", type=float, default=0.8)
...
parser.add_argument("--nut_contact_offset_mm", type=float, default=1.0)
parser.add_argument("--nut_dynamic_friction", type=float, default=2.0)
parser.add_argument("--nut_disable_gravity", dest="nut_disable_gravity", action="store_true", default=True)
parser.add_argument("--nut_enable_gravity", dest="nut_disable_gravity", action="store_false")
...
parser.add_argument("--tet_edge_length_r", type=float, default=1.0 / 24.0)
...
def _cylinder_surface_mesh(radius: float, height: float, radial_segments: int = 32) -> tuple[np.ndarray, np.ndarray]:
    zs = np.asarray([-0.5 * height, 0.5 * height], dtype=np.float32)
    theta_values = np.linspace(0.0, 2.0 * math.pi, radial_segments, endpoint=False, dtype=np.float32)
    points: list[tuple[float, float, float]] = []
    for z in zs:
        for theta in theta_values:
            points.append((radius * math.cos(float(theta)), radius * math.sin(float(theta)), float(z)))
    bottom_center = len(points)
    top_center = bottom_center + 1
    points.append((0.0, 0.0, float(zs[0])))
    points.append((0.0, 0.0, float(zs[1])))
    triangles: list[tuple[int, int, int]] = []

    def idx(layer: int, i: int) -> int:
        return layer * radial_segments + (i % radial_segments)

    for i in range(radial_segments):
        b0, b1 = idx(0, i), idx(0, i + 1)
        t0, t1 = idx(1, i), idx(1, i + 1)
        triangles.extend(((b0, t0, b1), (b1, t0, t1)))
...
def _make_nut_proxy() -> RigidObject:
    nut_cfg = RigidObjectCfg(
        ...
        spawn=sim_utils.CylinderCfg(
            ...
            visual_material=sim_utils.PreviewSurfaceCfg(diffuse_color=(0.82, 0.70, 0.42), roughness=0.48),
        ),
    )
...
def _auto_nut_position_from_pad_front(
    *,
    front_points_l: np.ndarray,
    ...
    nut_radius_m: float,
    nut_height_m: float,
    setup_gap_m: float = 0.8e-3,
) -> np.ndarray:
    front_center_l = np.mean(np.asarray(front_points_l, dtype=np.float64), axis=0)
    front_center_w = _world_from_local(front_center_l.reshape(1, 3), pad_pos_w, pad_quat_wxyz)[0].astype(np.float64)

    n_l = np.asarray((float(normal_sign), 0.0, 0.0), dtype=np.float64)
    n_w = _quat_apply(pad_quat_wxyz, n_l)
    n_w = n_w / max(float(np.linalg.norm(n_w)), EPS)

    to_link8 = np.asarray(link8_pos_w, dtype=np.float64) - front_center_w
    if float(np.dot(n_w, to_link8)) < 0.0:
        n_w = -n_w

    tool_safe_radius = math.sqrt(float(nut_radius_m) ** 2 + (0.5 * float(nut_height_m)) ** 2)
    return front_center_w + n_w * (tool_safe_radius + float(setup_gap_m))
...
def _local_fxyz_from_uipc_deformation(
    ...
    *,
    ...
    signed_dx = float(normal_sign) * (rest_front_l[:, 0] - current_front_l[:, 0])
    indent = np.clip(signed_dx, 0.0, None).astype(np.float32)
    ...
    local_fx = np.zeros_like(local_fz, dtype=np.float32)
    local_fy = np.zeros_like(local_fz, dtype=np.float32)
    local_fz *= mask
...
def _render_uipc_tool_vertices(nut: RigidObject, tool_local_points: np.ndarray) -> tuple[np.ndarray, np.ndarray, tuple[float, float, float, float]]:
    nut_state = nut.data.root_state_w[0].detach().cpu().numpy().astype(np.float64)
    nut_pos = nut_state[0:3]
    nut_quat = _quat_normalize(tuple(float(v) for v in nut_state[3:7]))
    return _world_from_local(tool_local_points, nut_pos, nut_quat), nut_pos, nut_quat
...
def main() -> None:
    ...
    tool_local_points, tool_triangles = _cylinder_surface_mesh(
        radius=float(args_cli.nut_radius_mm) * 1.0e-3,
        height=float(args_cli.nut_height_mm) * 1.0e-3,
        radial_segments=32,
    )
    initial_tool_points_w = _world_from_local(
        tool_local_points,
        np.asarray((float(args_cli.nut_manual_x_m), float(args_cli.nut_manual_y_m), float(args_cli.nut_manual_z_m)), dtype=np.float64),
        (1.0, 0.0, 0.0, 0.0),
    )
    _write_triangle_mesh(stage, TOOL_MESH, initial_tool_points_w, tool_triangles, color=(0.82, 0.70, 0.42), opacity=0.55)
    ...
    link8_path = _normalize_abs_or_robot_path(str(args_cli.nut_link_b))
    link8_view = _make_xform_prim_view(link8_path)
    link8_pos_t, _ = _read_xform_pose(link8_view, device=sim.device)
    link8_pos_now = link8_pos_t.detach().cpu().numpy().astype(np.float64)
    ...
    normal_check = _check_membrane_normal_alignment(
        ...
        link8_pos_w=link8_pos_now,
        warn_deg=float(args_cli.normal_alignment_warn_deg),
    )
    print(json.dumps({"membrane_normal_check": normal_check}, indent=2), flush=True)

    if str(args_cli.nut_place_mode) == "pad_front_auto":
        nut_position_m = _auto_nut_position_from_pad_front(
            front_points_l=front_points_l_init,
            pad_pos_w=pad_pos_now,
            pad_quat_wxyz=pad_quat_now,
            link8_pos_w=link8_pos_now,
            normal_sign=normal_sign,
            nut_radius_m=float(args_cli.nut_radius_mm) * 1.0e-3,
            nut_height_m=float(args_cli.nut_height_mm) * 1.0e-3,
            setup_gap_m=float(args_cli.nut_setup_gap_mm) * 1.0e-3,
        )
        nut_place_info = {
            "mode": "pad_front_auto",
            "link8_path": link8_path,
            "setup_gap_m": float(args_cli.nut_setup_gap_mm) * 1.0e-3,
            "normal_sign": int(normal_sign),
            "auto_center_m": [float(v) for v in nut_position_m],
        }
    else:
        nut_position_m, nut_place_info = _compute_nut_position(sim.device)
    _place_root(nut, nut_position_m, (1.0, 0.0, 0.0, 0.0), device=sim.device)
    nut.update(0.0)

    membrane_root = str(pad_contract["simulation_root"])
    _hide_mesh_prim(stage, membrane_mesh_path)
    tool_surface_points_w, setup_nut_pos, setup_nut_quat = _render_uipc_tool_vertices(nut, tool_local_points)
    _set_mesh_points(stage, TOOL_MESH, tool_surface_points_w)
    ...
    membrane.update(0.0)
    tool.update(0.0)
    tool_tet_vertices_l = _local_from_world(
        tool.init_vertex_pos.detach().cpu().numpy().astype(np.float32),
        setup_nut_pos,
        setup_nut_quat,
    )
    ...
    for warmup_step in range(max(0, int(args_cli.warmup_steps))):
        ...
        anchor_center_w = _world_from_local(anchor_center_l.reshape(1, 3), pad_pos, pad_quat)[0]
        _move_root_no_reset(anchor, anchor_center_w, pad_quat, device=sim.device)
        tool_points_w, _, _ = _render_uipc_tool_vertices(nut, tool_tet_vertices_l)
        tool.write_vertex_positions_to_sim(torch.as_tensor(tool_points_w, device=sim.device, dtype=torch.float32))
    ...
    for frame_id in range(total_frames):
        ...
        if frame_id % max(1, int(args_cli.log_every)) == 0 or frame_id == total_frames - 1:
            print(
                "[INFO] v5_1 direct-uipc "
                f"frame={frame_id:04d}/{total_frames} phase={phase} opening={current_opening_mm:.3f}mm "
                f"sum_fz={float(np.sum(local_fxyz[:, 2])):.6f} max_indent_mm={float(np.max(indent)) * 1000.0:.4f}",
                flush=True,
            )
    ...
    metadata = {
        "script_version": "v5_1_direct_uipc_mounted_grasp",
        "uipc_solver_used": True,
        "force_source": "uipc_deformation_proxy",
        "deformation_source": "uipc_membrane_surface_direct_contact",
        ...
        "nut_proxy": {
            "physx_prim_path": NUT_PRIM_PATH,
            "uipc_tool_prim_path": TOOL_ROOT,
