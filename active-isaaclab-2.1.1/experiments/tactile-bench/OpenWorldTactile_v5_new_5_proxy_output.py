...
EPS = 1.0e-12
SCRIPT_VERSION = "v5_new_5_official_proxy_tactile_output"
...
parser = argparse.ArgumentParser(
    description=(
        "V5 new stage 5: official proxy tactile output. This is the formal data-output version of "
        "the validated 3h pipeline. It uses only contact_geometry penetration proxy Fz with baseline "
        "correction as the tactile source, keeps native UIPC contact_gradient out of formal outputs, "
        "and writes local_fxyz, grid outputs, pressure videos, metadata, and native route rejection summary."
    )
)
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_5_official_proxy_tactile_output")
parser.add_argument("--workspace_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_5_workspace")
    ...
    try:
            ...
            if bool(args_cli.save_pressure_video):
                ...
                try:
                    for frame_idx in range(local_fxyz_grid_array.shape[0]):
                        fz_grid = local_fxyz_grid_array[frame_idx, ..., 2]
                        ...
                        if gray_writer is None:
                            gray_writer = _open_video_writer(output_dir / "pressure_fz_gray_sequence.mp4", gray_img)
                        if rgb_writer is None:
                            rgb_writer = _open_video_writer(output_dir / "pressure_fz_rgb_sequence.mp4", rgb_img)
                        gray_writer.write(cv2.cvtColor(gray_img, cv2.COLOR_RGB2BGR))
                        rgb_writer.write(cv2.cvtColor(rgb_img, cv2.COLOR_RGB2BGR))
                    pressure_video_written = gray_writer is not None and rgb_writer is not None
                finally:
        ...
        metadata = {
            ...
            "purpose": (
                ...
                "penetration_proxy_fz_only_baseline_corrected"
            ),
            "official_tactile_source": OFFICIAL_TACTILE_SOURCE,
            ...
            "pressure_video_mask_source": "local_fz_grid_positive_values",
