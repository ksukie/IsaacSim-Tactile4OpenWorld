...
_DEFAULT_POLICY_CKPT = _REPO_ROOT / "checkpoints/dreamtac_sim_eval/ckpt/sim_pickplace_stage2/stage2_last.pt"
_DEFAULT_JEPA_CKPT = _REPO_ROOT / "checkpoints/dreamtac_sim_eval/jepa_ckpt/vitl.pt"

parser = argparse.ArgumentParser(
    description="DreamTac closed-loop inference for the AgileX Piper randomized pick-and-place scene."
)
...
parser.add_argument(
    "--record_dir",
    type=str,
    default="dataset/dreamtac_sim_eval_rollouts",
    help="Directory where DreamTac inference HDF5 rollouts are written.",
)
parser.add_argument(
    "--max_episodes",
    type=int,
...
parser.add_argument(
    "--policy_ckpt",
    type=str,
    default=str(_DEFAULT_POLICY_CKPT),
    help="DreamTac Stage-2 checkpoint used for closed-loop inference.",
)
parser.add_argument(
    "--jepa_ckpt",
    type=str,
    default=str(_DEFAULT_JEPA_CKPT),
    help="V-JEPA2 tactile backbone checkpoint used by DreamTacPolicy.",
)
parser.add_argument(
    "--policy_device",
    type=str,
    default="cuda",
    help="Torch device for DreamTacPolicy. Use cuda for real closed-loop evaluation.",
)
parser.add_argument(
    "--infer_horizon",
    type=int,
    default=562,
    help="Maximum number of policy-controlled steps per inference episode.",
)
parser.add_argument(
    "--disable_auto_success_reset",
    default=False,
    action="store_true",
    help="Disable early reset when the object is stably placed in the container.",
)
parser.add_argument(
    "--success_hold_steps",
    type=int,
    default=15,
    help="Consecutive placed-and-released steps required before automatic success reset.",
)
parser.add_argument(
    "--success_gripper_open_threshold",
    type=float,
    default=0.026,
    help="Observed gripper opening threshold used to confirm release before automatic success reset.",
)
parser.add_argument(
    "--disable_policy_bgr",
    default=False,
    action="store_true",
    help="Feed online camera frames to the policy as RGB instead of OpenCV-style BGR.",
)
parser.add_argument(
    "--no_temporal_agg",
    default=False,
    action="store_true",
    help="Disable ACT temporal aggregation in DreamTacPolicy.act().",
)
...
def _import_dreamtac_policy(dreamtac_root: Path):
    stage2_path = dreamtac_root / "stage2_infer.py"
    if not stage2_path.exists():
        print(f"[ERROR] Missing stage2_infer.py: {stage2_path}")
        raise SystemExit(1)

    module_name = "_dreamtac_stage2_infer_runtime"
    spec = importlib.util.spec_from_file_location(module_name, stage2_path)
    if spec is None or spec.loader is None:
        print(f"[ERROR] Could not create import spec for: {stage2_path}")
        raise SystemExit(1)

    module = importlib.util.module_from_spec(spec)
    sys.modules[module_name] = module
    spec.loader.exec_module(module)
    return module.DreamTacPolicy, module


def _load_dreamtac_policy(stage2_ckpt: str, vitl_path: str, device: str, temporal_agg: bool):
    dreamtac_root = _REPO_ROOT / "checkpoints" / "dreamtac_sim_eval"
    if str(dreamtac_root) not in sys.path:
        sys.path.insert(0, str(dreamtac_root))

    ckpt_path = _resolve_repo_path(stage2_ckpt)
    jepa_path = _resolve_repo_path(vitl_path)
    for required_path, label in ((ckpt_path, "policy_ckpt"), (jepa_path, "jepa_ckpt")):
        if not required_path.exists():
            print(f"[ERROR] Missing {label}: {required_path}")
            raise SystemExit(1)

    print("[INFO] Loading DreamTac weights")
    print(f"[INFO]   policy_ckpt input : {stage2_ckpt}")
    print(f"[INFO]   policy_ckpt file  : {ckpt_path} ({_format_file_size(ckpt_path)})")
    print(f"[INFO]   jepa_ckpt input   : {vitl_path}")
    print(f"[INFO]   jepa_ckpt file    : {jepa_path} ({_format_file_size(jepa_path)})")
    print(f"[INFO]   device            : {device}")
    print(f"[INFO]   temporal_agg      : {temporal_agg}")

    try:
        DreamTacPolicy, stage2_module = _import_dreamtac_policy(dreamtac_root)
        print(f"[INFO]   stage2_infer file: {Path(stage2_module.__file__).resolve()}")
        print(f"[INFO]   policy class file: {inspect.getsourcefile(DreamTacPolicy)}")
    except ModuleNotFoundError as exc:
        print(
            "[ERROR] Missing Python module required by DreamTacPolicy in the current environment: "
            f"{exc.name}"
        )
        print("[ERROR] Environment packages were not modified. Use an environment that already has this module.")
        raise SystemExit(1) from exc
    except Exception as exc:
        print(f"[ERROR] Failed to import DreamTacPolicy: {type(exc).__name__}: {exc}")
        print(traceback.format_exc())
        print("[ERROR] Environment packages were not modified.")
        raise SystemExit(1) from exc

    try:
        print("[INFO] Constructing DreamTacPolicy", flush=True)
        ...
        print(
            "[INFO] DreamTacPolicy loaded "
            f"(chunk={policy.chunk}, action_dim={policy.action_dim}, "
            f"state_dim={policy.state_dim}, tactile_cams={policy.tactile_cams})"
        )
        return policy
    except ModuleNotFoundError as exc:
        print(
            "[ERROR] Missing Python module required while constructing DreamTacPolicy: "
            f"{exc.name}"
        )
        print("[ERROR] Environment packages were not modified. Use an environment that already has this module.")
        raise SystemExit(1) from exc
    except Exception as exc:
        print(f"[ERROR] DreamTacPolicy construction failed: {type(exc).__name__}: {exc}")
        print(traceback.format_exc())
        print("[ERROR] Environment packages were not modified.")
        raise SystemExit(1) from exc
...
class HDF5EpisodeRecorder:
    ...
    def close_episode(self, extra_attrs: dict[str, object] | None = None):
...
@dataclass
class InferenceEpisodeMetrics:
    ever_grasped: bool = False
    final_placed: bool = False
    final_xy_distance: float = float("inf")
    final_cylinder_z: float = float("inf")
    max_lift_delta: float = 0.0
    max_tactile_peak: float = 0.0
    last_gripper_opening: float = 0.0
    success_hold_count: int = 0
    episode_steps: int = 0
    done_reason: str = "horizon"

    @property
    def success(self) -> bool:
        return self.ever_grasped and self.final_placed


def _policy_frame_from_rgb(frame_rgb: np.ndarray, use_bgr: bool) -> np.ndarray:
    if not use_bgr:
...
def _get_policy_obs(
    ...
    top_rgb = HDF5EpisodeRecorder._to_uint8_rgb(env.scene_camera.data.output["rgb"])
    wrist_rgb = HDF5EpisodeRecorder._to_uint8_rgb(env.wrist_camera.data.output["rgb"])
    tactile = env.gsmini_left._data.output["tactile_force_field"][0].detach().cpu().numpy().astype(
        np.float32,
        copy=True,
    )
    qpos = recorder._piper_joint_state_7(env, env._robot.data.joint_pos)
    return {
        "top": _policy_frame_from_rgb(top_rgb, policy_bgr),
        "wrist": _policy_frame_from_rgb(wrist_rgb, policy_bgr),
        "tactile": tactile,
        "qpos": qpos,
    }


def _apply_policy_action(env: CylinderContainerSceneEnv, recorder: HDF5EpisodeRecorder, action: np.ndarray):
    action = np.asarray(action, dtype=np.float32).reshape(-1)
    if action.shape[0] != 7:
        raise RuntimeError(f"DreamTacPolicy.act() must return 7 values, got shape {action.shape}.")
    if not np.isfinite(action).all():
        raise RuntimeError(f"DreamTacPolicy.act() returned non-finite action: {action}")

    joint_pos_des = env._robot.data.joint_pos.clone()
    arm_joint_ids = recorder._resolve_arm_joint_ids(env)
    arm_target = torch.as_tensor(action[:6], device=env.device, dtype=joint_pos_des.dtype)
    joint_pos_des[:, arm_joint_ids] = arm_target.unsqueeze(0)

    gripper_opening = env._clamp_gripper_opening(float(action[6]))
    joint_pos_des[:, env._finger_joint_ids] = gripper_opening * env._finger_joint_signs

    joint_limits = env._robot.data.soft_joint_pos_limits
    lower = joint_limits[:, :, 0]
    upper = joint_limits[:, :, 1]
    joint_pos_des = torch.max(torch.min(joint_pos_des, upper), lower)

    env._last_joint_action = joint_pos_des.detach().clone()
...
def _advance_scripted_settle(
    ...
    """Match the collection-script settle phase before DreamTac takes control."""
    env._apply_pick_place_state_machine()
    env._pre_physics_step(None)
    env._apply_action()
    env._apply_grasp_assist_motion()
    _advance_world(env, camera_display, tactile_display, force_windows, step)


def _update_inference_metrics(env: CylinderContainerSceneEnv, metrics: InferenceEpisodeMetrics):
    grip_pos_w, _ = env._grip_frame_pose_w()
    cylinder_pos_w = env.cylinder.data.root_link_pos_w
    lift_delta = cylinder_pos_w[:, 2] - env._initial_cylinder_pos_w[:, 2]
    distance = torch.linalg.norm(cylinder_pos_w - grip_pos_w, dim=-1)
    grasped = (lift_delta > env.cfg.grasp_lift_threshold) & (distance < env.cfg.grasp_distance_threshold)
    metrics.ever_grasped = metrics.ever_grasped or bool(grasped[0].item())
    metrics.max_lift_delta = max(metrics.max_lift_delta, float(lift_delta[0].item()))
    metrics.max_tactile_peak = max(metrics.max_tactile_peak, env._tactile_force_peak())
    ...
    container_xy_w = container_xy_w + env.scene.env_origins[:, :2]
    xy_distance = torch.linalg.norm(cylinder_pos_w[:, :2] - container_xy_w, dim=-1)
    cylinder_z = cylinder_pos_w[:, 2]
    metrics.final_xy_distance = float(xy_distance[0].item())
    metrics.final_cylinder_z = float(cylinder_z[0].item())
    placed = (xy_distance < env.cfg.place_xy_threshold) & (cylinder_z < env.cfg.place_z_max)
    metrics.final_placed = bool(placed[0].item())


def _update_auto_success_reset(
    metrics: InferenceEpisodeMetrics,
    qpos: np.ndarray,
    success_hold_steps: int,
    gripper_open_threshold: float,
) -> bool:
    metrics.last_gripper_opening = float(qpos[6])
    released = metrics.last_gripper_opening >= gripper_open_threshold
    if metrics.ever_grasped and metrics.final_placed and released:
        metrics.success_hold_count += 1
    else:
        metrics.success_hold_count = 0
    return metrics.success_hold_count >= success_hold_steps
...
def _metrics_attrs(
    ...
    jepa_ckpt: str,
    infer_horizon: int,
    auto_success_reset: bool,
    success_hold_steps: int,
    success_gripper_open_threshold: float,
    policy_bgr: bool,
) -> dict[str, object]:
    return {
        "infer": True,
        "policy_ckpt": str(_resolve_repo_path(policy_ckpt)),
        "jepa_ckpt": str(_resolve_repo_path(jepa_ckpt)),
        "infer_horizon": int(infer_horizon),
        "policy_image_order": "BGR" if policy_bgr else "RGB",
        "episode_steps": int(metrics.episode_steps),
        "done_reason": str(metrics.done_reason),
        "auto_success_reset": bool(auto_success_reset),
        "success_hold_steps": int(success_hold_steps),
        "success_hold_count": int(metrics.success_hold_count),
        "success_gripper_open_threshold": float(success_gripper_open_threshold),
        "last_gripper_opening": float(metrics.last_gripper_opening),
        "success": bool(metrics.success),
        "ever_grasped": bool(metrics.ever_grasped),
        "final_placed": bool(metrics.final_placed),
        "final_xy_distance": float(metrics.final_xy_distance),
        "final_cylinder_z": float(metrics.final_cylinder_z),
        "max_lift_delta": float(metrics.max_lift_delta),
        "max_tactile_peak": float(metrics.max_tactile_peak),
    }
...
def run_inference(
    ...
    auto_success_reset: bool = True,
    success_hold_steps: int = 15,
    success_gripper_open_threshold: float = 0.026,
    policy_bgr: bool = True,
):
    if env.num_envs != 1:
        raise RuntimeError("DreamTac closed-loop inference currently supports num_envs=1 only.")

    print(f"Starting DreamTac closed-loop inference with {env.num_envs} env")
    success_hold_steps = max(1, int(success_hold_steps))
    success_gripper_open_threshold = float(success_gripper_open_threshold)
    print(f"[INFO] infer_horizon={infer_horizon}, settle_steps={env.cfg.settle_steps}")
    print(
        "[INFO] auto_success_reset="
        f"{auto_success_reset}, success_hold_steps={success_hold_steps}, "
        f"success_gripper_open_threshold={success_gripper_open_threshold:.4f}"
    )
    print(f"[INFO] policy_image_order={'BGR' if policy_bgr else 'RGB'}")
    env.reset()
    if env.cfg.enable_tactile:
    ...
    while simulation_app.is_running():
        ...
        metrics = InferenceEpisodeMetrics()
        print(f"[INFO] Episode {recorder.written_episodes}: scenario={env.current_scenario.name}")

        for _ in range(env.cfg.settle_steps):
            if not simulation_app.is_running():
                break
            _advance_scripted_settle(env, camera_display, tactile_display, force_windows, total_steps)
            total_steps += 1
            if max_steps is not None and total_steps >= max_steps:
                break

        warm_qpos = recorder._piper_joint_state_7(env, env._robot.data.joint_pos)
        print(f"[INFO] Warm-start qpos before DreamTac: {np.round(warm_qpos, 4)}")
        policy.reset()

        for t in range(infer_horizon):
            if not simulation_app.is_running():
                metrics.done_reason = "app_closed"
                break
            if max_steps is not None and total_steps >= max_steps:
                metrics.done_reason = "max_steps"
                break

            obs = _get_policy_obs(env, recorder, policy_bgr)
            action = policy.act(obs)
            _apply_policy_action(env, recorder, action)
            _advance_world(env, camera_display, tactile_display, force_windows, total_steps)
            _update_inference_metrics(env, metrics)
            qpos_after_step = recorder._piper_joint_state_7(env, env._robot.data.joint_pos)
            metrics.last_gripper_opening = float(qpos_after_step[6])
            auto_done = False
            if auto_success_reset:
                auto_done = _update_auto_success_reset(
                    metrics,
                    qpos_after_step,
                    success_hold_steps,
                    success_gripper_open_threshold,
            ...
            total_steps += 1
            metrics.episode_steps = t + 1
            if auto_done:
                metrics.done_reason = "success"
                print(
                    "[INFO] Auto success reset -> "
                    f"t={t}, hold={metrics.success_hold_count}/{success_hold_steps}, "
                    f"xy={metrics.final_xy_distance:.3f}, z={metrics.final_cylinder_z:.3f}, "
                    f"gripper={metrics.last_gripper_opening:.4f}"
                )
                break

        attrs = _metrics_attrs(
            metrics,
            args_cli.policy_ckpt,
            args_cli.jepa_ckpt,
            infer_horizon,
            auto_success_reset,
            success_hold_steps,
        ...
        recorder.close_episode(attrs)
        print(
            "[INFO] Inference result -> "
            f"done_reason={metrics.done_reason}, steps={metrics.episode_steps}, "
            f"success={metrics.success}, ever_grasped={metrics.ever_grasped}, "
            f"final_placed={metrics.final_placed}, final_xy_distance={metrics.final_xy_distance:.3f}, "
            f"final_cylinder_z={metrics.final_cylinder_z:.3f}, max_lift_delta={metrics.max_lift_delta:.3f}, "
            f"max_tactile_peak={metrics.max_tactile_peak:.4f}, "
            f"last_gripper_opening={metrics.last_gripper_opening:.4f}"
        )
...
def main():
    ...
    policy = _load_dreamtac_policy(
        args_cli.policy_ckpt,
        args_cli.jepa_ckpt,
        device=args_cli.policy_device,
        temporal_agg=not args_cli.no_temporal_agg,
    )
    print("[INFO]: DreamTac inference setup complete.")
    run_inference(
        env=experiment,
        policy=policy,
        max_steps=args_cli.max_steps,
        recorder=recorder,
        infer_horizon=max(0, args_cli.infer_horizon),
        max_episodes=args_cli.max_episodes,
        auto_success_reset=not args_cli.disable_auto_success_reset,
        success_hold_steps=args_cli.success_hold_steps,
        success_gripper_open_threshold=args_cli.success_gripper_open_threshold,
        policy_bgr=not args_cli.disable_policy_bgr,
    )
