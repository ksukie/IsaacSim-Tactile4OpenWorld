...
bool SimEngine::do_write_global_vertex_pos_pair_to_sim(
    span<const Vector3> previous_positions,
    ...
    SizeT vertex_count)
{
    for(auto system : systems())
    {
        if(system->name() == "uipc::backend::cuda::GlobalVertexManager")
            return system->write_vertex_pos_pair_to_sim(
                previous_positions, current_positions, global_vertex_offset, vertex_count);
    }
    spdlog::error("Global vertex pose-pair write requested without GlobalVertexManager");
    return false;
}

bool SimEngine::do_write_kinematic_abd_pose_pair_to_sim(IndexT body_id,
                                                        ...
                                                        Float dt)
{
    for(auto system : systems())
    {
        if(system->name() == "uipc::backend::cuda::AffineBodyDynamics")
            return system->write_kinematic_abd_pose_pair_to_sim(
                body_id, previous_transform, current_transform, dt);
    }
    spdlog::error("Kinematic ABD pose-pair write requested without AffineBodyDynamics");
    return false;
}

vector<Vector12> SimEngine::do_read_kinematic_abd_state_from_sim(IndexT body_id)
{
    for(auto system : systems())
        if(system->name() == "uipc::backend::cuda::AffineBodyDynamics")
            return system->read_kinematic_abd_state_from_sim(body_id);
    return {};
}
}  // namespace uipc::backend
