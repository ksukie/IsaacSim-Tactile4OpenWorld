...
TACTILE_NORMAL_STIFFNESS = 1000.0
TACTILE_KINETIC_FRICTION_MU = 0.8
TACTILE_FRICTION_REGULARIZATION_SPEED = 0.003
TACTILE_SLIP_SPEED_DEADBAND = 0.005
TACTILE_FRICTION_SMOOTHING_ALPHA = 0.25
...
class CylinderContainerSceneEnv(DirectRLEnv):
    ...
    def _compute_sdf_tactile_force_field(self):
        ...
        if not hasattr(self, "_tactile_friction_model_logged"):
            self._tactile_friction_model_logged = True
            print(
                "[INFO] Tactile Fx/Fy model -> regularized Coulomb sliding friction "
                f"(mu={TACTILE_KINETIC_FRICTION_MU:.3f}, "
                f"v_smooth={TACTILE_FRICTION_REGULARIZATION_SPEED:.4f} m/s, "
                f"deadband={TACTILE_SLIP_SPEED_DEADBAND:.4f} m/s, "
                f"alpha={TACTILE_FRICTION_SMOOTHING_ALPHA:.2f}); "
                "Fz remains SDF normal force."
            )
        ...
        for openworldtactile_sensor in [self.openworldtactile_left]:
            ...
            normal_force = TACTILE_NORMAL_STIFFNESS * penetration_depth
            contact_mask = penetration_depth > 0.0
            ...
            tactile_link_pos_w = self._robot.data.body_link_pos_w[:, self._tactile_body_idx]
            tactile_link_lin_vel_w = self._robot.data.body_link_lin_vel_w[:, self._tactile_body_idx]
            tactile_link_ang_vel_w = self._robot.data.body_link_ang_vel_w[:, self._tactile_body_idx]
            tactile_link_lin_vel_grid = tactile_link_lin_vel_w[:, None, None, :].expand(
                self.num_envs, height, width, 3
            )
            tactile_link_ang_vel_grid = tactile_link_ang_vel_w[:, None, None, :].expand(
                self.num_envs, height, width, 3
            )
            tactile_link_pos_grid = tactile_link_pos_w[:, None, None, :].expand(self.num_envs, height, width, 3)
            pad_point_vel_w = tactile_link_lin_vel_grid + torch.linalg.cross(
                tactile_link_ang_vel_grid, surface_points_w - tactile_link_pos_grid, dim=-1
            )

            cylinder_lin_vel_w = self.cylinder.data.root_link_lin_vel_w[:, None, None, :].expand(
                self.num_envs, height, width, 3
            )
            cylinder_ang_vel_w = self.cylinder.data.root_link_ang_vel_w[:, None, None, :].expand(
                self.num_envs, height, width, 3
            )
            object_point_vel_w = cylinder_lin_vel_w + torch.linalg.cross(
                cylinder_ang_vel_w, contact_points_w - cylinder_center_w, dim=-1
            )

            relative_vel_w = object_point_vel_w - pad_point_vel_w
            normal_vel_w = torch.sum(relative_vel_w * grad_w, dim=-1, keepdim=True) * grad_w
            slip_vel_w = relative_vel_w - normal_vel_w
            slip_speed = torch.linalg.norm(slip_vel_w, dim=-1, keepdim=True)
            friction_scale = (
                TACTILE_KINETIC_FRICTION_MU
                * normal_force.unsqueeze(-1)
                / torch.sqrt(slip_speed**2 + TACTILE_FRICTION_REGULARIZATION_SPEED**2)
            )
            friction_w = friction_scale * slip_vel_w
            sliding_mask = contact_mask.unsqueeze(-1) & (slip_speed >= TACTILE_SLIP_SPEED_DEADBAND)
            friction_w = torch.where(sliding_mask, friction_w, torch.zeros_like(friction_w))
            friction_w = torch.nan_to_num(friction_w, nan=0.0, posinf=0.0, neginf=0.0)
            normal_force = torch.nan_to_num(normal_force, nan=0.0, posinf=0.0, neginf=0.0)

            friction_local = math_utils.quat_apply_inverse(sensor_quat_grid, friction_w)
            raw_fxy = torch.stack((-friction_local[..., 1], -friction_local[..., 2]), dim=-1)
            raw_fxy = torch.nan_to_num(raw_fxy, nan=0.0, posinf=0.0, neginf=0.0)

            if not hasattr(self, "_tactile_fxy_smooth_cache"):
                self._tactile_fxy_smooth_cache = {}
            cache_key = id(openworldtactile_sensor)
            previous_fxy = self._tactile_fxy_smooth_cache.get(cache_key)
            if previous_fxy is None or previous_fxy.shape != raw_fxy.shape:
                smoothed_fxy = raw_fxy
            else:
                alpha = TACTILE_FRICTION_SMOOTHING_ALPHA
                smoothed_fxy = previous_fxy + alpha * (raw_fxy - previous_fxy)
            smoothed_fxy = torch.where(sliding_mask, smoothed_fxy, torch.zeros_like(smoothed_fxy))
            smoothed_fxy = torch.nan_to_num(smoothed_fxy, nan=0.0, posinf=0.0, neginf=0.0)
            self._tactile_fxy_smooth_cache[cache_key] = smoothed_fxy.detach().clone()

            force_field[..., 0] = smoothed_fxy[..., 0]
            force_field[..., 1] = smoothed_fxy[..., 1]
...
class HDF5EpisodeRecorder:
    ...
    def close_episode(self, env: CylinderContainerSceneEnv | None = None):
        ...
        with h5py.File(episode_path, "w") as h5:
            ...
            h5.attrs["tactile_tangential_force_model"] = "deadbanded_smoothed_regularized_coulomb_sliding_friction"
            h5.attrs["tactile_friction_mu"] = float(TACTILE_KINETIC_FRICTION_MU)
            h5.attrs["tactile_friction_regularization_speed"] = float(TACTILE_FRICTION_REGULARIZATION_SPEED)
            h5.attrs["tactile_slip_speed_deadband"] = float(TACTILE_SLIP_SPEED_DEADBAND)
            h5.attrs["tactile_friction_smoothing_alpha"] = float(TACTILE_FRICTION_SMOOTHING_ALPHA)
            h5.attrs["tactile_fz_meaning"] = "sdf_normal_force"
            h5.attrs["tactile_fx_fy_meaning"] = "deadbanded_smoothed_sliding_friction_in_tactile_image_axes"
