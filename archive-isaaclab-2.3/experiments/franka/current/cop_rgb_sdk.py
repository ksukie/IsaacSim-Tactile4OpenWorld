...
"""
Run Franka IK-Abs cube lift and preview ContactSensor-only COP OpenWorldTactile RGB.
...
* the map is rendered to textured OpenWorldTactile-style RGB: normal pressure drives hue,
  and local tangential contact force drives texture displacement for SDK fx/fy.

It does not use raycast or Gaussian pressure patches.

.. code-block:: bash

    ./isaaclab.sh -p experiments/franka/current/cop_rgb_sdk.py
...
parser = argparse.ArgumentParser(description="Pick/lift DexCube with Franka and preview COP textured OpenWorldTactile RGB maps.")
parser.add_argument(
...
parser.add_argument(
    "--output_dir",
    type=str,
    default="outputs/franka_lift_object_contact_cop_rgb",
...
parser.add_argument(
    ...
    type=float,
    default=0.45,
    help="Pixels of OpenWorldTactile texture displacement per local tangential newton.",
)
parser.add_argument(
...
parser.add_argument(
    ...
    action="store_true",
    default=False,
    help="Show a live Isaac UI preview of the left and right textured OpenWorldTactile RGB maps while the simulation runs.",
)
parser.add_argument(
    "--show_force_maps",
    action="store_true",
...
def contact_cop_force_map(
    ...
    """Reconstruct dense normal force and tangential texture-displacement maps."""
    rows, cols = pad_image_shape()
    ...
    contact_force_b = quat_apply_inverse(body_quat_w, contact_force_w)
    ...
    local_force_total = torch.sum(contact_force_b, dim=0)
    shear_x_n = float(local_force_total[0].item())
    shear_z_n = float(local_force_total[2].item())
    ...
    # Image y grows downward.  A positive local z shear is rendered as upward texture motion.
...
def render_openworldtactile_rgb(pad_maps: ContactPadMaps, renderer: OpenWorldTactileRGBRenderer) -> np.ndarray:
    """Render one virtual pad with OpenWorldTactile pressure color and shear-driven texture displacement."""
