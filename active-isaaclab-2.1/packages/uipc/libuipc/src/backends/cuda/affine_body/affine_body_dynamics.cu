...
bool AffineBodyDynamics::do_write_kinematic_abd_pose_pair_to_sim(
    IndexT body_id,
    const Matrix4x4& previous_transform,
    const Matrix4x4& current_transform,
    Float dt)
{
    return m_impl.write_kinematic_abd_pose_pair_to_sim(
        body_id, previous_transform, current_transform, dt);
}

vector<Vector12> AffineBodyDynamics::do_read_kinematic_abd_state_from_sim(IndexT body_id)
{
    return m_impl.read_kinematic_abd_state_from_sim(body_id);
}
...
{
    // `vertex_offset` is the ABD body id. Recover the affine transform that
    // maps this body's rest vertices to the requested world-space positions.
    // This path is used by a kinematic mirror; it does not modify any FEM data.
    if(vertex_offset < 0 || static_cast<SizeT>(vertex_offset) >= abd_body_count)
        return false;

    vector<SizeT> body_vertices;
    body_vertices.reserve(vertex_count);
    for(SizeT vertexI = 0; vertexI < h_vertex_id_to_body_id.size(); ++vertexI)
        if(h_vertex_id_to_body_id[vertexI] == vertex_offset)
            body_vertices.push_back(vertexI);

    if(body_vertices.size() != vertex_count || positions.size() != vertex_count
       || vertex_count < 4)
        return false;

    MatrixX design(vertex_count, 4);
    MatrixX targets(vertex_count, 3);
    for(SizeT localI = 0; localI < vertex_count; ++localI)
    {
        const Vector3& x_bar = h_vertex_id_to_J[body_vertices[localI]].x_bar();
        design(static_cast<Eigen::Index>(localI), 0) = 1.0;
        design.block<1, 3>(static_cast<Eigen::Index>(localI), 1) = x_bar.transpose();
        targets.row(static_cast<Eigen::Index>(localI)) = positions[localI].transpose();
    }

    const MatrixX coefficients = design.colPivHouseholderQr().solve(targets);
    if(coefficients.rows() != 4 || coefficients.cols() != 3
       || !coefficients.allFinite())
        return false;

    Matrix4x4 transform = Matrix4x4::Identity();
    transform.block<3, 1>(0, 3) = coefficients.row(0).transpose();
    transform.block<3, 3>(0, 0) = coefficients.block<3, 3>(1, 0).transpose();
    if(transform.block<3, 3>(0, 0).determinant() <= 0.0)
        return false;

    vector<Vector12> q = {transform_to_q(transform)};
    vector<Vector12> zero = {Vector12::Zero()};
    body_id_to_q.view(vertex_offset, 1).copy_from(span{q}.data());
    body_id_to_q_prev.view(vertex_offset, 1).copy_from(span{q}.data());
    body_id_to_q_temp.view(vertex_offset, 1).copy_from(span{q}.data());
    body_id_to_q_tilde.view(vertex_offset, 1).copy_from(span{q}.data());
    body_id_to_q_v.view(vertex_offset, 1).copy_from(span{zero}.data());
    body_id_to_dq.view(vertex_offset, 1).copy_from(span{zero}.data());
    muda::wait_stream(nullptr);

    h_body_id_to_q[vertex_offset] = q.front();
    h_body_id_to_q_v[vertex_offset] = Vector12::Zero();
    return true;
}

namespace
{
bool is_rigid_transform(const Matrix4x4& transform)
{
    if(!transform.allFinite())
        return false;
    const Vector4 expected_last_row{0.0, 0.0, 0.0, 1.0};
    if(!transform.row(3).transpose().isApprox(expected_last_row, 1.0e-9))
        return false;
    const Matrix3x3 rotation = transform.block<3, 3>(0, 0);
    return rotation.transpose().isApprox(rotation.inverse(), 1.0e-8)
           && std::abs(rotation.determinant() - 1.0) <= 1.0e-8;
}
}  // namespace

bool AffineBodyDynamics::Impl::write_kinematic_abd_pose_pair_to_sim(
    IndexT body_id,
    const Matrix4x4& previous_transform,
    const Matrix4x4& current_transform,
    Float dt)
{
    if(body_id < 0 || static_cast<SizeT>(body_id) >= abd_body_count)
        return false;
    if(!std::isfinite(dt) || dt <= 0.0)
        return false;
    if(!is_rigid_transform(previous_transform) || !is_rigid_transform(current_transform))
        return false;
    if(h_body_id_to_is_fixed[body_id] == 0)
        return false;

    const Vector12 q_prev = transform_to_q(previous_transform);
    const Vector12 q = transform_to_q(current_transform);
    const Vector12 dq = q - q_prev;
    const Vector12 q_v = dq / dt;
    vector<Vector12> previous = {q_prev};
    vector<Vector12> current = {q};
    vector<Vector12> delta = {dq};
    vector<Vector12> velocity = {q_v};
    body_id_to_q_prev.view(body_id, 1).copy_from(span{previous}.data());
    body_id_to_q.view(body_id, 1).copy_from(span{current}.data());
    body_id_to_q_temp.view(body_id, 1).copy_from(span{current}.data());
    body_id_to_q_tilde.view(body_id, 1).copy_from(span{current}.data());
    body_id_to_dq.view(body_id, 1).copy_from(span{delta}.data());
    body_id_to_q_v.view(body_id, 1).copy_from(span{velocity}.data());
    muda::wait_stream(nullptr);

    h_body_id_to_q[body_id] = q;
    h_body_id_to_q_v[body_id] = q_v;
    return true;
}

vector<Vector12> AffineBodyDynamics::Impl::read_kinematic_abd_state_from_sim(IndexT body_id)
{
    if(body_id < 0 || static_cast<SizeT>(body_id) >= abd_body_count)
        return {};
    vector<Vector12> state(6);
    body_id_to_q_prev.view(body_id, 1).copy_to(state.data() + 0);
    body_id_to_q.view(body_id, 1).copy_to(state.data() + 1);
    body_id_to_q_temp.view(body_id, 1).copy_to(state.data() + 2);
    body_id_to_q_tilde.view(body_id, 1).copy_to(state.data() + 3);
    body_id_to_dq.view(body_id, 1).copy_to(state.data() + 4);
    body_id_to_q_v.view(body_id, 1).copy_to(state.data() + 5);
    muda::wait_stream(nullptr);
    return state;
}
