...
# 中文说明：原始 GelSight/OpenWorldTactile 渲染对照 demo，会绕过 OpenWorldTactile RGB 渲染器，用于比较改造前后的触觉图像输出。
...
.. code-block:: bash
    ...
    # Usage
    python experiments/sensors/openworldtactile_gelsight_original_sensor.py \
        --use_tactile_rgb \
        --use_tactile_ff \
...
parser.add_argument(
    "--save_viz_dir",
    type=str,
    default="tactile_record_gelsight_original",
...
import isaaclab_contrib.sensors.openworldtactile_sensor.visuotactile_sensor as visuotactile_sensor_module
from isaaclab_contrib.sensors.openworldtactile_sensor.visuotactile_render import GelsightRender, compute_tactile_shear_image
from isaaclab_contrib.sensors.openworldtactile_sensor.visuotactile_sensor_data import VisuoTactileSensorData

from isaaclab_assets.sensors import GELSIGHT_R15_CFG


class OriginalGelsightRender(GelsightRender):
    def render(self, height_map: torch.Tensor, displacement: torch.Tensor | None = None) -> torch.Tensor:
        return super().render(height_map)


visuotactile_sensor_module.OpenWorldTactileTactileRGBRender = OriginalGelsightRender
