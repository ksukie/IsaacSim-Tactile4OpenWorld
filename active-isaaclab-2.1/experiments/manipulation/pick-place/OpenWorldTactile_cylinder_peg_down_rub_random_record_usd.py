...
class _ForceCurveHistory:
    ...
    def _force_totals(openworldtactile_sensor) -> np.ndarray:
        ...
        return np.sum(force_field, axis=(0, 1), dtype=np.float64).astype(np.float32)
    ...
    def draw(self) -> np.ndarray:
        ...
        for channel_idx, (name, color) in enumerate(zip(names, colors)):
            ...
            cv2.line(frame, (left, center_y), (self.width - right, center_y), (80, 80, 80), 1)
            ...
            usable_half_h = max(1.0, panel_h * 0.42)
            y_values = center_y - (series.astype(np.float32) / scale) * usable_half_h
