...
# 最简单运行:
#   ./isaaclab.sh -p experiments/rgb-pipeline/dual_openworldtactile_physical_lift_cube_rgb_fxyz_ui.py

# 中文说明：这个脚本演示两个竖直相对的 OpenWorldTactile/GelSight 触觉传感器夹住一个 mesh cube 后上提；默认 cube 固定在初始中心不动，只有左右传感器/夹爪移动。加 --allow_cube_physical_motion 后，cube 才会改为动态物体并由接触和摩擦决定是否被带起。UI 实时展示 GelSight RGB、OpenWorldTactile RGB、GelSight/OpenWorldTactile force field，以及 OpenWorldTactile RGB 输入 SDK 解算出的 FXYZ。
...
parser = argparse.ArgumentParser(
    description="Standalone dual OpenWorldTactile physical cube clamp-and-lift demo with GelSight/OpenWorldTactile RGB and FXYZ UI."
)
...
parser.add_argument(
    ...
    action="store_true",
    help="Let contact/friction move the cube. By default the cube is pinned while only the pads move.",
)
parser.add_argument(
    "--cube_root_rpy",
    type=float,
...
parser.add_argument(
    ...
    action="store_true",
    help="Enable gravity on the cube when --allow_cube_physical_motion is used.",
)
parser.add_argument(
...
TACTILE_ROWS = 20
TACTILE_COLS = 25
WINDOW_TITLE = "Dual OpenWorldTactile Physical Cube Lift GelSight/OpenWorldTactile RGB + FXYZ"
...
@configclass
class DualOpenWorldTactileCubeLiftSceneCfg(InteractiveSceneCfg):
    """Scene with two vertical OpenWorldTactile pads and one mesh cube between them."""
    ...
    contact_object = RigidObjectCfg(
        ...
        spawn=sim_utils.MeshCuboidCfg(
            ...
            rigid_props=sim_utils.RigidBodyPropertiesCfg(
                kinematic_enabled=not args_cli.allow_cube_physical_motion,
                disable_gravity=True if not args_cli.allow_cube_physical_motion else not args_cli.enable_object_gravity,
                solver_position_iteration_count=12,
                solver_velocity_iteration_count=1,
...
def reset_object_to_center(scene: InteractiveScene):
    """Reset the cube to the middle of the clamp."""
...
def pin_object_to_center(scene: InteractiveScene):
    """Keep the cube fixed at its initial center pose with zero velocity."""
    root_state = scene["contact_object"].data.default_root_state.clone()
    root_state[:, :3] += scene.env_origins
    root_state[:, 7:] = 0.0
    scene["contact_object"].write_root_state_to_sim(root_state)
...
def set_pad_pose(scene: InteractiveScene, half_gap: float, lift_offset: float):
    """Move both tactile pads without commanding the cube lift trajectory."""
    # 方式 A 的核心约束：这里只写左右 OpenWorldTactile 的 root pose。
    # 默认展示会把 cube 固定在中心；加 --allow_cube_physical_motion 后，cube 才能被接触/摩擦带动。
    device = scene.device
...
def clamp_phase(frame_count: int) -> tuple[float, float, str]:
    ...
    # 返回的 lift_offset 只作用于左右传感器；cube 不会被代码绑定跟随。
    ...
    if alpha < 1.0:
        # 物理上提阶段：两边传感器保持夹紧并向上移动。
        # 默认 cube 被 pin 在中心不动；只有加 --allow_cube_physical_motion 后才会被接触/摩擦带动。
...
def main():
    ...
    scene_cfg = DualOpenWorldTactileCubeLiftSceneCfg(num_envs=args_cli.num_envs, env_spacing=0.25)
    scene = InteractiveScene(scene_cfg)

    configure_cube_sdf_collision()
    sim.reset()
    print("[INFO]: Setup complete. Dual OpenWorldTactile physical cube lift GelSight/OpenWorldTactile RGB + FXYZ UI is live.")
