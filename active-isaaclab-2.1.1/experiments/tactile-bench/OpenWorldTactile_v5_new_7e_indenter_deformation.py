...
parser = argparse.ArgumentParser(
    description=(
        "V5 new 7e static indenter contact-deformation validation. It preserves the 7d back-face attachment "
        "and drives one UIPC indenter through staged normal indentation and unloading. Outputs are "
        "deformation, contact-mask, penetration, and recovery diagnostics only; no force or pressure is estimated."
    )
)
parser.add_argument("--asset_usd", type=str, default=str(DEFAULT_PAD_USD))
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_7e_static_contact")
parser.add_argument("--workspace_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_7e_static_contact_workspace")
...
parser.add_argument("--indentation_levels_mm", type=str, default="0,0.2,0.5")
...
parser.add_argument("--membrane_cells_y", type=int, default=10)
parser.add_argument("--membrane_cells_z", type=int, default=12)
...
def _build_indentation_schedule() -> list[tuple[str, float, bool, int]]:
    levels = _indentation_levels_mm()
    gap_mm = float(args_cli.tool_setup_gap_mm)
    schedule: list[tuple[str, float, bool, int]] = []
    schedule.extend(("pre_contact", -gap_mm, True, -1) for _ in range(int(args_cli.pre_contact_frames)))
    ...
    for level_idx, target in enumerate(levels):
        for step in range(int(args_cli.ramp_frames)):
            ...
            schedule.append((f"load_ramp_{target:g}mm", value, True, level_idx))
        schedule.extend(
            (f"load_hold_{target:g}mm", target, True, level_idx)
    ...
    for level_idx in range(len(levels) - 2, -1, -1):
        ...
        for step in range(int(args_cli.ramp_frames)):
            alpha = _smoothstep01(float(step + 1) / float(max(1, int(args_cli.ramp_frames))))
            value = previous + (target - previous) * alpha
            schedule.append((f"unload_ramp_{target:g}mm", value, False, level_idx))
        schedule.extend(
            (f"unload_hold_{target:g}mm", target, False, level_idx)
            for _ in range(int(args_cli.level_hold_frames))
        )
        previous = target
    ...
    for step in range(int(args_cli.ramp_frames)):
        ...
        schedule.append(("retreat", value, False, -1))
    schedule.extend(("recovery", -gap_mm, False, -1) for _ in range(int(args_cli.recovery_frames)))
    return schedule
...
def main() -> None:
    ...
    metadata = {
        ...
        "architecture": (
            "link8 articulation -> back-face UIPC attachment + animated UIPC indenter contact "
            "-> free front-surface deformation -> unload recovery"
        ),
        ...
        "contact_friction_enabled": False,
    ...
    while simulation_app.is_running() and (bool(args_cli.loop_indentation) or frame_idx < total_frames):
        ...
        phase, commanded_indentation_mm, loading, level_index = indentation_schedule[motion_frame_idx]
        _write_gripper_open(robot, float(args_cli.gripper_opening_mm))
        ...
        tool_target_delta_pad_l = np.asarray(
    ...
    if bool(args_cli.fail_on_verdict_fail) and not bool(verdict["static_contact_deformation_passed"]):
        raise RuntimeError(f"7e static-contact deformation verdict failed: {verdict}")
