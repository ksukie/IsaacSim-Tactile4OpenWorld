...
TACTILE_TAXEL_EPS = 1.0e-9
TACTILE_TAXEL_CONTACT_EPS = 2.0e-5
TACTILE_TAXEL_NORMAL_STIFFNESS = 500.0
TACTILE_TAXEL_NORMAL_DAMPING = 0.15
TACTILE_TAXEL_SHEAR_STIFFNESS = 180.0
TACTILE_TAXEL_SHEAR_DAMPING = 0.015
TACTILE_TAXEL_FRICTION_MU = 0.8
TACTILE_TAXEL_RELEASE_DECAY = 0.0
TACTILE_TAXEL_RELEASE_CLEAR_COMPRESSION = 5.0e-5
TACTILE_TAXEL_RELEASE_CLEAR_SHEAR = 5.0e-5
TACTILE_TAXEL_DISPLAY_FORCE_EPS = 1.0e-3
TACTILE_TAXEL_MAX_SHEAR_DISPLACEMENT = 0.004
TACTILE_TAXEL_DEBUG_INTERVAL_STEPS = 30
TACTILE_TAXEL_DEBUG_ALWAYS_UNTIL_STEP = 600
...
TACTILE_SDF_NEAREST_FALLBACK_SIGMA_RATIO = 0.25
...
def _openworldtactile_gelpad_local_pose_and_scale(sensor_cfg: GelSightMiniCfg):
    ...
    virtual_height = pad_height + _openworldtactile_sdf_contact_band(sensor_cfg)
    return (
        (pad_surface_depth - pad_height + virtual_height / 2.0, 0.0, 0.0),
        (virtual_height, pad_width, pad_length),
    )
...
class CylinderContainerSceneEnv(DirectRLEnv):
    ...
    def __init__(self, cfg: CylinderContainerSceneCfg, render_mode: str | None = None, **kwargs):
        ...
        self._taxel_normal_compression = None
        self._taxel_prev_normal_compression = None
        self._taxel_tangent_displacement = None
        self._taxel_contact_active = None
        self._taxel_state_shape = None
    ...
    def _get_tactile_force_surface_grid_local(self, openworldtactile_sensor) -> torch.Tensor:
        ...
        pad_surface_depth = _openworldtactile_sdf_sampling_surface_depth(openworldtactile_sensor.cfg)
    ...
    def _ensure_taxel_state(self, height: int, width: int, dtype: torch.dtype):
        state_shape = (self.num_envs, height, width)
        if self._taxel_state_shape == state_shape and self._taxel_normal_compression is not None:
            return

        self._taxel_normal_compression = torch.zeros(state_shape, device=self.device, dtype=dtype)
        self._taxel_prev_normal_compression = torch.zeros(state_shape, device=self.device, dtype=dtype)
        self._taxel_tangent_displacement = torch.zeros((*state_shape, 2), device=self.device, dtype=dtype)
        self._taxel_contact_active = torch.zeros(state_shape, device=self.device, dtype=torch.bool)
        self._taxel_state_shape = state_shape

    def _reset_taxel_state(self, env_ids: torch.Tensor | None = None):
        if self._taxel_normal_compression is None:
            return
        if env_ids is None:
            env_ids = torch.arange(self.num_envs, device=self.device, dtype=torch.long)
        elif not isinstance(env_ids, torch.Tensor):
        ...
        self._taxel_normal_compression[env_ids] = 0.0
        self._taxel_prev_normal_compression[env_ids] = 0.0
        self._taxel_tangent_displacement[env_ids] = 0.0
        self._taxel_contact_active[env_ids] = False

    def _maybe_log_taxel_tactile_debug(
        self,
        ...
        if step % TACTILE_TAXEL_DEBUG_INTERVAL_STEPS != 0:
            return

        contact_pixels = int(torch.count_nonzero(contact_active[0]).item())
        max_compression = float(torch.max(normal_compression[0]).item())
        normal_force_sum = float(torch.sum(normal_force[0]).item())
        normal_force_max = float(torch.max(normal_force[0]).item())
        shear_force_sum = float(torch.sum(torch.linalg.norm(shear_force_yz[0], dim=-1)).item())
        shear_force_max = float(torch.max(torch.linalg.norm(shear_force_yz[0], dim=-1)).item())
        min_signed_distance = float(torch.min(signed_distance[0]).item())

        should_print = (
            step <= TACTILE_TAXEL_DEBUG_ALWAYS_UNTIL_STEP
            or contact_pixels > 0
            or normal_force_sum > TACTILE_TAXEL_EPS
            or shear_force_sum > TACTILE_TAXEL_EPS
        )
        if not should_print:
            return
        ...
        print(
            "[INFO] taxel触觉调试 -> "
            f"步数={step}, "
            f"接触taxel数={contact_pixels}, "
            f"最大压缩={max_compression:.6f}, "
            f"Fz总和={normal_force_sum:.3f}, "
            f"Fz峰值={normal_force_max:.3f}, "
            f"切向力总和={shear_force_sum:.3f}, "
            f"切向力峰值={shear_force_max:.3f}, "
            f"圆柱到采样面的最小距离={min_signed_distance:.6f}",
            alignment_text,
            flush=True,
        )
    ...
    def _compute_sdf_tactile_force_field(self):
        ...
        if not hasattr(self, "_tactile_taxel_model_logged"):
            self._tactile_taxel_model_logged = True
            print(
                "[INFO] 触觉力模型 -> taxel 弹簧阵列。"
                "可见 OpenWorldTactile 膜保持不变；不创建硬膜 ContactSensor。"
                "300x300 采样点每个都是一个虚拟弹簧，Fz 来自法向压缩，Fx/Fy 来自切向拖拽状态。",
                flush=True,
            )
        ...
        for openworldtactile_sensor in [self.openworldtactile_left]:
            ...
            self._ensure_taxel_state(height, width, self.cylinder.data.root_link_pos_w.dtype)
            ...
            contact_active = penetration_depth > TACTILE_TAXEL_CONTACT_EPS
            ...
            contact_points_c = surface_points_c - signed_distance.unsqueeze(-1) * grad_c
            contact_points_w = cylinder_center_w + math_utils.quat_apply(cylinder_quat_grid, contact_points_c)

            robot_data = self._robot.data
            tactile_lin_vel_source = getattr(robot_data, "body_link_lin_vel_w", None)
            if tactile_lin_vel_source is None:
                tactile_lin_vel_source = getattr(robot_data, "body_lin_vel_w", None)
            tactile_ang_vel_source = getattr(robot_data, "body_link_ang_vel_w", None)
            if tactile_ang_vel_source is None:
                tactile_ang_vel_source = getattr(robot_data, "body_ang_vel_w", None)

            if tactile_lin_vel_source is None:
                tactile_lin_vel_w = torch.zeros((self.num_envs, 3), device=self.device, dtype=surface_points_w.dtype)
            else:
                tactile_lin_vel_w = tactile_lin_vel_source[:, self._tactile_body_idx].to(dtype=surface_points_w.dtype)
            if tactile_ang_vel_source is None:
                tactile_ang_vel_w = torch.zeros((self.num_envs, 3), device=self.device, dtype=surface_points_w.dtype)
            else:
                tactile_ang_vel_w = tactile_ang_vel_source[:, self._tactile_body_idx].to(dtype=surface_points_w.dtype)

            cyl_lin_vel_w = getattr(self.cylinder.data, "root_link_lin_vel_w", None)
            if cyl_lin_vel_w is None:
                cyl_lin_vel_w = getattr(self.cylinder.data, "root_lin_vel_w", None)
            cyl_ang_vel_w = getattr(self.cylinder.data, "root_link_ang_vel_w", None)
            if cyl_ang_vel_w is None:
                cyl_ang_vel_w = getattr(self.cylinder.data, "root_ang_vel_w", None)
            if cyl_lin_vel_w is None:
                cyl_lin_vel_w = torch.zeros((self.num_envs, 3), device=self.device, dtype=surface_points_w.dtype)
            else:
                cyl_lin_vel_w = cyl_lin_vel_w.to(dtype=surface_points_w.dtype)
            if cyl_ang_vel_w is None:
                cyl_ang_vel_w = torch.zeros((self.num_envs, 3), device=self.device, dtype=surface_points_w.dtype)
            else:
                cyl_ang_vel_w = cyl_ang_vel_w.to(dtype=surface_points_w.dtype)

            tactile_body_pos_w = self._robot.data.body_link_pos_w[:, self._tactile_body_idx].to(dtype=surface_points_w.dtype)
            r_pad_w = surface_points_w - tactile_body_pos_w[:, None, None, :]
            r_obj_w = contact_points_w - self.cylinder.data.root_link_pos_w[:, None, None, :].to(
                dtype=surface_points_w.dtype
            )
            tactile_ang_grid = tactile_ang_vel_w[:, None, None, :].expand(self.num_envs, height, width, 3)
            cyl_ang_grid = cyl_ang_vel_w[:, None, None, :].expand(self.num_envs, height, width, 3)
            v_pad_w = tactile_lin_vel_w[:, None, None, :] + torch.cross(tactile_ang_grid, r_pad_w, dim=-1)
            v_obj_w = cyl_lin_vel_w[:, None, None, :] + torch.cross(cyl_ang_grid, r_obj_w, dim=-1)
            v_rel_w = v_obj_w - v_pad_w
            v_rel_normal = torch.sum(v_rel_w * grad_w, dim=-1, keepdim=True)
            v_slip_w = v_rel_w - v_rel_normal * grad_w
            v_slip_local = math_utils.quat_apply_inverse(sensor_quat_grid, v_slip_w)
            v_slip_yz = torch.stack((v_slip_local[..., 1], v_slip_local[..., 2]), dim=-1)

            prev_compression = self._taxel_normal_compression
            prev_active = self._taxel_contact_active
            normal_compression = torch.where(
                contact_active,
                penetration_depth,
                torch.zeros_like(prev_compression),
            )
            normal_compression = torch.where(
                (~contact_active) & (normal_compression < TACTILE_TAXEL_RELEASE_CLEAR_COMPRESSION),
                torch.zeros_like(normal_compression),
                normal_compression,
            )
            compression_velocity = (normal_compression - prev_compression) / max(
                float(self.physics_dt), 1.0e-6
            )
            normal_force = (
                TACTILE_TAXEL_NORMAL_STIFFNESS * normal_compression
                + TACTILE_TAXEL_NORMAL_DAMPING * torch.clamp(compression_velocity, min=0.0)
            )
            normal_force = torch.clamp(normal_force, min=0.0)

            tangent_displacement = self._taxel_tangent_displacement
            integrated_tangent = tangent_displacement + v_slip_yz * float(self.physics_dt)
            new_contact = contact_active & ~prev_active
            integrated_tangent = torch.where(new_contact[..., None], v_slip_yz * float(self.physics_dt), integrated_tangent)
            tangent_displacement = torch.where(
                contact_active[..., None],
                integrated_tangent,
                torch.zeros_like(tangent_displacement),
            )
            released_tangent_norm = torch.linalg.norm(tangent_displacement, dim=-1, keepdim=True)
            tangent_displacement = torch.where(
                (~contact_active[..., None]) & (released_tangent_norm < TACTILE_TAXEL_RELEASE_CLEAR_SHEAR),
                torch.zeros_like(tangent_displacement),
                tangent_displacement,
            )
            tangent_norm = torch.linalg.norm(tangent_displacement, dim=-1, keepdim=True).clamp_min(1.0e-9)
            tangent_displacement = tangent_displacement * torch.clamp(
                TACTILE_TAXEL_MAX_SHEAR_DISPLACEMENT / tangent_norm,
                max=1.0,
            )
            shear_force_yz = (
                TACTILE_TAXEL_SHEAR_STIFFNESS * tangent_displacement
                + TACTILE_TAXEL_SHEAR_DAMPING * v_slip_yz
            )
            shear_force_yz = torch.where(contact_active[..., None], shear_force_yz, torch.zeros_like(shear_force_yz))
            shear_force_mag = torch.linalg.norm(shear_force_yz, dim=-1, keepdim=True).clamp_min(1.0e-9)
            max_shear = TACTILE_TAXEL_FRICTION_MU * normal_force.unsqueeze(-1)
            shear_force_yz = shear_force_yz * torch.clamp(max_shear / shear_force_mag, max=1.0)

            pad_width = openworldtactile_sensor.cfg.gelpad_dimensions.width
            pad_length = openworldtactile_sensor.cfg.gelpad_dimensions.length
            pad_height = openworldtactile_sensor.cfg.gelpad_dimensions.height
            hard_front_x = _openworldtactile_sdf_sampling_surface_depth(openworldtactile_sensor.cfg)
            hard_back_x = hard_front_x - pad_height
            cylinder_center_local = math_utils.quat_apply_inverse(
                sensor_quat_w,
                self.cylinder.data.root_link_pos_w - sensor_pos_w,
            )
            nearest_flat_idx = torch.argmin(signed_distance[0].reshape(-1))
            nearest_sample_local = surface_grid_local[0].reshape(-1, 3)[nearest_flat_idx]
            center_y_margin = pad_width * 0.5 - torch.abs(cylinder_center_local[0, 1])
            center_z_margin = pad_length * 0.5 - torch.abs(cylinder_center_local[0, 2])
            nearest_y_edge_ratio = torch.abs(nearest_sample_local[1]) / max(pad_width * 0.5, 1.0e-9)
            nearest_z_edge_ratio = torch.abs(nearest_sample_local[2]) / max(pad_length * 0.5, 1.0e-9)
            alignment_text = (
                f" 膜面x={hard_front_x:.5f}, 可见膜厚度范围x=[{hard_back_x:.5f},{hard_front_x:.5f}], "
                f"圆柱中心局部=({float(cylinder_center_local[0, 0].item()):.5f},"
                f"{float(cylinder_center_local[0, 1].item()):.5f},"
                f"{float(cylinder_center_local[0, 2].item()):.5f}), "
                f"中心到硬膜前表面x距离={float((cylinder_center_local[0, 0] - hard_front_x).item()):.5f}, "
                f"中心距膜Y边余量={float(center_y_margin.item()):.5f}, "
                f"中心距膜Z边余量={float(center_z_margin.item()):.5f}, "
                f"最近采样点局部=({float(nearest_sample_local[0].item()):.5f},"
                f"{float(nearest_sample_local[1].item()):.5f},"
                f"{float(nearest_sample_local[2].item()):.5f}), "
                f"最近点边缘比例Y/Z=({float(nearest_y_edge_ratio.item()):.2f},"
                f"{float(nearest_z_edge_ratio.item()):.2f})"
            )

            self._maybe_log_taxel_tactile_debug(
                contact_active,
                normal_compression,
                normal_force,
                shear_force_yz,
            ...
            force_field[..., 0] = -shear_force_yz[..., 0]
            force_field[..., 1] = -shear_force_yz[..., 1]
            force_field[..., 2] = normal_force
            force_field[:] = torch.nan_to_num(force_field, nan=0.0, posinf=0.0, neginf=0.0)

            self._taxel_prev_normal_compression = prev_compression.detach().clone()
            self._taxel_normal_compression = normal_compression.detach().clone()
            self._taxel_tangent_displacement = tangent_displacement.detach().clone()
            self._taxel_contact_active = contact_active.detach().clone()
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        self._reset_taxel_state(env_ids)
...
def _draw_positive_component(component: np.ndarray) -> np.ndarray:
    ...
    if float(np.max(component)) <= TACTILE_TAXEL_DISPLAY_FORCE_EPS:
        return np.zeros((*component.shape, 3), dtype=np.uint8)
    max_value = float(np.percentile(component, 99.0))
    if max_value <= TACTILE_TAXEL_DISPLAY_FORCE_EPS:
...
class HDF5EpisodeRecorder:
    ...
    def close_episode(self, env: CylinderContainerSceneEnv | None = None):
        ...
        with h5py.File(episode_path, "w") as h5:
            ...
            if env is None:
                virtual_gelpad_local_pos = np.full((3,), np.nan, dtype=np.float32)
                virtual_gelpad_size = np.full((3,), np.nan, dtype=np.float32)
                physx_softpad_local_pos = np.full((3,), np.nan, dtype=np.float32)
                physx_softpad_size = np.full((3,), np.nan, dtype=np.float32)
                sdf_contact_band = float("nan")
                sdf_sampling_surface_depth = float("nan")
                hard_softpad_thickness = float("nan")
            else:
                virtual_gelpad_local_pos_tuple, virtual_gelpad_size_tuple = _openworldtactile_gelpad_local_pose_and_scale(
                    env.cfg.openworldtactile_left
                )
                physx_softpad_local_pos_tuple, physx_softpad_size_tuple = _openworldtactile_physx_softpad_local_pose_and_scale(
                    env.cfg.openworldtactile_left
                )
                virtual_gelpad_local_pos = np.asarray(virtual_gelpad_local_pos_tuple, dtype=np.float32)
                virtual_gelpad_size = np.asarray(virtual_gelpad_size_tuple, dtype=np.float32)
                physx_softpad_local_pos = np.asarray(physx_softpad_local_pos_tuple, dtype=np.float32)
                physx_softpad_size = np.asarray(physx_softpad_size_tuple, dtype=np.float32)
                sdf_contact_band = float(_openworldtactile_sdf_contact_band(env.cfg.openworldtactile_left))
                sdf_sampling_surface_depth = float(_openworldtactile_sdf_sampling_surface_depth(env.cfg.openworldtactile_left))
                hard_softpad_thickness = float(env.cfg.openworldtactile_left.gelpad_dimensions.height)
            ...
            h5.attrs["tactile_force_model"] = "taxel弹簧阵列_法向压缩加切向拖拽"
            h5.attrs["tactile_contact_source"] = "圆柱SDF到OpenWorldTactile膜面taxel的解析接触"
            h5.attrs["tactile_taxel_surface_depth_m"] = sdf_sampling_surface_depth
            h5.attrs["tactile_taxel_visible_pad_thickness_m"] = hard_softpad_thickness
            h5.attrs["tactile_taxel_grid_size"] = np.asarray((300, 300), dtype=np.int32)
            h5.attrs["tactile_taxel_normal_stiffness"] = float(TACTILE_TAXEL_NORMAL_STIFFNESS)
            h5.attrs["tactile_taxel_normal_damping"] = float(TACTILE_TAXEL_NORMAL_DAMPING)
            h5.attrs["tactile_taxel_shear_stiffness"] = float(TACTILE_TAXEL_SHEAR_STIFFNESS)
            h5.attrs["tactile_taxel_shear_damping"] = float(TACTILE_TAXEL_SHEAR_DAMPING)
            h5.attrs["tactile_taxel_friction_mu"] = float(TACTILE_TAXEL_FRICTION_MU)
            h5.attrs["tactile_taxel_release_decay"] = float(TACTILE_TAXEL_RELEASE_DECAY)
            h5.attrs["tactile_taxel_release_clear_compression"] = float(TACTILE_TAXEL_RELEASE_CLEAR_COMPRESSION)
            h5.attrs["tactile_taxel_release_clear_shear"] = float(TACTILE_TAXEL_RELEASE_CLEAR_SHEAR)
            h5.attrs["tactile_taxel_max_shear_displacement"] = float(TACTILE_TAXEL_MAX_SHEAR_DISPLACEMENT)
            h5.attrs["tactile_taxel_contact_eps"] = float(TACTILE_TAXEL_CONTACT_EPS)
            h5.attrs["tactile_fx_fy_meaning"] = "taxel切向弹簧拖拽力_触觉图平面方向"
            h5.attrs["tactile_fz_meaning"] = "taxel法向弹簧压缩力"
