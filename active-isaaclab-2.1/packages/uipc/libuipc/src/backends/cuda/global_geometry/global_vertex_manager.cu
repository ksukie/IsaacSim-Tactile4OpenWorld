...
bool GlobalVertexManager::Impl::write_vertex_pos_pair_to_sim(
    span<const Vector3> previous,
    span<const Vector3> current,
    IndexT global_vertex_offset,
    SizeT vertex_count)
{
    if(global_vertex_offset < 0 || previous.size() != vertex_count
       || current.size() != vertex_count
       || static_cast<SizeT>(global_vertex_offset) + vertex_count > positions.size())
        return false;

    prev_positions.view(global_vertex_offset, vertex_count).copy_from(previous.data());
    positions.view(global_vertex_offset, vertex_count).copy_from(current.data());
    return true;
}
...
bool GlobalVertexManager::do_write_vertex_pos_pair_to_sim(
    span<const Vector3> previous_positions,
    span<const Vector3> current_positions,
    IndexT global_vertex_offset,
    SizeT vertex_count)
{
    return m_impl.write_vertex_pos_pair_to_sim(
