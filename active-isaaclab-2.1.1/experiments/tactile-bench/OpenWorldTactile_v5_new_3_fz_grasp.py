...
parser = argparse.ArgumentParser(
    description=(
        "V5 new stage 3: PhysX grasp plus UIPC Fz verification with UIPC_Pad mounted as a stable "
        "link7 child asset. This verifies mount stability, smooth gripper motion, "
        "dynamic cylinder grasp/lift, and a Fz-only UIPC deformation proxy. It intentionally "
        "does not create RGB tactile output, Fx/Fy output, or a NutTool."
    )
)
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_3_uipc_fz_grasp")
parser.add_argument("--workspace_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_3_workspace")
...
def main() -> None:
    ...
    print(
        json.dumps(
            {
                "script_version": "v5_new_3_uipc_fz_grasp",
    ...
    try:
        ...
        for phase in phases:
            ...
            for phase_frame in range(frame_count):
                ...
                sum_fz = float(np.sum(local_fxyz[:, 2]))
                max_fz = float(np.max(local_fxyz[:, 2])) if local_fxyz.shape[0] else 0.0
                max_indent_mm = float(np.max(indent)) * 1000.0 if indent.shape[0] else 0.0
                ...
                if total_frames % max(1, int(args_cli.log_every)) == 0:
                    ...
                    print(
                        ...
                        f"sum_fz={sum_fz:.6e} "
                        f"max_fz={max_fz:.6e} "
                        f"max_indent_mm={max_indent_mm:.6f} "
        ...
        metadata = {
            "script_version": "v5_new_3_uipc_fz_grasp",
            "purpose": "mounted_pad_physx_grasp_with_uipc_fz_only",
            ...
            "force_source": "uipc_deformation_proxy_fz_only",
            ...
            "uipc_vertex_sync": {
                ...
                "tool_sync": "local_vertex_cache_rebuilt_from_live_physx_cylinder_pose_before_uipc_step",
