...
"""
Run Franka IK-Abs nut lift and preview SDF RGB with shear texture flow.
...
* SDF penetration depth changes hue for SDK fz;
* tangential SDF taxel shear displaces the gray texture for SDK fxy;
* pressure hue changes are rendered with near-constant grayscale so they do not dominate optical flow.

It does not fall back to the older COP or friction scripts if an SDF collision mesh is missing.

.. code-block:: bash

    ./isaaclab.sh -p experiments/franka/current/ST_RGB_shear.py
...
parser.add_argument(
    "--output_dir",
    type=str,
    default="outputs/franka_lift_object_ST_RGB_shear",
...
parser.add_argument(
    ...
    type=float,
    default=6.0e-4,
    help="SDF penetration full scale for contact wavelength color rendering.",
)
parser.add_argument(
    "--openworldtactile_displacement_scale",
    type=float,
    default=2000.0,
    help="Reference 320x240 scale applied to SDF shear before gray-texture optical flow displacement.",
)
parser.add_argument(
    "--openworldtactile_displacement_clip_px",
    type=float,
    default=3.0,
    help="Clip rendered texture displacement in pixels; 0 disables clipping.",
)
parser.add_argument(
    "--openworldtactile_iso_saturation",
    type=int,
    default=120,
    help="HSV saturation used for equal-grayscale pressure hue rendering.",
)
parser.add_argument(
    "--wavelength_high_nm",
    type=float,
    default=645.0,
    help="No-contact/low-pressure wavelength in nm; 645 nm gives a visibly red baseline.",
)
parser.add_argument(
    "--wavelength_low_nm",
    type=float,
    default=450.0,
    help="High-pressure wavelength in nm for contact pixels.",
)
...
parser.add_argument(
    "--openworldtactile_flow_scale_factor",
    type=float,
    default=1.0,
    help="Local SDK optical-flow resize factor for this small virtual pad image.",
)
parser.add_argument(
    "--openworldtactile_flow_winsize",
    type=int,
    default=0,
    help="Local SDK Farneback winsize; 0 auto-scales it from the SDK 640x480 camera reference.",
)
parser.add_argument(
    "--openworldtactile_flow_levels",
    type=int,
    default=3,
    help="Local SDK Farneback pyramid levels for this small virtual pad image.",
)
parser.add_argument(
    "--sdf_object_usd",
    type=str,
...
parser.add_argument(
    ...
    action="store_true",
    default=True,
    help="Show a live Isaac UI preview of the left/right contact wavelength RGB and SDK fxyz maps.",
)
parser.add_argument(
    "--hide_rgb_maps",
    action="store_false",
...
parser.add_argument(
    "--openworldtactile_fxyz_arrow_scale",
    type=float,
    default=0.0,
    help="FXYZ preview arrow scale; 0 auto-scales from a 320x240 reference image.",
)
parser.add_argument(
    "--openworldtactile_fxyz_arrow_step",
    type=int,
    default=0,
    help="FXYZ preview arrow sampling step; 0 auto-scales from a 320x240 reference image.",
)
...
def _positive_odd_at_most(value: float, upper: float, minimum: int = 3) -> int:
    """Return a positive odd integer no larger than upper."""
    upper_int = max(minimum, int(round(upper)))
    if upper_int % 2 == 0:
        upper_int -= 1
    upper_int = max(minimum, upper_int)

    odd_value = max(minimum, int(round(value)))
    if odd_value % 2 == 0:
        odd_value += 1
    return min(odd_value, upper_int)


def openworldtactile_flow_effective_winsize() -> int:
    """Return the Farneback winsize adapted to the current RGB image size."""
    if args_cli.openworldtactile_flow_winsize > 0:
        return int(args_cli.openworldtactile_flow_winsize)

    rows, cols = pad_image_shape()
    current_min = min(rows, cols) * float(args_cli.openworldtactile_flow_scale_factor)
    # SDK defaults target a 640x480 camera frame resized by 0.5 before optical flow.
    reference_min = 480.0 * 0.5
    reference_winsize = 61.0
    scaled = reference_winsize * current_min / reference_min
    return _positive_odd_at_most(scaled, current_min)


def openworldtactile_resolution_inverse_scale() -> float:
    """Return the inverse resolution scale relative to a 320x240 OpenWorldTactile reference."""
    rows, cols = pad_image_shape()
    reference_min = 240.0
    return reference_min / max(1.0, float(min(rows, cols)))


def openworldtactile_displacement_effective_scale() -> float:
    """Return shear-to-pixel scale adapted to the current tactile image resolution."""
    return float(args_cli.openworldtactile_displacement_scale) * openworldtactile_resolution_inverse_scale()


def openworldtactile_displacement_effective_clip_px() -> float:
    """Return displacement clip adapted to the current tactile image resolution."""
    clip_px = float(args_cli.openworldtactile_displacement_clip_px)
    if clip_px <= 0.0:
        return 0.0
    return clip_px * min(2.0, openworldtactile_resolution_inverse_scale())


def openworldtactile_fxyz_arrow_scale() -> float:
    """Return the preview arrow length scale adapted to the current RGB image size."""
    if args_cli.openworldtactile_fxyz_arrow_scale > 0.0:
        return float(args_cli.openworldtactile_fxyz_arrow_scale)
    return max(8.0, 8.0 * openworldtactile_resolution_inverse_scale())


def openworldtactile_fxyz_arrow_step() -> int:
    """Return the preview arrow sampling step adapted to the current RGB image size."""
    if args_cli.openworldtactile_fxyz_arrow_step > 0:
        return int(args_cli.openworldtactile_fxyz_arrow_step)
    rows, cols = pad_image_shape()
    return max(4, int(round(25.0 * min(rows, cols) / 240.0)))
...
class SdfTaxelPadSampler:
    ...
    def compute_one(self, finger_body_id: int, points_b: torch.Tensor, env_id: int) -> ContactPadMaps:
        """Compute one finger's SDF penetration, normal-force, and shear maps."""
        ...
        image_displacement = torch.stack((env_shear[:, 0], -env_shear[:, 2]), dim=-1)

        height_map = depth[env_id].reshape(self.rows, self.cols).detach().cpu().numpy().astype(np.float32)
        force_map = env_normal.reshape(self.rows, self.cols).detach().cpu().numpy().astype(np.float32)
        displacement_map = (
            image_displacement.reshape(self.rows, self.cols, 2).detach().cpu().numpy().astype(np.float32)
        )
        contact_count = int(torch.count_nonzero(depth[env_id] > 0.0).item())
        total_force = float(torch.sum(env_normal).item())
        return ContactPadMaps(height_map, force_map, displacement_map, total_force, contact_count, None)
...
def wavelength_to_hsv_hue(wavelength_nm: np.ndarray) -> np.ndarray:
    """Return OpenCV HSV hue values for visible-light wavelength values."""
    rgb = wavelength_to_rgb(wavelength_nm)
    return cv2.cvtColor(rgb, cv2.COLOR_RGB2HSV)[..., 0]


def height_map_rgb_scale(height_map: np.ndarray) -> float:
    """Return the SDF penetration scale used by the contact wavelength renderer."""
    return max(float(args_cli.openworldtactile_max_pressure), 1.0e-9)


def force_map_to_gray_image(height_map: np.ndarray) -> np.ndarray:
    """Map one height/penetration map to an 8-bit grayscale image using the contact pressure scale."""
    scale = height_map_rgb_scale(height_map)
    height = np.nan_to_num(height_map, nan=0.0, posinf=scale, neginf=0.0)
    return (np.clip(np.clip(height, 0.0, None) / scale, 0.0, 1.0) * 255.0).astype(np.uint8)


class SdfContactWavelengthRgbRenderer:
    """Render SDF pressure hue with shear-displaced equal-gray texture."""

    def __init__(self):
        rows, cols = pad_image_shape()
        self.height = rows
        self.width = cols
        self.base_wavelength = np.full((rows, cols), args_cli.wavelength_high_nm, dtype=np.float32)
        self.texture_renderer = OpenWorldTactileRGBRenderer(
            height=rows,
            width=cols,
            ...
            displacement_scale=openworldtactile_displacement_effective_scale(),
        )
        self._iso_gray_cap = self._compute_iso_gray_cap()
        self._iso_gray_base = self._iso_gray_cap * 0.82

    def render_baseline(self) -> np.ndarray:
        """Return a no-contact red wavelength baseline with static gray texture."""
        return self._render_iso_gray_rgb(self.base_wavelength, self._warped_gray_texture(None))

    def render(self, height_map: np.ndarray | None = None, displacement: np.ndarray | None = None) -> np.ndarray:
        """Render pressure as hue and shear as gray-texture displacement."""
        pressure = self._contact_pressure(height_map)
        wavelength = args_cli.wavelength_high_nm - pressure * (
            args_cli.wavelength_high_nm - args_cli.wavelength_low_nm
        )
        return self._render_iso_gray_rgb(wavelength, self._warped_gray_texture(displacement))

    def _contact_pressure(self, height_map: np.ndarray | None) -> np.ndarray:
        ...
        if height.shape != (self.height, self.width):
            height = cv2.resize(height, (self.width, self.height), interpolation=cv2.INTER_LINEAR)
        scale = height_map_rgb_scale(height)
        height = np.nan_to_num(height, nan=0.0, posinf=scale, neginf=0.0)
        pressure = np.clip(np.clip(height, 0.0, None) / scale, 0.0, 1.0).astype(np.float32)
        kernel = self.texture_renderer._odd_kernel(args_cli.openworldtactile_pressure_blur)
        if kernel > 1:
            pressure = cv2.GaussianBlur(pressure, (kernel, kernel), 0)
        return np.clip(pressure, 0.0, 1.0).astype(np.float32)

    def _warped_gray_texture(self, displacement: np.ndarray | None) -> np.ndarray:
        dx, dy = self.texture_renderer._prepare_displacement(
            displacement,
            displacement_x=None,
            displacement_y=None,
        )
        clip_px = openworldtactile_displacement_effective_clip_px()
        if clip_px > 0.0:
            dx = np.clip(dx, -clip_px, clip_px)
            dy = np.clip(dy, -clip_px, clip_px)
        value = self.texture_renderer._warp_texture(dx, dy).astype(np.float32)
        reference = max(float(self.texture_renderer.base_value), 1.0)
        target_gray = value * (self._iso_gray_base / reference)
        return np.clip(target_gray, 1.0, self._iso_gray_cap).astype(np.float32)

    def _render_iso_gray_rgb(self, wavelength: np.ndarray, target_gray: np.ndarray) -> np.ndarray:
        hue = wavelength_to_hsv_hue(wavelength).astype(np.uint8)
        saturation = np.full((self.height, self.width), args_cli.openworldtactile_iso_saturation, dtype=np.uint8)

        hsv_max = np.empty((self.height, self.width, 3), dtype=np.uint8)
        hsv_max[..., 0] = hue
        hsv_max[..., 1] = saturation
        hsv_max[..., 2] = 255
        rgb_max = cv2.cvtColor(hsv_max, cv2.COLOR_HSV2RGB).astype(np.float32)
        gray_max = self._rgb_gray(rgb_max)
        value = np.clip(np.asarray(target_gray, dtype=np.float32) * 255.0 / np.maximum(gray_max, 1.0), 0.0, 255.0)

        hsv = hsv_max.copy()
        hsv[..., 2] = np.rint(value).astype(np.uint8)
        return cv2.cvtColor(hsv, cv2.COLOR_HSV2RGB)

    def _compute_iso_gray_cap(self) -> float:
        hues = np.arange(180, dtype=np.uint8).reshape(1, 180)
        hsv = np.empty((1, 180, 3), dtype=np.uint8)
        hsv[..., 0] = hues
        hsv[..., 1] = np.uint8(args_cli.openworldtactile_iso_saturation)
        hsv[..., 2] = 255
        rgb = cv2.cvtColor(hsv, cv2.COLOR_HSV2RGB).astype(np.float32)
        return max(float(np.min(self._rgb_gray(rgb))), 1.0)

    @staticmethod
    def _rgb_gray(rgb: np.ndarray) -> np.ndarray:
        return 0.299 * rgb[..., 0] + 0.587 * rgb[..., 1] + 0.114 * rgb[..., 2]


def make_openworldtactile_renderer() -> OpenWorldTactileRGBRenderer:
    """Create one SDK-native textured OpenWorldTactile renderer for the virtual pad grid."""
    rows, cols = pad_image_shape()
...
def render_openworldtactile_rgb(pad_maps: ContactPadMaps, renderer: OpenWorldTactileRGBRenderer) -> np.ndarray:
    """Render one virtual pad from SDF penetration and shear maps."""
    return renderer.render(pad_maps.height_map, displacement=pad_maps.displacement)
...
def make_openworldtactile_bridge(renderer: OpenWorldTactileRGBRenderer | None = None) -> IsaacLabOpenWorldTactileBridge:
    ...
    bridge.sensor.scale_factor = float(args_cli.openworldtactile_flow_scale_factor)
    bridge.sensor.opflow_params["winsize"] = openworldtactile_flow_effective_winsize()
    bridge.sensor.opflow_params["levels"] = int(args_cli.openworldtactile_flow_levels)
    return bridge
...
def openworldtactile_fxyz_visual_image(
    ...
    if bridge.calibrated:
        ...
        if fx_matrix is not None and fy_matrix is not None:
            ...
            arrow_step = openworldtactile_fxyz_arrow_step()
            arrow_scale = openworldtactile_fxyz_arrow_scale()
            for y in range(0, rows, arrow_step):
                for x in range(0, cols, arrow_step):
                    end_x = int(np.clip(round(x + float(fx[y, x]) * arrow_scale), 0, cols - 1))
                    end_y = int(np.clip(round(y + float(fy[y, x]) * arrow_scale), 0, rows - 1))
...
class ForceRgbPreviewWindow:
    ...
    def __init__(
        ...
        zero_rgb = np.zeros((rows, cols, 3), dtype=np.uint8)
...
def maybe_save_rgb_pair(
    ...
    left_gray = force_map_to_gray_image(left_result.height_map)
    right_gray = force_map_to_gray_image(right_result.height_map)
    ...
    print(
        ...
        f"left_height_scale={height_map_rgb_scale(left_result.height_map):.6e}m "
        ...
        f"right_height_scale={height_map_rgb_scale(right_result.height_map):.6e}m "
...
def main():
    ...
    if args_cli.openworldtactile_displacement_scale < 0.0:
        raise ValueError("--openworldtactile_displacement_scale must be non-negative.")
    if args_cli.openworldtactile_displacement_clip_px < 0.0:
        raise ValueError("--openworldtactile_displacement_clip_px must be non-negative.")
    if not 1 <= args_cli.openworldtactile_iso_saturation <= 255:
        raise ValueError("--openworldtactile_iso_saturation must be in [1, 255].")
    ...
    if args_cli.wavelength_high_nm <= args_cli.wavelength_low_nm:
        raise ValueError("--wavelength_high_nm must be greater than --wavelength_low_nm.")
    ...
    if args_cli.openworldtactile_flow_scale_factor <= 0.0:
        raise ValueError("--openworldtactile_flow_scale_factor must be positive.")
    if args_cli.openworldtactile_flow_winsize < 0 or (
        args_cli.openworldtactile_flow_winsize > 0 and args_cli.openworldtactile_flow_winsize % 2 == 0
    ):
        raise ValueError("--openworldtactile_flow_winsize must be 0 for auto or a positive odd integer.")
    if args_cli.openworldtactile_flow_levels <= 0:
        raise ValueError("--openworldtactile_flow_levels must be positive.")
    ...
    if args_cli.openworldtactile_fxyz_arrow_scale < 0.0:
        raise ValueError("--openworldtactile_fxyz_arrow_scale must be non-negative.")
    if args_cli.openworldtactile_fxyz_arrow_step < 0:
        raise ValueError("--openworldtactile_fxyz_arrow_step must be non-negative.")
    ...
    print(
        "[INFO]: SDF RGB shear renderer "
        f"rgb_size={pad_image_shape()} "
        f"max_pressure={args_cli.openworldtactile_max_pressure} "
        f"wavelength_nm=({args_cli.wavelength_high_nm}, {args_cli.wavelength_low_nm}) "
        f"iso_saturation={args_cli.openworldtactile_iso_saturation} "
        ...
        f"resolution_inverse_scale={openworldtactile_resolution_inverse_scale():.3f} "
        f"displacement_scale_arg={args_cli.openworldtactile_displacement_scale} "
        f"displacement_scale_effective={openworldtactile_displacement_effective_scale():.3f} "
        f"displacement_clip_arg={args_cli.openworldtactile_displacement_clip_px} "
        f"displacement_clip_effective={openworldtactile_displacement_effective_clip_px():.3f}."
    )
    print(
        "[INFO]: OpenWorldTactile SDK optical flow "
        f"scale_factor={args_cli.openworldtactile_flow_scale_factor} "
        f"winsize_arg={args_cli.openworldtactile_flow_winsize} "
        f"winsize_effective={openworldtactile_flow_effective_winsize()} "
        f"levels={args_cli.openworldtactile_flow_levels}."
    )
    print(
        "[INFO]: OpenWorldTactile FXYZ preview arrows "
        f"scale_arg={args_cli.openworldtactile_fxyz_arrow_scale} "
        f"scale_effective={openworldtactile_fxyz_arrow_scale():.3f} "
        f"step_arg={args_cli.openworldtactile_fxyz_arrow_step} "
        f"step_effective={openworldtactile_fxyz_arrow_step()}."
    )
    if args_cli.show_rgb_maps:
        print(
