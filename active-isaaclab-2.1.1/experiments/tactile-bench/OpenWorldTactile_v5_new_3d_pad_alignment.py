...
parser = argparse.ArgumentParser(
    description=(
        "V5 new stage 3d: PhysX grasp plus baseline-cleaned UIPC Fz with UIPC_Pad mounted as a stable "
        ...
        "and aligns the UIPC_Pad membrane center to the grasped cylinder in selected axes. "
        "It intentionally does not create RGB tactile output, Fx/Fy output, or a NutTool."
    )
)
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_3d_pad_center_align")
parser.add_argument("--workspace_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_3d_workspace")
...
parser.add_argument("--pad_center_feedback_gain", type=float, default=1.0)
...
def _pad_center_feedback_active(phase_name: str) -> bool:
    return bool(args_cli.pad_center_feedback) and _phase_at_or_after(
        str(phase_name),
        str(args_cli.pad_center_feedback_start_phase),
    )
...
def main() -> None:
    ...
    print(
        json.dumps(
            {
                "script_version": "v5_new_3d_pad_center_align",
    ...
    try:
        ...
        for phase in phases:
            ...
            for phase_frame in range(frame_count):
                ...
                pad_center_feedback_active = _pad_center_feedback_active(phase_name)
                pad_center_feedback_w = np.zeros(3, dtype=np.float64)
                if pad_center_feedback_active:
                    for axis_idx in _axis_indices(str(args_cli.pad_center_feedback_axes)):
                        pad_center_feedback_w[axis_idx] = (
                            float(args_cli.pad_center_feedback_gain) * pad_center_error_for_control_w[axis_idx]
                        )
                ...
                pad_center_error_yz_mm = float(np.linalg.norm(pad_center_error_after_w[1:3]) * 1000.0)
        ...
        metadata = {
            "script_version": "v5_new_3d_pad_center_align",
            "purpose": "mounted_pad_physx_grasp_with_uipc_fz_only_baseline_clean_contact_gap_debug_and_pad_center_align",
            ...
            "pad_center_alignment": {
                ...
                "feedback_definition": (
                    "nominal_ik_target_w + pad_center_feedback_gain * center_error_w on pad_center_feedback_axes; "
                    "manual grasp_target offsets are included in nominal waypoints"
                ),
