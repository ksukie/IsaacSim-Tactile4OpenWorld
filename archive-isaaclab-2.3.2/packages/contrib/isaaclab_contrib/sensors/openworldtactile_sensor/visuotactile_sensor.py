...
# 中文说明：OpenWorldTactile 视觉触觉传感器主体，负责采集 depth、计算触觉力场，并按配置选择触觉 RGB 渲染路线。
...
from .visuotactile_render import GelsightRender, OpenWorldTactileTactileRGBRender
...
class VisuoTactileSensor(SensorBase):
    ...
    **Example Usage:**
        For a complete working example, see: ``scripts/demos/sensors/openworldtactile/openworldtactile_example.py``
    ...
    **Current Limitations:**
    ...
    def _initialize_camera_tactile(self):
        ...
        rgb_render_mode = self.cfg.rgb_render_mode.lower()
        if rgb_render_mode == "openworldtactile":
            self._tactile_rgb_render = OpenWorldTactileTactileRGBRender(self.cfg.render_cfg, device=self.device)
        elif rgb_render_mode == "gelsight":
            self._tactile_rgb_render = GelsightRender(self.cfg.render_cfg, device=self.device)
        else:
            raise ValueError(
                f"Unsupported tactile RGB render mode: {self.cfg.rgb_render_mode}. "
                "Expected 'openworldtactile' or 'gelsight'."
            )
    ...
    def _update_buffers_impl(self, env_ids: Sequence[int]):
        ...
        # Update camera-based tactile data after force field so OpenWorldTactile RGB can use same-frame shear displacement.
        if self.cfg.enable_camera_tactile:
    ...
    def _update_camera_tactile(self, env_ids: Sequence[int] | slice):
        ...
        if depth_key:
            ...
            if self.cfg.rgb_render_mode.lower() == "openworldtactile":
                self._data.tactile_rgb_image[env_ids] = self._tactile_rgb_render.render(
                    diff.squeeze(-1),
                    displacement=self._get_openworldtactile_displacement_maps(env_ids),
                )
            else:
                self._data.tactile_rgb_image[env_ids] = self._tactile_rgb_render.render(diff.squeeze(-1))

    def _get_openworldtactile_displacement_maps(self, env_ids: Sequence[int] | slice) -> torch.Tensor | None:
        """Return force-field shear reshaped for OpenWorldTactile RGB texture displacement."""
        if not self.cfg.enable_force_field or self._data.tactile_shear_force is None:
            return None

        rows, cols = self.cfg.tactile_array_size
        shear = self._data.tactile_shear_force[env_ids]
        if shear.numel() == 0:
            return None

        return shear.reshape(-1, rows, cols, 2)
