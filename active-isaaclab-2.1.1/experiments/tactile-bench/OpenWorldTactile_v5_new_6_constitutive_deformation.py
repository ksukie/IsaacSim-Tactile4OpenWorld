...
EPS = 1.0e-12
SCRIPT_VERSION = "v5_new_6_constitutive_deformation_tactile"
EXPECTED_CONDA_ENV = "isaaclab211"
OFFICIAL_TACTILE_SOURCE = "uipc_membrane_deformation_constitutive_fxyz"
OFFICIAL_FZ_SOURCE = "uipc_front_surface_normal_deformation_constitutive_fz"
OFFICIAL_FX_FY_SOURCE = "uipc_front_surface_tangential_deformation_constitutive_friction_capped"
OFFICIAL_FORCE_SOURCE = "uipc_membrane_surface_deformation_constitutive_fxyz"
OFFICIAL_RAW_FORCE_SOURCE = "uipc_membrane_surface_deformation_constitutive_fxyz_pre_contact_mask"
...
OFFICIAL_OUTPUT_FILES = (
    ...
    "local_normal_compression_vertices.npy",
...
parser = argparse.ArgumentParser(
    description=(
        "V5 new stage 6: constitutive membrane-deformation tactile output. Fx/Fy/Fz are "
        "computed from the same UIPC front-surface deformation spring-damper model, with "
        "back-face drift correction and friction-capped tangential shear. Contact geometry "
        "is retained as a diagnostic/contact-mask source, not as the official Fz source."
    )
)
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_6_constitutive_deformation_tactile")
parser.add_argument("--workspace_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_6_workspace")
...
parser.add_argument(
    ...
    type=float,
    default=3.0e7,
    help="Legacy 5c contact-geometry gain, retained only for compatibility diagnostics in v5_new_6.",
)
...
parser.add_argument(
    "--fz_proxy_source",
    choices=("contact_geometry", "deformation"),
    default="deformation",
    help="Legacy compatibility flag. v5_new_6 official Fz always uses membrane deformation.",
)
...
parser.add_argument(
    ...
    type=float,
    default=None,
    help="Legacy alias for --shear_stiffness.",
)
parser.add_argument(
    "--shear_damping_n_s_per_m3",
    type=float,
    default=None,
    help="Legacy alias for --shear_damping.",
)
...
parser.add_argument("--highlight_uipc_membrane_visual", dest="highlight_uipc_membrane_visual", action="store_true", default=True)
parser.add_argument("--no_highlight_uipc_membrane_visual", dest="highlight_uipc_membrane_visual", action="store_false")
parser.add_argument("--uipc_membrane_visual_opacity", type=float, default=0.95)
parser.add_argument("--show_uipc_membrane_debug_plane", dest="show_uipc_membrane_debug_plane", action="store_true", default=True)
parser.add_argument("--hide_uipc_membrane_debug_plane", dest="show_uipc_membrane_debug_plane", action="store_false")
parser.add_argument("--uipc_membrane_debug_plane_size_mm", type=float, default=200.0)
parser.add_argument("--uipc_membrane_debug_plane_opacity", type=float, default=0.35)
...
if args_cli.shear_stiffness_n_per_m3 is not None:
    args_cli.shear_stiffness = float(args_cli.shear_stiffness_n_per_m3)
else:
    args_cli.shear_stiffness_n_per_m3 = float(args_cli.shear_stiffness)
if args_cli.shear_damping_n_s_per_m3 is not None:
    args_cli.shear_damping = float(args_cli.shear_damping_n_s_per_m3)
else:
    args_cli.shear_damping_n_s_per_m3 = float(args_cli.shear_damping)
...
UIPC_MEMBRANE_DEBUG_PLANE_ROOT = f"{RUNTIME_ROOT}/DebugUipcMembranePlane"
UIPC_MEMBRANE_DEBUG_PLANE_PATH = f"{UIPC_MEMBRANE_DEBUG_PLANE_ROOT}/front_yz_plane"
PHASE_ORDER = (
    "SETTLE_AFTER_RESET",
    "HOME",
    "APPROACH_PICK",
...
def _validate_args() -> None:
    ...
    if not (0.0 <= float(args_cli.uipc_membrane_visual_opacity) <= 1.0):
        parser.error("--uipc_membrane_visual_opacity must be in [0, 1].")
    if float(args_cli.uipc_membrane_debug_plane_size_mm) <= 0.0:
        parser.error("--uipc_membrane_debug_plane_size_mm must be > 0.")
    if not (0.0 <= float(args_cli.uipc_membrane_debug_plane_opacity) <= 1.0):
        parser.error("--uipc_membrane_debug_plane_opacity must be in [0, 1].")
...
def _highlight_uipc_membrane_mesh(
    stage: Usd.Stage,
    *,
    pad_asset_root: str,
    membrane_mesh_path: str,
    color: tuple[float, float, float] = (0.0, 0.35, 1.0),
    opacity: float = 0.95,
) -> bool:
    prim = stage.GetPrimAtPath(str(membrane_mesh_path))
    if not prim.IsValid():
        return False
    imageable = UsdGeom.Imageable(prim)
    if imageable:
    ...
    if gprim:
        ...
        gprim.CreateDisplayOpacityAttr().Set([float(opacity)])
        gprim.CreateDoubleSidedAttr().Set(True)
    shader_path = f"{pad_asset_root}/materials/physical_membrane_material/PreviewSurface"
    shader = stage.GetPrimAtPath(shader_path)
    if shader.IsValid():
        diffuse_attr = shader.GetAttribute("inputs:diffuseColor")
        if diffuse_attr:
            diffuse_attr.Set(Gf.Vec3f(float(color[0]), float(color[1]), float(color[2])))
        opacity_attr = shader.GetAttribute("inputs:opacity")
        if opacity_attr:
...
def _compute_constitutive_deformation_fxyz(
    *,
    rest_front_l: np.ndarray,
    current_front_corr_l: np.ndarray,
    prev_front_corr_l: np.ndarray | None,
    vertex_area: np.ndarray,
    normal_sign: int,
    dt: float,
    ...
    rest = np.asarray(rest_front_l, dtype=np.float32)
    current = np.asarray(current_front_corr_l, dtype=np.float32)
    area = np.asarray(vertex_area, dtype=np.float32).reshape(-1)
    if rest.shape != current.shape or rest.ndim != 2 or rest.shape[1] != 3:
        raise ValueError(f"rest/current front surface shape mismatch: {rest.shape} vs {current.shape}")
    if area.shape[0] != rest.shape[0]:
        raise ValueError(f"vertex area count mismatch: {area.shape[0]} vs {rest.shape[0]}")

    signed_normal_disp = float(normal_sign) * (rest[:, 0] - current[:, 0])
    compression = np.clip(signed_normal_disp, 0.0, None).astype(np.float32)
    shear_disp_yz = (current[:, 1:3] - rest[:, 1:3]).astype(np.float32)

    if prev_front_corr_l is None or np.asarray(prev_front_corr_l).shape != current.shape:
        compression_vel = np.zeros_like(compression, dtype=np.float32)
        shear_vel_yz = np.zeros_like(shear_disp_yz, dtype=np.float32)
    else:
        prev = np.asarray(prev_front_corr_l, dtype=np.float32)
        prev_compression = np.clip(float(normal_sign) * (rest[:, 0] - prev[:, 0]), 0.0, None).astype(np.float32)
        prev_shear_disp_yz = (prev[:, 1:3] - rest[:, 1:3]).astype(np.float32)
        inv_dt = 1.0 / max(float(dt), EPS)
        compression_vel = ((compression - prev_compression) * inv_dt).astype(np.float32)
        shear_vel_yz = ((shear_disp_yz - prev_shear_disp_yz) * inv_dt).astype(np.float32)

    normal_pressure_raw = (
        float(normal_stiffness) * compression
        + float(normal_damping) * np.clip(compression_vel, 0.0, None)
    ).astype(np.float32)
    normal_force_raw = (area * np.clip(normal_pressure_raw, 0.0, None)).astype(np.float32)
    contact_mask = compression > float(pressure_threshold_m)
    normal_force = (normal_force_raw * contact_mask.astype(np.float32)).astype(np.float32)

    shear_raw_yz = (
        (float(shear_stiffness) * shear_disp_yz + float(shear_damping) * shear_vel_yz) * area[:, None]
    ).astype(np.float32)
    shear_raw_yz[:, 0] *= float(shear_sign_y)
    shear_raw_yz[:, 1] *= float(shear_sign_z)

    shear_raw_masked_yz = shear_raw_yz.copy()
    if not bool(enable_tangential_shear):
        shear_raw_masked_yz[:] = 0.0
    shear_raw_masked_yz[~contact_mask] = 0.0

    friction_cap = (float(friction_mu) * normal_force).astype(np.float32)
    shear_raw_mag = np.linalg.norm(shear_raw_masked_yz, axis=1).astype(np.float32)
    shear_scale = np.minimum(1.0, friction_cap / np.maximum(shear_raw_mag, EPS)).astype(np.float32)
    shear_capped_yz = (shear_raw_masked_yz * shear_scale[:, None]).astype(np.float32)
    shear_capped_yz[~contact_mask] = 0.0

    raw_local_fxyz = np.stack(
        (
            shear_capped_yz[:, 0],
            shear_capped_yz[:, 1],
            normal_force_raw,
    ...
    local_fxyz[:, 2] = normal_force

    shear_mag = np.linalg.norm(shear_capped_yz, axis=1).astype(np.float32)
    friction_violation = np.maximum(shear_mag - friction_cap, 0.0).astype(np.float32)
    diag: dict[str, float | int | bool | str] = {
        "enabled": True,
        "force_source": OFFICIAL_FORCE_SOURCE,
        "model": "v27_membrane_deformation_spring_damper_with_back_drift_correction",
        "contact_vertices": int(np.count_nonzero(contact_mask)),
        "tangential_shear_enabled": bool(enable_tangential_shear),
        "compression_max_mm": float(np.max(compression)) * 1000.0 if compression.size else 0.0,
        "compression_velocity_max_mm_s": (
            float(np.max(np.clip(compression_vel, 0.0, None))) * 1000.0 if compression_vel.size else 0.0
        ),
        "shear_disp_max_mm": float(np.max(np.linalg.norm(shear_disp_yz, axis=1))) * 1000.0
        if shear_disp_yz.size
        else 0.0,
        "shear_velocity_max_mm_s": float(np.max(np.linalg.norm(shear_vel_yz, axis=1))) * 1000.0
        if shear_vel_yz.size
        else 0.0,
        "normal_force_raw_sum": float(np.sum(normal_force_raw, dtype=np.float64)),
        "normal_force_sum": float(np.sum(normal_force, dtype=np.float64)),
        "normal_force_max": float(np.max(normal_force)) if normal_force.size else 0.0,
        "raw_shear_max": float(np.max(shear_raw_mag)) if shear_raw_mag.size else 0.0,
        "capped_shear_max": float(np.max(shear_mag)) if shear_mag.size else 0.0,
        "friction_cap_max": float(np.max(friction_cap)) if friction_cap.size else 0.0,
        "friction_capped_vertices": int(np.count_nonzero(shear_scale < 0.999)),
        "friction_cap_violation_max": float(np.max(friction_violation)) if friction_violation.size else 0.0,
        "sum_fx": float(np.sum(local_fxyz[:, 0], dtype=np.float64)),
        "sum_fy": float(np.sum(local_fxyz[:, 1], dtype=np.float64)),
        "sum_shear_abs": float(np.sum(shear_mag, dtype=np.float64)),
    }
    return (
        raw_local_fxyz,
        ...
        shear_disp_yz,
        shear_vel_yz,
        shear_raw_masked_yz.astype(np.float32),
        shear_capped_yz.astype(np.float32),
        friction_cap.astype(np.float32),
        diag,
    )
...
def _uipc_membrane_debug_plane_points_w(
    *,
    current_front_l: np.ndarray,
    pad_pos_w: np.ndarray,
    pad_quat_wxyz: tuple[float, float, float, float],
    ...
    front = np.asarray(current_front_l, dtype=np.float32)
    center_l = _bbox_center(front).astype(np.float64)
    half = 0.5 * float(size_m)
    plane_l = np.asarray(
        [
            [center_l[0], center_l[1] - half, center_l[2] - half],
            [center_l[0], center_l[1] + half, center_l[2] - half],
            [center_l[0], center_l[1] + half, center_l[2] + half],
            [center_l[0], center_l[1] - half, center_l[2] + half],
        ],
    ...
    return _world_from_local(plane_l, pad_pos_w, pad_quat_wxyz).astype(np.float32)


def _write_or_update_uipc_membrane_debug_plane(
    stage: Usd.Stage,
    *,
    current_front_l: np.ndarray,
    pad_pos_w: np.ndarray,
    ...
    points_w = _uipc_membrane_debug_plane_points_w(
    ...
    if stage.GetPrimAtPath(UIPC_MEMBRANE_DEBUG_PLANE_PATH).IsValid():
        _set_mesh_points(stage, UIPC_MEMBRANE_DEBUG_PLANE_PATH, points_w)
        prim = stage.GetPrimAtPath(UIPC_MEMBRANE_DEBUG_PLANE_PATH)
        imageable = UsdGeom.Imageable(prim)
        if imageable:
            imageable.MakeVisible()
        gprim = UsdGeom.Gprim(prim)
        if gprim:
            gprim.CreateDisplayColorAttr().Set([Gf.Vec3f(0.0, 0.8, 1.0)])
            gprim.CreateDisplayOpacityAttr().Set([float(opacity)])
            gprim.CreateDoubleSidedAttr().Set(True)
    else:
        _write_triangle_mesh(
            stage,
            UIPC_MEMBRANE_DEBUG_PLANE_PATH,
            points_w,
            triangles,
            color=(0.0, 0.8, 1.0),
            opacity=float(opacity),
        )
    center_w = np.mean(points_w, axis=0)
    return {
        "enabled": True,
        "prim_path": UIPC_MEMBRANE_DEBUG_PLANE_PATH,
        "source": "current_uipc_front_face_bbox_center",
        "plane_frame": "pad_local_yz_at_current_front_face_center",
        "normal": "pad_local_x",
        "size_mm": float(size_m) * 1000.0,
        "opacity": float(opacity),
        "center_w_m": [float(v) for v in center_w],
    }
...
def main() -> None:
        ...
        json.dumps(
            {
                ...
                "fx_fy_contract": (
                    "pad-local Y/Z tangential constitutive shear from back-drift-corrected UIPC "
                    "front-surface deformation, friction-capped by constitutive Fz"
                ),
    ...
    uipc_membrane_highlighted = False
    if bool(args_cli.hide_uipc_membrane_visual):
    ...
    elif bool(args_cli.highlight_uipc_membrane_visual):
        uipc_membrane_highlighted = _highlight_uipc_membrane_mesh(
            stage,
            pad_asset_root=pad_asset_root,
            membrane_mesh_path=membrane_mesh_path,
            opacity=float(args_cli.uipc_membrane_visual_opacity),
        )
        print(
            f"[INFO] UIPC membrane compute visual highlighted={uipc_membrane_highlighted} "
            f"path={membrane_mesh_path}",
            flush=True,
        )
    uipc_sim = UipcSim(
        UipcSimCfg(
    ...
    uipc_membrane_debug_plane_info: dict[str, object] = {
        "enabled": bool(args_cli.show_uipc_membrane_debug_plane),
        "prim_path": UIPC_MEMBRANE_DEBUG_PLANE_PATH,
        "source": "current_uipc_front_face_bbox_center",
        "plane_frame": "pad_local_yz_at_current_front_face_center",
        "status": "disabled" if not bool(args_cli.show_uipc_membrane_debug_plane) else "initialized",
    }
    if bool(args_cli.show_uipc_membrane_debug_plane):
        uipc_membrane_debug_plane_info = _write_or_update_uipc_membrane_debug_plane(
            stage,
            current_front_l=rest_front_l,
            pad_pos_w=rest_pad_pos,
            pad_quat_wxyz=rest_pad_quat,
            size_m=float(args_cli.uipc_membrane_debug_plane_size_mm) * 1.0e-3,
            opacity=float(args_cli.uipc_membrane_debug_plane_opacity),
        )
    ...
    local_normal_compression_vertex_frames: list[np.ndarray] = []
    ...
    prev_constitutive_front_corr_l: np.ndarray | None = None
    ...
    try:
        ...
        for phase in phases:
            ...
            for phase_frame in range(frame_count):
                ...
                (
                    ...
                    shear_friction_cap,
                    shear_diag,
                ) = _compute_constitutive_deformation_fxyz(
                    rest_front_l=rest_front_l,
                    current_front_corr_l=current_front_corr_l,
                    prev_front_corr_l=prev_constitutive_front_corr_l,
                    ...
                    enable_tangential_shear=bool(args_cli.enable_tangential_shear_proxy),
                )
                if _uipc_baseline_phase(phase_name):
                    ...
                    baseline_tangent_yz_frames.append(tangent_disp_yz.copy())
                ...
                if tool_enabled and baseline_tangent_yz is None:
                    if baseline_tangent_yz_frames:
                        ...
                        baseline_tangent_yz = np.zeros_like(tangent_disp_yz, dtype=np.float32)
                geometry_contact_frame = int(contact_geom["contact_candidate_count"]) > 0
                if tool_enabled and geometry_contact_frame:
                    corrected_fz = local_fxyz[:, 2].astype(np.float32)
                    corrected_indent = raw_indent.astype(np.float32)
                    shear_contact_mask = np.logical_and(
                        corrected_mask,
                        corrected_fz > float(args_cli.shear_contact_fz_threshold),
                    )
                    shear_capped_yz = shear_capped_yz.copy()
                    shear_raw_yz = shear_raw_yz.copy()
                    shear_capped_yz[~shear_contact_mask] = 0.0
                    shear_raw_yz[~shear_contact_mask] = 0.0
                    local_fxyz[:, 0] = shear_capped_yz[:, 0]
                    local_fxyz[:, 1] = shear_capped_yz[:, 1]
                    raw_local_fxyz[:, 0] = shear_capped_yz[:, 0]
                    raw_local_fxyz[:, 1] = shear_capped_yz[:, 1]
                else:
                    corrected_fz = np.zeros_like(raw_local_fxyz[:, 2], dtype=np.float32)
                    corrected_indent = np.zeros_like(raw_indent, dtype=np.float32)
                    corrected_mask = np.zeros_like(corrected_mask, dtype=bool)
                    shear_contact_mask = np.zeros_like(corrected_mask, dtype=bool)
                    local_fxyz = np.zeros_like(raw_local_fxyz, dtype=np.float32)
                ...
                shear_tangent_rel_yz = tangent_disp_yz.astype(np.float32)
                shear_prev_tangent_rel_yz = shear_tangent_rel_yz.copy()
                shear_prev_contact_mask = shear_contact_mask.copy()
                prev_constitutive_front_corr_l = current_front_corr_l.copy()
                shear_mag_final = np.linalg.norm(shear_capped_yz, axis=1).astype(np.float32)
                shear_raw_mag_final = np.linalg.norm(shear_raw_yz, axis=1).astype(np.float32)
                shear_cap_violation_final = np.maximum(shear_mag_final - shear_friction_cap, 0.0).astype(np.float32)
                shear_diag.update(
                    {
                        "tool_enabled": bool(tool_enabled),
                        "contact_mask_source": (
                            "contact_geometry_frame_candidate_count_positive_and_uipc_membrane_compression_threshold"
                        ),
                        "contact_geometry_role": "official_contact_gate_and_diagnostic_not_force_magnitude_source",
                        "contact_geometry_frame_gate": bool(geometry_contact_frame),
                        "contact_geometry_contact_candidate_count": int(contact_geom["contact_candidate_count"]),
                        "contact_geometry_yz_overlap_candidate_count": int(contact_geom["yz_overlap_candidate_count"]),
                        "contact_geometry_max_signed_penetration_mm": float(contact_geom["max_signed_penetration_mm"]),
                        "contact_vertices": int(np.count_nonzero(shear_contact_mask)),
                        "normal_force_sum": float(np.sum(corrected_fz, dtype=np.float64)),
                        "raw_shear_max": float(np.max(shear_raw_mag_final)) if shear_raw_mag_final.size else 0.0,
                        "capped_shear_max": float(np.max(shear_mag_final)) if shear_mag_final.size else 0.0,
                        "friction_cap_max": float(np.max(shear_friction_cap)) if shear_friction_cap.size else 0.0,
                        "friction_cap_violation_max": (
                            float(np.max(shear_cap_violation_final)) if shear_cap_violation_final.size else 0.0
                        ),
                        "sum_fx": float(np.sum(local_fxyz[:, 0], dtype=np.float64)),
                        "sum_fy": float(np.sum(local_fxyz[:, 1], dtype=np.float64)),
                        "sum_shear_abs": float(np.sum(shear_mag_final, dtype=np.float64)),
                    }
                )
                ...
                local_normal_compression_vertex_frames.append(corrected_indent.astype(np.float32))
        ...
        if uipc_frame_count > 0:
            ...
            local_normal_compression_vertex_array = np.stack(
                local_normal_compression_vertex_frames,
                axis=0,
            ).astype(np.float32)
            ...
            if bool(args_cli.enable_pressure_gradient_features):
                ...
                indent_vertices_array = local_normal_compression_vertex_array.astype(np.float32)
            ...
            pressure_gradient_diagnostics = {
                ...
                "base_version": SCRIPT_VERSION,
                "enabled": bool(args_cli.enable_pressure_gradient_features),
                "base_tactile_source": OFFICIAL_TACTILE_SOURCE,
                ...
                "native_uipc_contact_gradient_used": False,
                "native_uipc_pressure_used": False,
                "unit_note": "constitutive deformation pressure gradient, not calibrated real Pa/m",
                "grid_axis_semantics": {
                    "axis_0": "frame",
                ...
                "gradient_component_semantics": {
                    ...
                    "indent_grad_y": "d(membrane_normal_compression)/d(membrane_y), computed along axis_2",
                    "indent_grad_z": "d(membrane_normal_compression)/d(membrane_z), computed along axis_1",
                },
                ...
                "indent_feature_note": {
                    "indent_source": "local_normal_compression_vertices.npy",
                    "normal_stiffness": float(args_cli.normal_stiffness),
                    "normal_damping": float(args_cli.normal_damping),
                    "node_area_m2": float(indent_node_area_m2),
                    "unit": "meter",
                    "note": "v5_new_6 stores membrane front-surface compression directly from rest_front_l - current_front_corr_l.",
                },
                "idw": {
                    "k": int(args_cli.pressure_gradient_idw_k),
                    "eps": float(args_cli.pressure_gradient_idw_eps),
                    "source": "front_face_vertex_yz + local_normal_compression_vertices",
                },
            }
            ...
            shear_proxy_diagnostics = {
                ...
                "channel_order": {
                    "local_fx": "pad local Y tangential constitutive membrane shear force",
                    "local_fy": "pad local Z tangential constitutive membrane shear force",
                    "local_fz": "pad local X normal constitutive membrane compression force",
                },
                "model": {
                    "back_face_drift_correction": True,
                    "rest_reference": "post_warmup_uipc_surface_rest_front_l",
                    "baseline_source": "saved_for_diagnostics_only_not_subtracted_from_official_force",
                    "contact_gated_by": "contact_geometry_frame_candidate_count_positive AND membrane_compression_threshold",
                    "stick_reference_used": False,
                    "spring_damper": True,
                    ...
                    "shear_stiffness": float(args_cli.shear_stiffness),
                    "shear_damping": float(args_cli.shear_damping),
                    "legacy_shear_stiffness_n_per_m3_alias": float(args_cli.shear_stiffness_n_per_m3),
                    "legacy_shear_damping_n_s_per_m3_alias": float(args_cli.shear_damping_n_s_per_m3),
                    "friction_mu": float(args_cli.friction_mu),
            ...
            shear_video_diagnostics = {
                ...
                "current_stage_note": (
                    "Fx/Fy/Fz come from the v5_new_6 constitutive membrane-deformation estimator: "
                    "back-drift-corrected UIPC front-surface compression and tangential displacement, "
                    "spring-damper response, and mu*Fz friction cap. Contact geometry is diagnostic-only."
                ),
                "not_used": [
            ...
            np.save(output_dir / "local_normal_compression_vertices.npy", local_normal_compression_vertex_array)
            ...
            np.save(output_dir / "local_normal_compression_vertices.npy", empty_vertex_scalar)
        ...
        metadata = {
            ...
            "expected_conda_env": EXPECTED_CONDA_ENV,
            "runtime_conda_env": str(os.environ.get("CONDA_DEFAULT_ENV", "")),
            "version_role": "constitutive_membrane_deformation_tactile",
            "purpose": (
                "restore_v27_style_membrane_deformation_force_estimator_inside_the_v5_mounted_uipc_pad_framework"
            ),
            "official_tactile_source": OFFICIAL_TACTILE_SOURCE,
            ...
            "constitutive_force_model": {
                "reference": "v2.7 MembraneForceEstimator logic integrated into v5 mounted UIPC pad pipeline",
                "rest_reference": "post_warmup_uipc_surface_rest_front_l",
                "front_surface": "front_indices local +X face",
                "back_drift_correction": "_front_corrected_by_back_drift",
                "normal_compression_formula": "normal_sign * (rest_front_l[:,0] - current_front_corr_l[:,0]) clipped at 0",
                "tangential_displacement_formula": "current_front_corr_l[:,1:3] - rest_front_l[:,1:3]",
                "normal_force_formula": "front_face_vertex_area * max(normal_stiffness*compression + normal_damping*positive_compression_velocity, 0)",
                "shear_force_formula": "front_face_vertex_area * (shear_stiffness*tangential_displacement + shear_damping*tangential_velocity)",
                "friction_cap": "sqrt(fx^2+fy^2) <= friction_mu * fz",
                "normal_stiffness": float(args_cli.normal_stiffness),
                "normal_damping": float(args_cli.normal_damping),
                "shear_stiffness": float(args_cli.shear_stiffness),
                "shear_damping": float(args_cli.shear_damping),
            ...
            "pressure_field_source": "mesh_front_face_constitutive_membrane_deformation_fz",
            ...
            "created_membrane_normal_line": bool(args_cli.show_membrane_normal_line),
            "membrane_normal_line": membrane_normal_line_info,
            "created_uipc_membrane_debug_plane": bool(args_cli.show_uipc_membrane_debug_plane),
            "uipc_membrane_debug_plane": uipc_membrane_debug_plane_info,
            "hidden_uipc_tool_visual": bool(args_cli.hide_uipc_tool_visual),
            "uipc_tool_visual_hidden_prim_count": int(uipc_tool_visual_hidden_count),
            "hidden_uipc_membrane_visual": bool(args_cli.hide_uipc_membrane_visual),
            "uipc_membrane_visual_hidden_prim_count": int(uipc_membrane_visual_hidden_count),
            "uipc_membrane_visual_highlighted": bool(uipc_membrane_highlighted),
            "uipc_membrane_visual_highlight_color_rgb": [0.0, 0.35, 1.0],
            "uipc_membrane_visual_highlight_opacity": float(args_cli.uipc_membrane_visual_opacity),
            "hidden_pad_camera_surface": bool(args_cli.hide_pad_camera_surface),
            "pad_camera_surface_hidden": bool(pad_camera_surface_hidden),
            "pad_visual_camera_mesh": str(pad_contract["visual_camera_mesh"]),
            "pad_visual_camera_mesh_exists": bool(pad_contract["visual_camera_mesh_exists"]),
            ...
            "pressure_mask_grid_source": "contact_geometry_frame_gate_and_uipc_membrane_compression_threshold_vertices",
            "fz_proxy_source": "uipc_membrane_deformation_constitutive_official; legacy_arg="
            + str(args_cli.fz_proxy_source),
            "baseline_subtraction_used": False,
            "baseline_role": "diagnostic_only_saved_for_comparison_not_subtracted_from_official_force",
            ...
            "native_uipc_contact_force_replaces_proxy": False,
            "fx_fy_contract": (
                "local_fx/local_fy are pad-local Y/Z tangential constitutive shear channels derived from "
                "back-drift-corrected UIPC front-surface tangential deformation and friction-capped by mu*Fz; "
                "local_fz is derived from the same membrane normal deformation estimator"
            ),
            "fx_fy_zero_placeholders": False,
            "tactile_data_semantics": {
                ...
                "shear_proxy_visualization_layer": {
                    ...
                    "role": "force_conserving_reconstruction_of_constitutive_tangential_shear_channels",
            ...
            "uipc_runtime_membrane": {
                ...
                "hidden": bool(args_cli.hide_uipc_membrane_visual),
                "visual_hidden_for_display_clean": bool(args_cli.hide_uipc_membrane_visual),
                "visual_highlighted": bool(uipc_membrane_highlighted),
                "visual_highlight_color_rgb": [0.0, 0.35, 1.0],
                "visual_highlight_opacity": float(args_cli.uipc_membrane_visual_opacity),
