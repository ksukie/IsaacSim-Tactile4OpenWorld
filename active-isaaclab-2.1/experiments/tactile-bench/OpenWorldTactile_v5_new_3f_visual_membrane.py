...
parser = argparse.ArgumentParser(
    description=(
        "V5 new stage 3f: PhysX grasp plus baseline-cleaned UIPC Fz with UIPC_Pad mounted as a stable "
        "link7 child asset. This verifies mount stability, smooth gripper motion, "
        "dynamic cylinder grasp/lift, and a Fz-only UIPC deformation proxy with non-contact "
        "baseline suppression, logs UIPC CylinderTool-to-membrane signed gap diagnostics, "
        "hides UIPC compute visuals and duplicate pad visual membrane layers, and aligns the UIPC_Pad "
        "membrane center in pad-local plane axes with a frozen correction after lower-to-grasp. "
        "It intentionally does not create RGB tactile output, Fx/Fy output, or a NutTool."
    )
)
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_3f_single_visual_membrane")
parser.add_argument("--workspace_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_3f_workspace")
...
parser.add_argument("--grasp_target_y_offset_mm", type=float, default=0.0)
parser.add_argument("--grasp_target_z_offset_mm", type=float, default=0.0)
...
parser.add_argument("--hide_uipc_membrane_visual", dest="hide_uipc_membrane_visual", action="store_true", default=True)
parser.add_argument("--show_uipc_membrane_visual", dest="hide_uipc_membrane_visual", action="store_false")
parser.add_argument("--hide_pad_visual_back_mesh", dest="hide_pad_visual_back_mesh", action="store_true", default=True)
...
def _validate_args() -> None:
    ...
    if float(args_cli.mapping_error_warn_mm) < 0.0:
...
def _rehide_uipc_compute_visuals(
    stage: Usd.Stage,
    membrane_root: str,
    pad_visual_back_mesh: str | None = None,
) -> tuple[int, int]:
    membrane_count = 0
    tool_count = 0
    ...
    if bool(args_cli.hide_pad_visual_back_mesh) and pad_visual_back_mesh:
        _hide_prim_if_valid(stage, pad_visual_back_mesh)
...
def _uipc_contact_geometry(
    *,
    ...
    max_penetration_m = float(np.max(np.clip(-signed_gap_m, 0.0, None))) if signed_gap_m.size else 0.0
...
def main() -> None:
        ...
        json.dumps(
            {
                ...
                "visual_target_is_simulation_mesh": str(pad_contract["visual_target"])
    ...
    if bool(args_cli.list_pad_visual_prims):
        print(
            json.dumps(
                {
                    "pad_visual_debug_prims": pad_visual_debug_records,
                },
                indent=2,
            ),
            flush=True,
        )
    if str(pad_contract["visual_target"]) == str(pad_contract["membrane_sim_mesh"]):
        print(
            "[WARN] pad visual target is the UIPC simulation membrane mesh; hiding the simulation root may hide the visible pad membrane.",
            flush=True,
        )
    ...
    print(
        json.dumps(
            {
                "script_version": "v5_new_3f_single_visual_membrane",
    ...
    robot.update(0.0)
    cylinder.update(0.0)
    current_pad_pos, current_pad_quat = _read_xform_pose(pad_view, device=sim.device)
    ...
    rest_pad_pos, rest_pad_quat = _read_xform_pose(pad_view, device=sim.device)
    gel_init_vertices_w = membrane.init_vertex_pos.detach().cpu().numpy().astype(np.float32)
    gel_init_vertices_l = _local_from_world(gel_init_vertices_w, rest_pad_pos, rest_pad_quat)
    ...
    for warmup_step in range(max(0, int(args_cli.uipc_warmup_steps))):
        ...
        if render:
            ...
            uipc_sim.update_render_meshes()
            _rehide_uipc_compute_visuals(stage, membrane_root, str(pad_contract["visual_back_mesh"]))
    ...
    try:
        ...
        for phase in phases:
            ...
            for phase_frame in range(frame_count):
                ...
                if hold_object:
                    _place_object_root(cylinder, object_pos, object_quat, device=sim.device)
                    cylinder.update(0.0)
                pad_pos_for_uipc, pad_quat_for_uipc = _read_xform_pose(pad_view, device=sim.device)
                ...
                raw_local_fxyz_grid = _vertex_values_to_grid(visual_mapper, raw_local_fxyz[visual_to_front])
                local_fxyz_grid = _vertex_values_to_grid(visual_mapper, local_fxyz[visual_to_front])
                pressure_mask_grid = _vertex_values_to_grid(visual_mapper, corrected_mask[visual_to_front]).astype(bool)
        ...
        metadata = {
            "script_version": "v5_new_3f_single_visual_membrane",
            "purpose": (
                "mounted_pad_physx_grasp_with_uipc_fz_only_baseline_clean_contact_gap_debug_"
                "single_visual_membrane_and_pad_center_freeze"
            ),
            ...
            "pad_visual_back_mesh_exists": bool(pad_contract["visual_back_mesh_exists"]),
            "pad_visual_debug_prims": pad_visual_debug_records,
            "pad_visual_target_is_simulation_mesh": str(pad_contract["visual_target"]) == str(pad_contract["membrane_sim_mesh"]),
            ...
            "pad_center_alignment": {
                ...
                "pad_front_center_source": "bbox_center_of_pad_visual_membrane_camera_surface_in_pad_local_frame",
            ...
            "pad_visual_path_check": {
                "pad_visual_target": str(pad_contract["visual_target"]),
                "kept_visual_membrane": str(pad_contract["visual_target"]),
                "pad_visual_back_mesh": str(pad_contract["visual_back_mesh"]),
                "pad_visual_back_mesh_exists": bool(pad_contract["visual_back_mesh_exists"]),
                "pad_visual_back_mesh_hidden": bool(pad_visual_back_mesh_hidden),
                "uipc_membrane_root": str(pad_contract["simulation_root"]),
                "uipc_membrane_mesh": str(pad_contract["membrane_sim_mesh"]),
                "visual_target_is_simulation_mesh": str(pad_contract["visual_target"]) == str(pad_contract["membrane_sim_mesh"]),
            },
            ...
            "self_calibration": {
                ...
                "visual_mapping_error_max_mm": mapping_error_max * 1000.0,
            },
            ...
            json.dumps(
                {
                    ...
                    "pad_visual_back_mesh": str(pad_contract["visual_back_mesh"]),
                    "pad_visual_back_mesh_exists": bool(pad_contract["visual_back_mesh_exists"]),
                    "pad_visual_target_is_simulation_mesh": str(pad_contract["visual_target"])
                    == str(pad_contract["membrane_sim_mesh"]),
                    "baseline_frame_count": len(baseline_fz_frames),
                    "max_sum_fz_raw": float(np.max(sum_fz_raw_frames)) if sum_fz_raw_frames else 0.0,
                    "max_sum_fz_corrected": float(np.max(sum_fz_frames)) if sum_fz_frames else 0.0,
