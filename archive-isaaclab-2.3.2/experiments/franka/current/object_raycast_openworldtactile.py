...
"""
Run Franka IK-Abs cube lift with object-only virtual tactile pads and OpenWorldTactile RGB.

This script keeps the stock Franka lift state machine and adds two read-only
virtual tactile pads on the Panda fingers. Each pad raycasts only against the
target object, turns the nearest object hits into a localized elastomer-like
deformation patch, renders that patch through the OpenWorldTactile RGB renderer, and feeds
the RGB frame to the OpenWorldTactile SDK bridge for virtual force output.
...
.. code-block:: bash
    ...
    ./isaaclab.sh -p experiments/franka/current/object_raycast_openworldtactile.py --headless
...
parser = argparse.ArgumentParser(description="Pick/lift DexCube with Franka and save object-only OpenWorldTactile tactile RGB.")
parser.add_argument(
...
parser.add_argument(
    "--output_dir",
    type=str,
    default="outputs/franka_lift_object_only_openworldtactile",
...
parser.add_argument(
    ...
    help="Virtual tactile pad grid size in meters.",
)
parser.add_argument(
...
parser.add_argument(
    "--virtual_pad_contact_band",
    type=float,
    default=0.010,
    help="Local distance band in meters around the nearest object hit that forms the contact core.",
)
parser.add_argument(
    "--virtual_pad_max_distance",
    type=float,
    default=0.020,
    help="Maximum object-only raycast distance in meters.",
)
parser.add_argument(
...
parser.add_argument(
    "--pressure_patch_radius_px",
    type=int,
    default=18,
    help="Ellipse dilation radius in final OpenWorldTactile pixels for expanding the local contact core.",
)
parser.add_argument(
    "--pressure_blur_sigma_px",
    type=float,
    default=9.0,
    help="Gaussian blur sigma in final OpenWorldTactile pixels for smoothing the virtual deformation patch.",
)
parser.add_argument(
    "--pressure_min_active_pixels",
    type=int,
    default=3,
    help="Minimum active low-resolution ray pixels required before rendering a deformation patch.",
)
...
class VirtualPadXZPatternCfg:
    ...
    size: tuple[float, float] = (0.020, 0.030)
...
def make_virtual_pad_cfg(name: str, finger_link: str, side_sign: float):
    """Create an object-only virtual tactile pad raycaster on one Panda finger."""
...
def add_virtual_pad_sensors(env_cfg: LiftEnvCfg):
    """Add left and right object-only virtual tactile pads."""
...
def virtual_pressure_map(raycaster, env_id: int) -> np.ndarray:
    """Convert object-only ray hits into one OpenWorldTactile-sized virtual deformation patch."""
    rows, cols = pad_image_shape()
    image_height, image_width = openworldtactile_image_shape()
    hits = raycaster.data.ray_hits_w[env_id]
    starts = raycaster._ray_starts_w[env_id]
    if hits.shape[0] != rows * cols:
        raise RuntimeError(f"Virtual pad ray count mismatch: got {hits.shape[0]}, expected {rows * cols}.")

    hit_is_finite = torch.isfinite(hits).all(dim=-1)
    distance = torch.linalg.norm(hits - starts, dim=-1)
    valid_hit = (
        hit_is_finite
        & torch.isfinite(distance)
        & (distance >= 0.0)
        & (distance <= args_cli.virtual_pad_max_distance)
    )
    zero_pressure = np.zeros((image_height, image_width), dtype=np.float32)
    if int(valid_hit.sum().item()) < args_cli.pressure_min_active_pixels:
        return zero_pressure

    min_distance = torch.min(distance[valid_hit])
    relative_distance = distance - min_distance
    core_valid = valid_hit & (relative_distance <= args_cli.virtual_pad_contact_band)
    if int(core_valid.sum().item()) < args_cli.pressure_min_active_pixels:
        return zero_pressure

    low_res_pressure = torch.zeros_like(distance)
    low_res_pressure[core_valid] = torch.clamp(
        (args_cli.virtual_pad_contact_band - relative_distance[core_valid]) / args_cli.virtual_pad_contact_band,
        0.0,
        1.0,
    )
    pressure = low_res_pressure.reshape(rows, cols).detach().cpu().numpy().astype(np.float32)
    pressure = cv2.resize(pressure, (image_width, image_height), interpolation=cv2.INTER_LINEAR)

    patch_radius = int(args_cli.pressure_patch_radius_px)
    if patch_radius > 0:
        kernel_size = patch_radius * 2 + 1
        kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (kernel_size, kernel_size))
        pressure = cv2.dilate(pressure, kernel)

    blur_sigma = float(args_cli.pressure_blur_sigma_px)
    if blur_sigma > 0.0:
        kernel_size = int(np.ceil(blur_sigma * 6.0)) | 1
        pressure = cv2.GaussianBlur(pressure, (kernel_size, kernel_size), blur_sigma)

    pressure_max = float(np.nanmax(pressure)) if pressure.size else 0.0
    if pressure_max <= 1.0e-6:
        return zero_pressure
    pressure = np.clip(pressure / pressure_max, 0.0, 1.0)
...
def maybe_save_virtual_tactile_pair(
    ...
    """Save object-only OpenWorldTactile RGB frames during grasp and lift states."""
    ...
    left_pressure = virtual_pressure_map(scene["left_virtual_pad"], args_cli.record_env_id)
    right_pressure = virtual_pressure_map(scene["right_virtual_pad"], args_cli.record_env_id)
...
def main():
    ...
    if args_cli.virtual_pad_contact_band <= 0.0:
        raise ValueError("--virtual_pad_contact_band must be positive.")
    if args_cli.virtual_pad_max_distance <= 0.0:
        raise ValueError("--virtual_pad_max_distance must be positive.")
    if args_cli.virtual_pad_contact_band > args_cli.virtual_pad_max_distance:
        raise ValueError("--virtual_pad_contact_band must be <= --virtual_pad_max_distance.")
    ...
    if args_cli.pressure_patch_radius_px < 0:
        raise ValueError("--pressure_patch_radius_px must be non-negative.")
    if args_cli.pressure_blur_sigma_px < 0.0:
        raise ValueError("--pressure_blur_sigma_px must be non-negative.")
    if args_cli.pressure_min_active_pixels <= 0:
        raise ValueError("--pressure_min_active_pixels must be positive.")
    ...
    rows, cols = pad_image_shape()
    image_height, image_width = openworldtactile_image_shape()
    print(f"[INFO]: Saving object-only OpenWorldTactile tactile outputs under: {output_dir.resolve()}")
    print(
        "[INFO]: Virtual pad grid "
        f"rows={rows} cols={cols} size={tuple(args_cli.virtual_pad_size)} "
        f"resolution={args_cli.virtual_pad_resolution} contact_band={args_cli.virtual_pad_contact_band} "
        f"side_offset={args_cli.virtual_pad_side_offset} z_offset={args_cli.virtual_pad_z_offset}."
    )
    print(
        "[INFO]: OpenWorldTactile output "
        f"height={image_height} width={image_width} patch_radius_px={args_cli.pressure_patch_radius_px} "
        f"blur_sigma_px={args_cli.pressure_blur_sigma_px} min_active_pixels={args_cli.pressure_min_active_pixels}."
    )
    ...
    try:
        ...
        print(f"[INFO]: Saved {saved_count} object-only OpenWorldTactile tactile RGB frame pairs.")
    finally:
        left_bridge.close()
        right_bridge.close()
        env.close()
