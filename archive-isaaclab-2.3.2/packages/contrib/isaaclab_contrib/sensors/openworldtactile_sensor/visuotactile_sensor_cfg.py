...
# 中文说明：视觉触觉传感器配置文件，定义 GelSight/OpenWorldTactile 参数，并加入 OpenWorldTactile RGB 渲染所需的颜色、压力和位移缩放参数。
...
OWT_ASSET_ROOT = os.environ.get(
    "OWT_ASSET_ROOT",
    f"{ISAACLAB_NUCLEUS_DIR}/OpenWorldTactile",
)
...
class GelSightRenderCfg:
    ...
    Example:
        Using predefined sensor configuration::
            ...
            from isaaclab_contrib.sensors.openworldtactile_sensor import VisuoTactileSensorCfg
    ...
    base_data_path: str = OWT_ASSET_ROOT
    """Base path to the directory containing sensor calibration data. Defaults to
    Isaac Lab Nucleus directory at ``{ISAACLAB_NUCLEUS_DIR}/OpenWorldTactile``.
    """
    ...
    openworldtactile_max_pressure: float = 1.0
    """Maximum depth/pressure value used to normalize OpenWorldTactile-style Hue rendering."""

    openworldtactile_base_hue: float = 0.0
    """OpenCV Hue value used for the no-contact OpenWorldTactile-style RGB baseline."""

    openworldtactile_max_hue: float = 120.0
    """OpenCV Hue value used for the maximum OpenWorldTactile-style pressure/depth value."""

    openworldtactile_saturation: int = 255
    """HSV saturation used by the OpenWorldTactile-style RGB renderer."""

    openworldtactile_base_value: int = 210
    """HSV value channel baseline used by the OpenWorldTactile-style RGB renderer."""

    openworldtactile_texture_strength: int = 36
    """Static value-channel texture strength used for OpenWorldTactile-style optical-flow features."""

    openworldtactile_texture_blur: int = 3
    """Gaussian blur kernel size for the OpenWorldTactile-style value-channel texture."""

    openworldtactile_pressure_blur: int = 5
    """Gaussian blur kernel size for the OpenWorldTactile-style pressure/depth map."""

    openworldtactile_displacement_scale: float = 1.0
    """Scale factor for future OpenWorldTactile-style displacement rendering."""
...
class VisuoTactileSensorCfg(SensorBaseCfg):
    ...
    rgb_render_mode: str = "openworldtactile"
    """Camera tactile RGB renderer route. Supported values are ``"openworldtactile"`` and ``"gelsight"``."""
