...
parser = argparse.ArgumentParser(
    description=(
        "V5 new stage 3c: PhysX grasp plus baseline-cleaned UIPC Fz with UIPC_Pad mounted as a stable "
        ...
        "baseline suppression, then logs UIPC CylinderTool-to-membrane signed gap diagnostics. "
        "It intentionally does not create RGB tactile output, Fx/Fy output, or a NutTool."
    )
)
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_3c_contact_gap_debug")
parser.add_argument("--workspace_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_3c_workspace")
...
def _uipc_tool_enabled(phase_name: str) -> bool:
    phase_to_index = {name: index for index, name in enumerate(PHASE_ORDER)}
    enable_phase = str(args_cli.uipc_tool_enable_phase)
    return phase_to_index[str(phase_name)] >= phase_to_index[enable_phase]
...
def main() -> None:
    ...
    print(
        json.dumps(
            {
                "script_version": "v5_new_3c_contact_gap_debug",
    ...
    try:
        ...
        for phase in phases:
            ...
            for phase_frame in range(frame_count):
                ...
                object_pos_for_uipc, object_quat_for_uipc = _rigid_object_pose_np(cylinder)
                if not tool_enabled:
        ...
        metadata = {
            "script_version": "v5_new_3c_contact_gap_debug",
            "purpose": "mounted_pad_physx_grasp_with_uipc_fz_only_baseline_clean_and_contact_gap_debug",
