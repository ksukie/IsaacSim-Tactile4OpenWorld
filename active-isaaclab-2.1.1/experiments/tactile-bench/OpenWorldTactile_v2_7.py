...
parser = argparse.ArgumentParser(
    description=(
        "V2.7 OpenWorldTactile UIPC physical-sensor bench. It uses a fixed, more "
        "realistic soft-gel baseline, keeps deformation fxyz as a reference, "
        "and records that native UIPC contact force is not yet exposed."
    )
)
parser.add_argument(
    "--shape",
    ...
    help="Indenter geometry used to press the membrane. V2.7 defaults to the small cuboid contact tool.",
)
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_surface_force_v2_7")
parser.add_argument("--indent_depth_mm", type=float, default=0.5)
...
parser.add_argument(
    ...
    default=True,
    action="store_true",
    help="Run the bench without writing fxyz, metadata, preview frames, or videos. This is the V2.7 default.",
)
parser.add_argument(
    "--save_output",
    dest="no_save",
...
parser.add_argument(
    ...
    default=True,
    action="store_true",
    help="Repeat the press trajectory until the app closes or Ctrl+C is pressed. This is the V2.7 default.",
)
parser.add_argument(
    "--single_run",
    dest="loop_forever",
...
def main() -> None:
    ...
    if args_cli.display_tactile:
        if omni_ui is None:
            ...
            title = "OpenWorldTactile UIPC V2.7 Live fxyz"
            if display_tactile_camera:
                display_width += display_camera_panel_width
                title = "OpenWorldTactile UIPC V2.7 Live fxyz + visual camera"
    ...
    print(
        "[INFO] OpenWorldTactileBench V2.7 started: "
    ...
    if not should_save:
        print(
            "[INFO] OpenWorldTactileBench V2.7 complete: "
    ...
    metadata = {
        ...
        "physical_sensor_model": {
            "intent": "fixed_soft_gel_layer_baseline_not_visual_tuning",
