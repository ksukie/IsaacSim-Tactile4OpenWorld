...
from membrane_local_frame import (
    AUTHORITATIVE_MEMBRANE_PRIM_PATH,
    audit_membrane_frame,
    compute_front_vertex_area,
)
...
parser = argparse.ArgumentParser(
    description=(
        "Read-only visualization-diagnostic branch derived from the frozen 7g shear setup. It records "
        "membrane-local surface histories for -X/+X unload and signed Y/Z display cases; it does not "
        "replace or revise the frozen 7f/7g data contract."
    )
)
parser.add_argument("--asset_usd", type=str, default=str(DEFAULT_PAD_USD))
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_7g_membrane_visualization")
parser.add_argument("--workspace_dir", type=str, default="/tmp/openworldtactile_uipc_v5_new_7g_membrane_visualization_workspace")
...
def _validate_args() -> None:
    ...
    if float(args_cli.lateral_amplitude_mm) < 0.0:
        parser.error("--lateral_amplitude_mm must be >= 0.")
    for name in (
        "render_every",
        "log_every",
        "autosave_every",
...
def _write_initial_alignment(
    membrane: UipcObject,
    rest_vertices_membrane_l: np.ndarray,
    membrane_pos_w: np.ndarray,
    membrane_quat_w: tuple[float, float, float, float],
) -> None:
    aligned_vertices_w = _world_from_local(
        rest_vertices_membrane_l, membrane_pos_w, membrane_quat_w
    )
...
def _save_outputs(output_dir: Path, records: dict[str, list[object]], summary: dict[str, object]) -> None:
    ...
    np.save(output_dir / "membrane_pose_w.npy", np.asarray(records["membrane_pose_w"]))
    ...
    current_local = np.asarray(records["membrane_surface_current_local"], dtype=np.float32)
    deformation = np.asarray(records["membrane_displacement_local"], dtype=np.float32)
    np.save(output_dir / "membrane_surface_current_local.npy", current_local)
    np.save(output_dir / "membrane_displacement_local.npy", deformation)
...
def main() -> None:
    ...
    authored_membrane_pos_w, authored_membrane_quat_w = _stage_world_pose(
        stage, membrane_mesh_path
    )
    membrane_pos_pad_l, membrane_quat_pad_l = _relative_child_pose(
        authored_pad_pos_w,
        authored_pad_quat_w,
        authored_membrane_pos_w,
        authored_membrane_quat_w,
    )
    ...
    start_membrane_pos_w, start_membrane_quat_w = _compose_child_pose(
        start_pad_pos_w,
        start_pad_quat_w,
        membrane_pos_pad_l,
        membrane_quat_pad_l,
    ...
    # The tetrahedralization inverse recovers membrane_sim_mesh-local coordinates. It must not be
    # relabeled as Pad-local even when the current asset happens to author an identity child pose.
    rest_vertices_membrane_l = _local_from_world_matrix(
        stage_vertices_w,
        uipc_mesh_world_from_local,
    ).astype(np.float32)
    back_tet_indices, front_tet_indices, tet_thickness_m = _face_indices(rest_vertices_membrane_l)
    initial_back_world = _world_from_local(
        rest_vertices_membrane_l[back_tet_indices],
        start_membrane_pos_w,
        start_membrane_quat_w,
    )
    ...
    source_front_x_l = float(np.max(rest_vertices_membrane_l[:, 0]))
    source_front_center_l = np.mean(rest_vertices_membrane_l[front_tet_indices], axis=0)
    tool_center_membrane_l0 = np.asarray(
        (
    ...
    if tool_shape == "flat_box":
        tool_surface_membrane_l0, tool_triangles, tool_tetrahedra = _flat_box_surface_mesh_l(
            tool_center_membrane_l0,
            tool_half_width_m,
            tool_half_length_m,
        )
    else:
        tool_surface_membrane_l0, tool_triangles = _cylinder_surface_mesh_l(
            tool_center_membrane_l0,
            tool_radius_m,
            tool_half_length_m,
            int(args_cli.tool_segments),
        )
    tool_surface_w0 = _world_from_local(
        tool_surface_membrane_l0,
        start_membrane_pos_w,
        start_membrane_quat_w,
    )
    ...
    rest_surface_membrane_l = _local_from_world(
        _uipc_surface(membrane), start_membrane_pos_w, start_membrane_quat_w
    ).astype(np.float32)
    back_surface_indices, front_surface_indices, surface_thickness_m = _face_indices(
        rest_surface_membrane_l
    )
    ...
    tool_initial_surface_membrane_l = _local_from_world(
        tool_initial_surface_w, start_membrane_pos_w, start_membrane_quat_w
    )
    ...
    metadata = {
        "script_version": "OpenWorldTactile_v5_new_7g_local_visualization_probe",
        "diagnostic_only": True,
        "formal_7f_7g_contract_modified": False,
        "architecture": (
            "link8 articulation -> back-face UIPC attachment + frictional normal indentation "
            "-> signed membrane-local tangent tool motion -> surface deformation -> recovery"
        ),
        ...
        "rest_local_frame_source": "inverse of UipcObject.init_world_transform; membrane_sim_mesh local",
        "coordinate_frame": "membrane_local",
        "membrane_axes": {"+X": "outward_normal", "+Y": "tangent_1", "+Z": "tangent_2"},
        ...
        "tool_center_membrane_local_m": [float(value) for value in tool_center_membrane_l0],
        "indentation_levels_mm": _indentation_levels_mm(),
        "lateral_axis_membrane_local": str(args_cli.lateral_axis),
        "lateral_direction": str(args_cli.lateral_direction),
        "lateral_amplitude_mm": float(args_cli.lateral_amplitude_mm),
        "authoritative_membrane_prim_path": membrane_mesh_path,
        "membrane_pos_pad_local_m": membrane_pos_pad_l.tolist(),
        "membrane_quat_pad_local_wxyz": list(membrane_quat_pad_l),
        "membrane_world_transform_sampling": (
            "per-frame live Pad parent pose composed with authored membrane_sim_mesh local pose; "
            "USD-stage world xform is not live under PhysX/Fabric"
        ),
    ...
    surface_triangles = _uipc_surface_triangles(membrane)
    front_surface_mask = np.zeros(rest_surface_membrane_l.shape[0], dtype=bool)
    front_surface_mask[front_surface_indices] = True
    vertex_area, front_triangles = compute_front_vertex_area(
        rest_surface_membrane_l,
        surface_triangles,
        front_surface_mask,
    )
    np.save(output_dir / "surface_triangles.npy", surface_triangles)
    np.save(output_dir / "front_surface_triangles.npy", front_triangles)
    np.save(output_dir / "front_surface_mask.npy", front_surface_mask)
    np.save(output_dir / "vertex_area.npy", vertex_area)
    np.save(output_dir / "rest_tet_vertices_membrane_local.npy", rest_vertices_membrane_l)
    np.save(output_dir / "membrane_surface_rest_local.npy", rest_surface_membrane_l)
    ...
    records: dict[str, list[object]] = {
        ...
        "uipc_surface_pose_w": [],
        "uipc_surface_w": [],
        "membrane_surface_current_local": [],
        "membrane_displacement_local": [],
        "contact_vertex_mask": [],
    ...
    while simulation_app.is_running() and (bool(args_cli.loop_indentation) or frame_idx < total_frames):
        ...
        membrane_pos_w, membrane_quat_w = _compose_child_pose(
            pad_pos_w,
            pad_quat_w,
            membrane_pos_pad_l,
            membrane_quat_pad_l,
        ...
        tool_target_delta_membrane_l = np.asarray(
            (-tool_motion_depth_mm * 1.0e-3, 0.0, 0.0), dtype=np.float64
        )
        lateral_axis_index = 1 if str(args_cli.lateral_axis) == "y" else 2
        tool_target_delta_membrane_l[lateral_axis_index] = float(commanded_lateral_mm) * 1.0e-3
        tool_target_delta_w = _quat_to_matrix(membrane_quat_w) @ tool_target_delta_membrane_l
        tool_motion_state["translation_w"] = tool_target_delta_w
        manual_uipc_step()

        vertices_w = _uipc_vertices(membrane)
        surface_w = _uipc_surface(membrane)
        surface_membrane_l = _local_from_world(surface_w, membrane_pos_w, membrane_quat_w)
        surface_deformation_membrane_l = (
            surface_membrane_l - rest_surface_membrane_l
        ).astype(np.float32)
        ...
        tool_surface_membrane_l = _local_from_world(
            tool_surface_w, membrane_pos_w, membrane_quat_w
        )
        tool_actual_translation_w = np.mean(tool_surface_w - tool_initial_surface_w, axis=0)
        tool_actual_translation_membrane_l = (
            tool_actual_translation_w @ _quat_to_matrix(membrane_quat_w)
        )
        actual_lateral_mm = float(
            tool_actual_translation_membrane_l[lateral_axis_index] * 1000.0
        )
        actual_tool_motion_depth_mm = float(
            (
                np.min(tool_initial_surface_membrane_l[:, 0])
                - np.min(tool_surface_membrane_l[:, 0])
            )
            * 1000.0
        )
        ...
        contact_vertex_mask, contact_diagnostics = _contact_geometry_diagnostics(
            ...
            tool_min_x_l=float(np.min(tool_surface_membrane_l[:, 0])),
            tool_center_yz_l=np.mean(tool_surface_membrane_l[:, 1:3], axis=0),
        ...
        records["membrane_pose_w"].append(
            np.asarray([*membrane_pos_w, *membrane_quat_w], dtype=np.float32)
        )
        ...
        records["membrane_surface_current_local"].append(surface_membrane_l.astype(np.float32))
        records["membrane_displacement_local"].append(surface_deformation_membrane_l)
    ...
    max_camera_position_error_mm = _finite_max(records["camera_position_error_mm"])
    max_camera_orientation_error_deg = _finite_max(records["camera_orientation_error_deg"])
    final_deformation = np.asarray(records["membrane_displacement_local"][-1], dtype=np.float64)
    ...
    frame_audit = audit_membrane_frame(
        np.asarray(records["membrane_pose_w"], dtype=np.float64),
        np.asarray(records["pad_pose_w"], dtype=np.float64),
        rest_surface_membrane_l,
        front_triangles,
    )
    frame_contract = frame_audit.as_json()
    np.save(
        output_dir / "membrane_from_pad_rotation.npy",
        frame_audit.membrane_from_pad_rotation,
    )
    np.save(
        output_dir / "pad_from_membrane_rotation.npy",
        frame_audit.pad_from_membrane_rotation,
    )
    (output_dir / "membrane_frame_contract.json").write_text(
        json.dumps(frame_contract, indent=2, ensure_ascii=False) + "\n"
    )
    checks = {
        "authoritative_membrane_prim_path_is_frozen": (
            membrane_mesh_path == AUTHORITATIVE_MEMBRANE_PRIM_PATH
        ),
        **{
            f"membrane_frame_{name}": passed
            for name, passed in frame_contract["checks"].items()
        },
    ...
    verdict = {
        ...
        "thresholds": {
            ...
            "membrane_frame_contract": frame_contract["thresholds"],
        },
        "observed": {
            "back_constrained_coverage": back_constrained_coverage,
            "front_constrained_fraction": front_constrained_fraction,
            ...
            "tool_motion_update_count": int(tool_motion_state["update_count"]),
            "tool_motion_last_frame": int(tool_motion_state["last_frame"]),
            "membrane_frame_contract": frame_contract["observed"],
        },
        "force_source": "none",
        "pressure_source": "none",
    }
