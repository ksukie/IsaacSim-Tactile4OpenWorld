# Copyright (c) 2022-2025, The OpenWorldTactile Project Developers.
# All rights reserved.
#
...
from openworldtactile_uipc.utils import MeshGenerator, TetMeshCfg

from .uipc_object_deformable_data import UipcObjectDeformableData
from .uipc_object_rigid_data import UipcObjectRigidData

if TYPE_CHECKING:
    from openworldtactile_uipc.sim import UipcIsaacAttachmentsCfg, UipcSim
...
class UipcObject(AssetBase):
    ...
    def write_kinematic_abd_pose_pair_to_sim(
        ...
        """Write one kinematic ABD substep with preserved pose history.

        The transforms map this object's initial UIPC vertices into the world
        poses at the start and end of the substep.  Unlike the generic vertex
        reset API, this preserves ``q_prev``, ``dq``, and generalized velocity.
        """
        if self._system_name != "uipc::backend::cuda::AffineBodyDynamics":
            raise TypeError("Kinematic ABD pose pairs can only be written to an affine body")
        ...
        if previous.shape != (4, 4) or current.shape != (4, 4):
            raise ValueError("previous_transform and current_transform must have shape (4, 4)")
        if not np.isfinite(previous).all() or not np.isfinite(current).all():
            raise ValueError("Kinematic ABD transforms must be finite")
        if not np.isfinite(dt) or float(dt) <= 0.0:
            raise ValueError("dt must be finite and > 0")
        # AffineBodyDynamics addresses one-body object instances by body id,
        # unlike its generic vertex API which uses per-system vertex offsets.
        body_id = self.local_system_id - 1
        success = self.uipc_sim.world.write_kinematic_abd_pose_pair_to_sim(
            int(body_id), previous, current, float(dt)
        )
        if not success:
            raise RuntimeError(
                f"Failed to write kinematic ABD pose pair for body id {body_id}; "
                "the body must be fixed/kinematic and both transforms must be rigid"
            )

    def write_external_rigid_boundary_pose_to_sim(
        self,
        previous_transform: np.ndarray,
        current_transform: np.ndarray,
        dt: float,
    ) -> None:
        """Synchronize an externally owned rigid collision boundary.

        This is the coupling-facing API for a rigid body whose authoritative
        pose and velocity live in another simulator (for example PhysX).  The
        UIPC affine body must be fixed/kinematic: it contributes collision
        geometry only and never advances an independent rigid-body state.

        Both poses are required so UIPC receives the external boundary motion
        history instead of a zero-velocity teleport.  The collision manager
        receives the same previous/current vertex pair for friction history,
        while the external simulator remains the single source of rigid-body
        state.
        """
        if self._system_name != "uipc::backend::cuda::AffineBodyDynamics":
            raise TypeError("External rigid boundaries must use an affine body")
        if not bool(getattr(self.cfg.constitution_cfg, "kinematic", False)):
            raise ValueError("External rigid boundaries must be configured kinematic=True")
        previous = np.asarray(previous_transform, dtype=np.float64)
        current = np.asarray(current_transform, dtype=np.float64)
        if previous.shape != (4, 4) or current.shape != (4, 4):
            raise ValueError("External boundary transforms must have shape (4, 4)")
        if not np.isfinite(previous).all() or not np.isfinite(current).all():
            raise ValueError("External boundary transforms must be finite")
        if not np.isfinite(dt) or float(dt) <= 0.0:
            raise ValueError("External boundary dt must be finite and > 0")
        body_id = self.local_system_id - 1
        pose_success = self.uipc_sim.world.write_kinematic_abd_pose_pair_to_sim(
            int(body_id), previous, current, float(dt)
        )
        if not pose_success:
            raise RuntimeError(
                f"Failed to synchronize external rigid boundary body id {body_id}; "
                "the boundary must be fixed/kinematic and both transforms must be rigid"
            )

        # AffineBodyDynamics and the collision system own separate device
        # buffers.  A fixed ABD body's Newton displacement is zero, so writing
        # q/q_prev alone cannot move the collision vertices.  Mirror the same
        # pose pair into GlobalVertexManager explicitly.
        initial_positions = self.init_vertex_pos.detach().cpu().numpy().astype(
            np.float64, copy=False
        )
        previous_positions = (
            initial_positions @ previous[:3, :3].T + previous[:3, 3]
        )
        current_positions = (
            initial_positions @ current[:3, :3].T + current[:3, 3]
        )
        geo = self.geo_slot_list[0].geometry()
        global_vertex_offset_attr = geo.meta().find(builtin.global_vertex_offset)
        if global_vertex_offset_attr is None:
            raise RuntimeError(
                "External rigid boundary has no global vertex offset; "
                "UIPC setup_sim() must run before synchronization"
            )
        global_vertex_offset = int(global_vertex_offset_attr.view()[0])
        collision_success = self.uipc_sim.world.write_global_vertex_pos_pair_to_sim(
            previous_positions,
            current_positions,
            global_vertex_offset,
            self._vertex_count,
        ...
        if not collision_success:
            raise RuntimeError(
                f"Failed to synchronize collision vertices for external rigid boundary "
                f"body id {body_id} at global vertex offset {global_vertex_offset}"
            )

    def read_kinematic_abd_state_from_sim(self) -> np.ndarray:
        """Return ``[q_prev, q, q_temp, q_tilde, dq, q_v]`` for diagnostics."""
        if self._system_name != "uipc::backend::cuda::AffineBodyDynamics":
            raise TypeError("Kinematic ABD state is only available for an affine body")
        state = np.asarray(
            self.uipc_sim.world.read_kinematic_abd_state_from_sim(
                self.local_system_id - 1
            ),
            dtype=np.float64,
        )
        if state.shape != (6, 12):
            raise RuntimeError(f"Unexpected kinematic ABD state shape: {state.shape}")
        return state
