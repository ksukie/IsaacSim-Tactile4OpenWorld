...
parser = argparse.ArgumentParser(
    description=(
        "V5.2 contact smoke. Native Piper carries UIPC_Pad on link7, a kinematic "
        "UIPC-only NutTool starts outside the membrane, then moves a small distance "
        "along the membrane normal to verify that only contact produces Fz."
    )
)
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_uipc_v5_2_contact_smoke_static_tool")
parser.add_argument("--workspace_dir", type=str, default="/tmp/openworldtactile_uipc_v5_2_contact_smoke_workspace")
...
parser.add_argument("--tool_press_frames", type=int, default=30)
parser.add_argument("--tool_hold_frames", type=int, default=30)
...
parser.add_argument("--tool_setup_gap_mm", type=float, default=3.0)
parser.add_argument("--tool_press_depth_mm", type=float, default=0.3)
...
def _validate_args() -> None:
    ...
    for name in (
        ...
        if int(getattr(args_cli, name)) < 0:
            parser.error(f"--{name} must be >= 0.")
    if int(args_cli.tool_press_frames) < 1:
        parser.error("--tool_press_frames must be >= 1.")
...
def main() -> None:
    ...
    fixed_pad_pos = pad_pos_t.detach().cpu().numpy().astype(np.float64)
    fixed_pad_quat = _quat_normalize(tuple(float(v) for v in pad_quat_t.detach().cpu().numpy()))
    fixed_anchor_center_w = _world_from_local(anchor_center_l.reshape(1, 3), fixed_pad_pos, fixed_pad_quat)[0]
    _move_root_no_reset(anchor, fixed_anchor_center_w, fixed_pad_quat, device=sim.device)
    anchor.update(0.0)
    ...
    tool_n_w = _quat_apply(fixed_pad_quat, tool_n_l)
    ...
    tool_initial_center_w = front_center_w + tool_n_w * (
        tool_safe_radius + float(args_cli.tool_setup_gap_mm) * 1.0e-3
    )
    ...
    tool_surface_points_w = tool_surface_points + tool_initial_center_w.reshape(1, 3)
    ...
    membrane_points_w = _world_from_local(membrane_points_l, fixed_pad_pos, fixed_pad_quat)
    visual_points_w = _world_from_local(visual_grid_points, fixed_pad_pos, fixed_pad_quat)
    ...
    rest_pad_pos = fixed_pad_pos.copy()
    rest_pad_quat = fixed_pad_quat
    ...
    total_frames = int(args_cli.pre_contact_frames) + int(args_cli.tool_press_frames) + int(args_cli.tool_hold_frames)
    current_opening_mm = open_mm
    for frame_id in range(total_frames):
        if not simulation_app.is_running():
            break
        if frame_id < int(args_cli.pre_contact_frames):
            tool_motion_depth_mm = 0.0
            tool_press_depth_mm = 0.0
            phase = "pre_contact"
        elif frame_id < int(args_cli.pre_contact_frames) + int(args_cli.tool_press_frames):
            rel = frame_id - int(args_cli.pre_contact_frames)
            alpha = _smoothstep01(float(rel) / float(max(1, int(args_cli.tool_press_frames) - 1)))
            tool_motion_depth_mm = (float(args_cli.tool_setup_gap_mm) + float(args_cli.tool_press_depth_mm)) * alpha
            tool_press_depth_mm = max(0.0, tool_motion_depth_mm - float(args_cli.tool_setup_gap_mm))
            phase = "press"
        else:
            tool_motion_depth_mm = float(args_cli.tool_setup_gap_mm) + float(args_cli.tool_press_depth_mm)
            tool_press_depth_mm = float(args_cli.tool_press_depth_mm)
            phase = "hold"

        tool_offset_w = -tool_n_w * (tool_motion_depth_mm * 1.0e-3)
        tool_vertices_w = tool_init_vertices_w + tool_offset_w.reshape(1, 3).astype(np.float32)
        ...
        current_surface_l = _local_from_world(current_surface_w, fixed_pad_pos, fixed_pad_quat)
        ...
        if frame_id % max(1, int(args_cli.log_every)) == 0 or frame_id == total_frames - 1:
            print(
                ...
                f"tool_motion_depth_mm={tool_motion_depth_mm:.6f} "
                f"tool_press_depth_mm={tool_press_depth_mm:.6f} "
    ...
    if not local_fxyz_frames:
        raise RuntimeError("No v5_2 contact smoke frames were collected.")
    ...
    metadata = {
        "script_version": "v5_2_static_pad_contact_smoke",
        "pad_only": False,
        "contact_smoke": True,
        "static_pad_contact_smoke": True,
        "uipc_solver_used": True,
        "force_source": "deformation_proxy_fz_only",
        "deformation_source": "uipc_membrane_surface_static_tool_contact",
        ...
        "attachment": {
            ...
            "anchor_follows": None,
            "anchor_fixed_from": pad_motion_root,
            "anchor_static_after_setup": True,
            "anchor_from_back_face": True,
            "anchor_local_center_m": [float(v) for v in anchor_center_l],
            "anchor_fixed_world_center_m": [float(v) for v in fixed_anchor_center_w],
            "anchor_size_m": [float(v) for v in anchor_size],
        },
        "fixed_pad_pose": {
            "source": pad_motion_root,
            "pos_w_m": [float(v) for v in fixed_pad_pos],
            "quat_wxyz": [float(v) for v in fixed_pad_quat],
        ...
        "uipc_tool": {
            ...
            "target_penetration_depth_m": float(args_cli.tool_press_depth_mm) * 1.0e-3,
            "total_motion_depth_m": (float(args_cli.tool_setup_gap_mm) + float(args_cli.tool_press_depth_mm)) * 1.0e-3,
            "initial_center_w_m": [float(v) for v in tool_initial_center_w],
            "press_direction_w": [float(v) for v in (-tool_n_w)],
        },
        "pad_mount": {
