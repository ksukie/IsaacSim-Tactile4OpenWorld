...
parser = argparse.ArgumentParser(
    description=(
        "V2.4 OpenWorldTactile UIPC membrane surface force bench. It keeps the V2.3 "
        "surface-force fxyz path stable and can optionally save visual-only "
        "internal camera RGB from a non-physical texture skin."
    )
)
parser.add_argument(
    "--shape",
    ...
    help="Indenter geometry used to press the membrane. V2.4 defaults to a small cuboid contact tool.",
)
parser.add_argument("--output_dir", type=str, default="/tmp/openworldtactile_surface_force_v2_4")
...
parser.add_argument(
    ...
    default=True,
    action="store_true",
    help="Run the bench without writing fxyz, metadata, preview frames, or videos. This is the V2.4 default.",
)
parser.add_argument(
    "--save_output",
    dest="no_save",
...
parser.add_argument(
    ...
    default=True,
    action="store_true",
    help="Repeat the press trajectory until the app closes or Ctrl+C is pressed. This is the V2.4 default.",
)
parser.add_argument(
    "--single_run",
    dest="loop_forever",
...
def _make_tool(shape: str, args: argparse.Namespace) -> ToolSpec:
    ...
    if shape == "hollow_frame":
        points, triangles, max_protrusion = _heightfield_tool_mesh(
            ...
            max(args.texture_bump_height_mm, EPS) * 1.0e-3,
            rng,
            ny=64,
            nz=64,
        )
        return ToolSpec(points, triangles, "hollow_frame", -max_protrusion, max_protrusion)
    points, triangles, max_protrusion = _heightfield_tool_mesh(
        shape,
...
def main() -> None:
    ...
    if args_cli.display_tactile:
        if omni_ui is None:
            ...
            title = "OpenWorldTactile UIPC V2.4 Live fxyz"
            if display_tactile_camera:
                display_width += display_camera_panel_width
                title = "OpenWorldTactile UIPC V2.4 Live fxyz + visual camera"
    ...
    print(
        "[INFO] OpenWorldTactileBench V2.4 started: "
    ...
    if not should_save:
        print(
            "[INFO] OpenWorldTactileBench V2.4 complete: "
