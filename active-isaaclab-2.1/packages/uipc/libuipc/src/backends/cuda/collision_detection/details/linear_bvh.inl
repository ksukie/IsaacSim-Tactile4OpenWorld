...
{
template <typename T>
MUDA_INLINE MUDA_GENERIC constexpr const T& bvh_min(const T& a, const T& b) noexcept
{
    return b < a ? b : a;
}

template <typename T>
MUDA_INLINE MUDA_GENERIC constexpr const T& bvh_max(const T& a, const T& b) noexcept
{
    return a < b ? b : a;
}
                                              ...
                                              uint32_t       idx)
{
    ...
    const int delta_min = bvh_min(L_delta, R_delta);
    int       l_max     = 2;
    int       delta     = -1;
    ...
    on(s)
        ...
        .apply(num_internal_nodes,
                ...
                num_objects] __device__(int idx) mutable
               {
                   ...
                   if(detail::bvh_min(ij.x, ij.y) == gamma)
                   {
                       nodes(idx).left_idx += num_objects - 1;
                   }
                   if(detail::bvh_max(ij.x, ij.y) == gamma + 1)
                   {
