...
TACTILE_PHYSX_DISTRIBUTION_EPS = 1.0e-9
...
class CylinderContainerSceneCfg(DirectRLEnvCfg):
    ...
    openworldtactile_contact = ContactSensorCfg(
        prim_path="/World/envs/env_.*/Robot/openworldtactile_case_left",
        update_period=0.0,
        history_length=0,
        track_pose=False,
...
class CylinderContainerSceneEnv(DirectRLEnv):
    ...
    def _maybe_log_physx_tactile_contact_debug(
        self,
        filtered_force_w: torch.Tensor,
        net_force_w: torch.Tensor | None,
        selected_force_w: torch.Tensor,
        force_source: str,
        contact_weight: torch.Tensor,
        signed_distance: torch.Tensor,
        force_matrix_w: torch.Tensor | None = None,
        weight_source: str = "软膜距离带",
        alignment_text: str = "",
    ):
        step = int(getattr(self, "step_count", 0))
        if step % TACTILE_PHYSX_DEBUG_INTERVAL_STEPS != 0:
            return

        filtered_norm = torch.linalg.norm(filtered_force_w, dim=-1)
        filtered_force_norm = float(filtered_norm[0].item())
        net_force_norm = float("nan")
        if net_force_w is not None and net_force_w.numel() > 0:
            net_force_norm = float(torch.linalg.norm(net_force_w, dim=-1)[0].item())
        selected_force_norm = float(torch.linalg.norm(selected_force_w, dim=-1)[0].item())
        contact_weight_sum = float(torch.sum(contact_weight[0]).item())
        contact_weight_max = float(torch.max(contact_weight[0]).item())
        min_signed_distance = float(torch.min(signed_distance[0]).item())
        filter_force_norms = "none"
        if force_matrix_w is not None and force_matrix_w.numel() > 0:
            filter_norm_tensor = torch.linalg.norm(force_matrix_w[0, 0], dim=-1)
            filter_force_norms = ",".join(f"{float(value.item()):.6f}" for value in filter_norm_tensor)

        should_print = (
            step <= TACTILE_PHYSX_DEBUG_ALWAYS_UNTIL_STEP
            or filtered_force_norm > TACTILE_PHYSX_CONTACT_FORCE_EPS
            or net_force_norm > TACTILE_PHYSX_CONTACT_FORCE_EPS
            or contact_weight_sum > TACTILE_PHYSX_CONTACT_FORCE_EPS
        )
        if not should_print:
            return
        ...
        print(
            "[INFO] 触觉接触调试 -> "
            f"步数={step}, "
            f"过滤接触力范数={filtered_force_norm:.6f}, "
            f"总接触力范数={net_force_norm:.6f}, "
            f"实际使用力范数={selected_force_norm:.6f}, "
            f"力来源={force_source}, "
            f"权重来源={weight_source}, "
            f"软膜权重和={contact_weight_sum:.3f}, "
            f"软膜最大权重={contact_weight_max:.3f}, "
            f"圆柱到采样面的最小距离={min_signed_distance:.6f}",
            f"各过滤形状力范数=[{filter_force_norms}]",
            alignment_text,
            flush=True,
        )
    ...
    def _compute_sdf_tactile_force_field(self):
        ...
        if not hasattr(self, "_tactile_physx_model_logged"):
            self._tactile_physx_model_logged = True
            print(
                "[INFO] 触觉力模型 -> 硬膜提供 PhysX 总接触力，软膜距离带把总力分布到 300x300。"
                "硬膜厚度等于原始膜厚 h；软膜总厚度为 2h，其中前 h 与硬膜重叠，新增透明 h 只用于分布权重。"
                f"写入触觉图前硬膜力会乘以输出比例 {TACTILE_PHYSX_FORCE_OUTPUT_SCALE:.1e}。",
                flush=True,
            )
        ...
        for openworldtactile_sensor in [self.openworldtactile_left]:
            ...
            contact_band = max(_openworldtactile_sdf_contact_band(openworldtactile_sensor.cfg), TACTILE_PHYSX_DISTRIBUTION_EPS)
            contact_weight = torch.clamp((contact_band - signed_distance) / contact_band, min=0.0, max=1.0)

            if not hasattr(self, "openworldtactile_contact"):
                raise RuntimeError("当前触觉模型需要 openworldtactile_contact 传感器来读取硬膜接触力。")

            force_matrix_w = self.openworldtactile_contact.data.force_matrix_w
            if force_matrix_w is None or force_matrix_w.numel() == 0:
                raise RuntimeError("openworldtactile_contact 没有 force_matrix_w，无法读取过滤后的硬膜接触力。")

            physx_force_w = force_matrix_w[:, 0, :, :].sum(dim=1)
            physx_force_w = torch.nan_to_num(physx_force_w, nan=0.0, posinf=0.0, neginf=0.0)
            net_force_w = None
            if self.openworldtactile_contact.data.net_forces_w is not None and self.openworldtactile_contact.data.net_forces_w.numel() > 0:
                net_force_w = self.openworldtactile_contact.data.net_forces_w[:, 0, :]
                net_force_w = torch.nan_to_num(net_force_w, nan=0.0, posinf=0.0, neginf=0.0)
            selected_force_w = physx_force_w
            force_source = "过滤接触力"
            if TACTILE_PHYSX_USE_NET_FORCE_FALLBACK and net_force_w is not None:
                filtered_norm = torch.linalg.norm(physx_force_w, dim=-1, keepdim=True)
                net_norm = torch.linalg.norm(net_force_w, dim=-1, keepdim=True)
                use_net_force = (filtered_norm <= TACTILE_PHYSX_CONTACT_FORCE_EPS) & (
                    net_norm > TACTILE_PHYSX_CONTACT_FORCE_EPS
                )
                selected_force_w = torch.where(use_net_force, net_force_w, physx_force_w)
                if bool(use_net_force[0].item()):
                    force_source = "总接触力兜底"
            if torch.linalg.norm(selected_force_w[0]).item() <= TACTILE_PHYSX_CONTACT_FORCE_EPS:
                force_source = "无接触力"
            output_force_w = selected_force_w * TACTILE_PHYSX_FORCE_OUTPUT_SCALE
            physx_force_local = math_utils.quat_apply_inverse(sensor_quat_w, output_force_w)
            ...
            soft_front_x = hard_front_x + contact_band
            ...
            alignment_text = (
                f" 膜范围x=[{hard_back_x:.5f},{hard_front_x:.5f}], 透明软膜前缘x={soft_front_x:.5f}, "
            ...
            weight_sum = torch.sum(contact_weight, dim=(1, 2), keepdim=True)
            has_sdf_contact = weight_sum > TACTILE_PHYSX_DISTRIBUTION_EPS
            has_physx_contact = (
                torch.linalg.norm(selected_force_w, dim=-1, keepdim=True) > TACTILE_PHYSX_CONTACT_FORCE_EPS
            )
            weight_source = "软膜距离带"
            if TACTILE_SDF_USE_NEAREST_WEIGHT_FALLBACK:
                needs_weight_fallback = has_physx_contact[:, None, :] & ~has_sdf_contact
                if bool(needs_weight_fallback[0, 0, 0].item()):
                    relative_distance = signed_distance - signed_distance.amin(dim=(1, 2), keepdim=True)
                    fallback_sigma = max(
                        contact_band * TACTILE_SDF_NEAREST_FALLBACK_SIGMA_RATIO,
                        TACTILE_PHYSX_DISTRIBUTION_EPS,
                    )
                    nearest_weight = torch.exp(
                        -torch.clamp(relative_distance, min=0.0) ** 2 / (2.0 * fallback_sigma**2)
                    )
                    contact_weight = torch.where(needs_weight_fallback, nearest_weight, contact_weight)
                    weight_sum = torch.sum(contact_weight, dim=(1, 2), keepdim=True)
                    has_sdf_contact = weight_sum > TACTILE_PHYSX_DISTRIBUTION_EPS
                    weight_source = "最近区域兜底"
            self._maybe_log_physx_tactile_contact_debug(
                physx_force_w,
                net_force_w,
                selected_force_w,
                force_source,
            ...
            weights = torch.where(
                has_sdf_contact,
                contact_weight / weight_sum.clamp_min(TACTILE_PHYSX_DISTRIBUTION_EPS),
                torch.zeros_like(contact_weight),
            )
            valid_contact = has_sdf_contact & has_physx_contact[:, None, :]
            weights = torch.where(valid_contact, weights, torch.zeros_like(weights))

            pixel_force_local = weights.unsqueeze(-1) * physx_force_local[:, None, None, :]
            pixel_force_local = torch.nan_to_num(pixel_force_local, nan=0.0, posinf=0.0, neginf=0.0)

            force_field[..., 0] = -pixel_force_local[..., 1]
            force_field[..., 1] = -pixel_force_local[..., 2]
            force_field[..., 2] = torch.abs(pixel_force_local[..., 0])
    ...
    def _setup_scene(self):
        ...
        if self.cfg.enable_tactile:
            _enable_openworldtactile_physx_contact_reporting(self.cfg.openworldtactile_left)
        if self.device == "cpu":
        ...
        if self.cfg.enable_tactile:
            ...
            self.openworldtactile_contact = ContactSensor(self.cfg.openworldtactile_contact)
            self.scene.sensors["openworldtactile_contact"] = self.openworldtactile_contact
...
class HDF5EpisodeRecorder:
    ...
    def close_episode(self, env: CylinderContainerSceneEnv | None = None):
        ...
        with h5py.File(episode_path, "w") as h5:
            ...
            h5.attrs["tactile_force_model"] = "硬膜提供PhysX总接触力_软膜距离带分布到触觉图"
            h5.attrs["tactile_physx_contact_source"] = "硬膜与圆柱的接触力_过滤失败时用总接触力兜底"
            h5.attrs["tactile_virtual_gelpad_local_pos"] = virtual_gelpad_local_pos
            h5.attrs["tactile_virtual_gelpad_size_m"] = virtual_gelpad_size
            h5.attrs["tactile_sdf_contact_band_m"] = sdf_contact_band
            h5.attrs["tactile_sdf_sampling_surface_depth_m"] = sdf_sampling_surface_depth
            h5.attrs["tactile_sdf_extra_virtual_band_m"] = sdf_contact_band
            h5.attrs["tactile_softpad_total_thickness_m"] = float(virtual_gelpad_size[0])
            h5.attrs["tactile_hard_softpad_overlap_m"] = hard_softpad_thickness
            h5.attrs["tactile_physx_softpad_shape"] = "硬膜厚度为h_软膜总厚度为2h_前h重叠_后h透明"
            h5.attrs["tactile_physx_softpad_local_pos"] = physx_softpad_local_pos
            h5.attrs["tactile_physx_softpad_size_m"] = physx_softpad_size
            h5.attrs["tactile_physx_softpad_thickness_ratio"] = float(OWT_PHYSX_SOFTPAD_THICKNESS_RATIO)
            h5.attrs["tactile_physx_softpad_surface_inset_ratio"] = float(OWT_PHYSX_SOFTPAD_SURFACE_INSET_RATIO)
            h5.attrs["tactile_physx_softpad_friction"] = float(OWT_PHYSX_SOFTPAD_FRICTION)
            h5.attrs["tactile_physx_softpad_density"] = float(OWT_PHYSX_SOFTPAD_DENSITY)
            h5.attrs["tactile_physx_softpad_contact_offset"] = float(OWT_PHYSX_SOFTPAD_CONTACT_OFFSET)
            h5.attrs["tactile_physx_softpad_rest_offset"] = float(OWT_PHYSX_SOFTPAD_REST_OFFSET)
            h5.attrs["tactile_physx_softpad_compliant_stiffness"] = float(
                OWT_PHYSX_SOFTPAD_COMPLIANT_STIFFNESS
            )
            h5.attrs["tactile_physx_softpad_compliant_damping"] = float(OWT_PHYSX_SOFTPAD_COMPLIANT_DAMPING)
            h5.attrs["tactile_distribution_weight"] = "软膜距离带归一化权重"
            h5.attrs["tactile_physx_uses_net_force_fallback"] = bool(TACTILE_PHYSX_USE_NET_FORCE_FALLBACK)
            h5.attrs["tactile_physx_force_output_scale"] = float(TACTILE_PHYSX_FORCE_OUTPUT_SCALE)
            h5.attrs["tactile_physx_filter_shapes_are_summed"] = True
            h5.attrs["tactile_sdf_uses_nearest_weight_fallback"] = bool(TACTILE_SDF_USE_NEAREST_WEIGHT_FALLBACK)
            h5.attrs["tactile_fx_fy_meaning"] = "硬膜接触力分布到触觉图平面方向"
            h5.attrs["tactile_fz_meaning"] = "硬膜接触力分布到触觉图法向强度"
...
def run_simulator(
    ...
    while simulation_app.is_running():
        ...
        if env.cfg.enable_tactile:
            env.openworldtactile_contact.update(dt=env.physics_dt, force_recompute=True)
