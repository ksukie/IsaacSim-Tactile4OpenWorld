...
# 中文说明：这个脚本演示两个竖直相对的 OpenWorldTactile/GelSight 触觉传感器先夹住竖起的螺母，再只移动传感器本体上提；螺母孔轴对准左右传感器按压方向，中间孔洞应在触觉图中保留为空。螺母不会被代码绑定跟随，是否被带起完全由接触和摩擦决定。UI 实时展示 GelSight RGB、OpenWorldTactile RGB、GelSight/OpenWorldTactile force field，以及 OpenWorldTactile RGB 输入 SDK 解算出的 FXYZ。
...
parser = argparse.ArgumentParser(
    description="Standalone dual OpenWorldTactile physical clamp-and-lift demo with GelSight/OpenWorldTactile RGB and FXYZ UI."
)
...
parser.add_argument(
    ...
    type=float,
    default=-0.002,
    help="Closed distance from center to each tactile surface; negative values intentionally over-close the pads.",
)
parser.add_argument("--surface_height", type=float, default=0.14, help="World Z height of the tactile surface centers.")
parser.add_argument("--nut_height", type=float, default=0.14, help="World Z height of the nut center.")
parser.add_argument(
    "--nut_root_rpy",
    type=float,
    nargs=3,
    default=(-math.pi * 0.5, 0.0, 0.0),
    help="Nut root RPY. Default stands the nut upright with its hole axis aligned to the left/right pads.",
)
parser.add_argument(
...
parser.add_argument(
    ...
    action="store_true",
    help="Enable gravity on the nut. Default keeps gravity off so the first physical-lift test is easier to stabilize.",
)
parser.add_argument(
...
TACTILE_ROWS = 20
TACTILE_COLS = 25
WINDOW_TITLE = "Dual OpenWorldTactile Physical Lift GelSight/OpenWorldTactile RGB + FXYZ"
...
NUT_OBJECT_QUAT = quat_from_rpy(tuple(args_cli.nut_root_rpy))
...
class DualOpenWorldTactileClampSceneCfg(InteractiveSceneCfg):
    ...
    contact_object = RigidObjectCfg(
        ...
        init_state=RigidObjectCfg.InitialStateCfg(pos=(0.0, 0.0, args_cli.nut_height), rot=NUT_OBJECT_QUAT),
    )
...
def set_pad_pose(scene: InteractiveScene, half_gap: float, lift_offset: float):
    """Move both tactile pads; the nut is not attached and must be carried by contact/friction."""
    # 方式 A 的核心约束：这里只写左右 OpenWorldTactile 的 root pose。
    # 不写 contact_object/root pose，因此螺母能否上移完全取决于 PhysX 接触、摩擦和闭合挤压力。
    device = scene.device
...
def clamp_phase(frame_count: int) -> tuple[float, float, str]:
    ...
    # 返回的 lift_offset 只作用于左右传感器；螺母不会被代码绑定跟随。
    # closed_half_gap 可以为负数，表示两个触觉表面目标位置越过中心线，给螺母提供额外夹紧量。
    ...
    if alpha < 1.0:
        ...
        # 如果夹持力/摩擦足够，螺母会被接触带起来；否则会滑落或留在原处。
...
def main():
    ...
    sim.reset()
    print("[INFO]: Setup complete. Dual OpenWorldTactile physical lift GelSight/OpenWorldTactile RGB + FXYZ UI is live.")
